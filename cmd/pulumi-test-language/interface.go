...
func (eng *languageTestServer) PrepareLanguageTests(
	...
	pctx, err := plugin.NewContextWithRoot(ctx, snk, snk, nil, "", "", nil, false, nil, nil, nil, nil, nil, nil)
	if err != nil {
...
func (eng *languageTestServer) RunLanguageTest(
	...
	pctx, err := plugin.NewContextWithRoot(
		ctx, snk, snk, nil, token.TemporaryDirectory, token.TemporaryDirectory, nil, false, nil, nil, nil, nil, nil, nil)
	if err != nil {
		return nil, fmt.Errorf("setup plugin context: %w", err)
	}
