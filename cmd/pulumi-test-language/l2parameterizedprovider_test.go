...
type L2ParameterizedProvider struct {
	pulumirpc.UnimplementedLanguageRuntimeServer

	tempDir string
	// Skip creating the pkg resource in Run
	skipResource bool
	// Skip returning the pkg resource plugin in GetRequiredPlugins
	skipRequiredPlugins bool
}

func (h *L2ParameterizedProvider) Pack(
	...
	if req.PackageDirectory == filepath.Join(h.tempDir, "sdks", "pkg-1.0.0") {
		return &pulumirpc.PackResponse{
			ArtifactPath: filepath.Join(req.DestinationDirectory, "pkg-1.0.0.sdk"),
		}, nil
...
func (h *L2ParameterizedProvider) GenerateProject(
	...
	if req.LocalDependencies["pkg"] != filepath.Join(h.tempDir, "artifacts", "pkg-1.0.0.sdk") {
		return nil, fmt.Errorf("unexpected pkg sdk %s", req.LocalDependencies["pkg"])
	}
	if !req.Strict {
		return nil, errors.New("expected strict to be true")
	}
	if req.TargetDirectory != filepath.Join(h.tempDir, "projects", "l2-parameterized-provider") {
	...
	if project.Name != "l2-parameterized-provider" {
...
func (h *L2ParameterizedProvider) GeneratePackage(
	ctx context.Context, req *pulumirpc.GeneratePackageRequest,
) (*pulumirpc.GeneratePackageResponse, error) {
	if req.LocalDependencies["pulumi"] != filepath.Join(h.tempDir, "artifacts", "core.sdk") {
		return nil, fmt.Errorf("unexpected core sdk %s", req.LocalDependencies["pulumi"])
	}
	if req.Directory != filepath.Join(h.tempDir, "sdks", "pkg-1.0.0") {
...
func (h *L2ParameterizedProvider) GetRequiredPlugins(
	ctx context.Context, req *pulumirpc.GetRequiredPluginsRequest,
) (*pulumirpc.GetRequiredPluginsResponse, error) {
	if req.Info.ProgramDirectory != filepath.Join(h.tempDir, "projects", "l2-parameterized-provider") {
...
func (h *L2ParameterizedProvider) GetProgramDependencies(
...
func (h *L2ParameterizedProvider) InstallDependencies(
	req *pulumirpc.InstallDependenciesRequest, server pulumirpc.LanguageRuntime_InstallDependenciesServer,
) error {
	if req.Info.RootDirectory != filepath.Join(h.tempDir, "projects", "l2-parameterized-provider") {
...
func (h *L2ParameterizedProvider) Run(
...
// Run a simple test with a parameterized package, this tests resources, reads and invokes.
func TestL2ParameterizedProvider(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	tempDir := t.TempDir()
	engine := &languageTestServer{}
	runtime := &L2ParameterizedProvider{tempDir: tempDir}
	...
	runResponse, err := engine.RunLanguageTest(ctx, &testingrpc.RunLanguageTestRequest{
		...
		Test:  "l2-parameterized-provider",
	})
	require.NoError(t, err)
