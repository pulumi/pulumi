...
export class Context {
    ...
    // Translates a TypeScript location into a LumiIL location.
    ...
    // Translates a TypeScript position into a LumiIL position (0 to 1 based).
    ...
    public newMissingLumifileError(path: string, dependency: boolean, exts: string[]): Diagnostic {
        let altExts: string = "";
        if (exts.length > 0) {
            path = path + exts[0];
            if (exts.length > 1) {
        ...
        return {
            ...
            code:     1,
            message:  `No ${dependency ? "dependency" : "source"} Lumifile was found at '${path}'${altExts}`,
        };
    }

    public newMissingLumipackNameError(path: string): Diagnostic {
        return {
            category: DiagnosticCategory.Error,
            code:     2,
            message:  `Lumipack '${path}' is missing a name`,
        };
    }

    public newMalformedLumifileError(path: string, err: Error): Diagnostic {
        return {
            category: DiagnosticCategory.Error,
            code:     3,
            message:  `Lumifile '${path}' is malformed: ${err}`,
        };
    }
    ...
    public newGeneratorsNotSupportedError(node: ts.Node): Diagnostic {
        return {
            category: DiagnosticCategory.Error,
            code:     101,
            message:  "Generator functions are not supported in the LumiJS subset",
            loc:      this.locationFrom(node),
        };
    }
    ...
    public newRestParamsNotSupportedError(node: ts.Node): Diagnostic {
        return {
            category: DiagnosticCategory.Error,
            code:     102,
            message:  "Rest-style parameters are not supported in the LumiJS subset",
            loc:      this.locationFrom(node),
        };
    }
    ...
    public newInvalidDeclarationStatementError(node: ts.Node): Diagnostic {
        return {
            category: DiagnosticCategory.Error,
            code:     500,
            message:  `Declaration node ${ts.SyntaxKind[node.kind]} isn't a valid LumiJS statement`,
            loc:      this.locationFrom(node),
        };
    }
    ...
    public newMissingDependencyError(pkg: tokens.PackageToken): Diagnostic {
        return {
            ...
            message:  `Package '${pkg}' was encountered during compilation, ` +
                `but wasn't listed as a dependency in the Lumifile`,
        };
    }
}
