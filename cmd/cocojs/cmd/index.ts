...
async function main(args: string[]): Promise<number> {
    ...
    if (format) {
        ...
        // Otherwise, by default, just use the Nut default extension.
        if (!format) {
            format = cocojs.pack.defaultFormatExtension;
        }
    }

    // Look up the marshaler for the given extension.  If none is found, it's an invalid extension.
    let marshaler: ((out: any) => string) | undefined = cocojs.pack.marshalers.get(format);
    if (!marshaler) {
        let formats: string = "";
        for (let supported of cocojs.pack.marshalers) {
            if (formats) {
                formats += ", ";
            }
            formats += supported[0];
        }
        console.error(`Unrecognized Nut format extension: ${format};`);
        console.error(`    available formats: ${formats}`);
        return -3;
    }

    // Now go ahead and compile.
    let result: cocojs.compiler.CompileResult = await cocojs.compiler.compile(path);
    ...
    if (result.pkg && cocojs.diag.success(result.diagnostics)) {
        ...
        if (outfile === "-") {
            ...
            // Now write out the Nutpack file, terminated with a newline.
