...
export class Context {
    ...
    public newAsyncNotSupportedError(node: ts.Node): Diagnostic {
        return {
            category: DiagnosticCategory.Error,
            code:     100,
            message:  "Async functions are not supported in the CocoJS subset",
    ...
    public newGeneratorsNotSupportedError(node: ts.Node): Diagnostic {
        return {
            category: DiagnosticCategory.Error,
            code:     101,
            message:  "Generator functions are not supported in the CocoJS subset",
            loc:      this.locationFrom(node),
        };
    }
    ...
    public newRestParamsNotSupportedError(node: ts.Node): Diagnostic {
        return {
            category: DiagnosticCategory.Error,
            code:     102,
            message:  "Rest-style parameters are not supported in the CocoJS subset",
            loc:      this.locationFrom(node),
        };
    }
    ...
    public newInvalidDeclarationStatementError(node: ts.Node): Diagnostic {
        return {
            category: DiagnosticCategory.Error,
            code:     500,
            message:  `Declaration node ${ts.SyntaxKind[node.kind]} isn't a valid CocoJS statement`,
            loc:      this.locationFrom(node),
        };
    }
    ...
    public newInvalidTypeError(node: ts.Node, ty: ts.Type): Diagnostic {
        ...
        return {
            category: DiagnosticCategory.Error,
            code:     501,
            message:  `Type ${name}(kind ${ts.TypeFlags[ty.flags]}) is not supported in CocoJS`,
            loc:      this.locationFrom(node),
        };
    }
