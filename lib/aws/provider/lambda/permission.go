...
func (p *permissionProvider) Query(ctx context.Context) ([]*lambda.PermissionItem, error) {
	return nil, nil
}
...
/*
	...
	for _, fun := range funcs.Functions {
		names = append(names, *fun.FunctionName)
	}

	var permissions []*lambda.Permission
	for _, functionName := range names {
		permission, err := p.Get(ctx, resource.ID(functionName))
		if err != nil {
			return nil, err
		}
