...
func printPropertyValue(
	...
	if v.IsNull() {
		...
		if a.IsText() {
			...
			a = resource.MassageUserProgramCodeAsset(a, debug)
			massaged := a.Text
...
func printAssetDiff(
	...
	if oldAsset.IsText() {
		if newAsset.IsText() {
			...
			oldAsset = resource.MassageUserProgramCodeAsset(oldAsset, debug)
			newAsset = resource.MassageUserProgramCodeAsset(newAsset, debug)

			massagedOldText := oldAsset.Text
			massagedNewText := newAsset.Text
