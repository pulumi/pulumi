...
type deploymentSourceFunc func(
	...
	target *deploy.Target, plugctx *plugin.Context, lifecycleHooks *deploy.LifecycleHooks) (deploy.Source, error)
...
func newDeployment(
	...
	lifecycleHooks := deploy.NewLifecycleHooks(plugctx.DialOptions)
	...
	source, err := opts.SourceFunc(
		cancelCtx, ctx.BackendClient, opts, proj, pwd, main, projinfo.Root, target, plugctx, lifecycleHooks)
	if err != nil {
	...
	if !opts.isImport {
		depl, err = deploy.NewDeployment(
			...
			localPolicyPackPaths, ctx.BackendClient, lifecycleHooks)
	} else {
