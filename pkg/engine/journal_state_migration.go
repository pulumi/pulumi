...
//
// The patches are complete resource states, not property-level diffs. Reference rewriting happens while the states are
// still typed and decrypted. Replay applies those values without rerunning the callback, interpreting SuccessorURNs, or
// traversing encrypted property data.
func (sm *JournalSnapshotManager) StateMigration(transaction *deploy.StateMigrationTransaction) error {
	...
	// operation. The currentNewResources map contains only the latest states that JournalReplayer would retain.
