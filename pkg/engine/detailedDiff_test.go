...
func TestTranslateDetailedDiffDiffsDuplicated(t *testing.T) {
	t.Parallel()
	var (
		// A = plugin.PropertyDiff{Kind: plugin.DiffAdd}
		D = plugin.PropertyDiff{Kind: plugin.DiffDelete}
	...
	diff := TranslateDetailedDiff(&StepEventMetadata{
		...
		Diffs:        diffs,
		Res:          &StepEventStateMetadata{Inputs: oldInputs, Outputs: state},
	}, false)
	...
	expected := &resource.ObjectDiff{
		...
		Updates: map[resource.PropertyKey]resource.ValueDiff{
			"privileges": {
				Old: resource.NewPropertyValue([]string{
					"CREATE EXTERNAL TABLE",
					"CREATE TABLE",
					"CREATE VIEW",
					"CREATE TEMPORARY TABLE",
				...
				New: resource.NewPropertyValue([]string{
					"USAGE",
				}),
				Array: &resource.ArrayDiff{
					Adds: map[int]resource.PropertyValue{},
					Deletes: map[int]resource.PropertyValue{
						0: resource.NewStringProperty("CREATE EXTERNAL TABLE"),
						1: resource.NewStringProperty("CREATE TABLE"),
						2: resource.NewStringProperty("CREATE VIEW"),
						3: resource.NewStringProperty("CREATE TEMPORARY TABLE"),
						4: resource.NewStringProperty("USAGE"),
					},
					Sames: map[int]resource.PropertyValue{},
					Updates: map[int]resource.ValueDiff{
						0: {
							Old: resource.NewStringProperty("CREATE EXTERNAL TABLE"),
							New: resource.NewStringProperty("USAGE"),
						},
					},
				},
			},
