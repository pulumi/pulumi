...
type StepEventMetadata struct {
	...
	Res          *StepEventStateMetadata               // latest state known for the resource (worst case, old).
	Keys         []resource.PropertyKey                // keys causing replacement (CreateStep and ReplaceStep only).
	Diffs        []resource.PropertyKey                // the keys causing diffs
	DetailedDiff map[property.Path]plugin.PropertyDiff // the rich, structured diff
