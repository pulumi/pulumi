...
// globsToPropertyPaths converts a slice of [property.Glob] (the form carried on in-memory state)
// to the legacy [resource.PropertyPath] slice consumed by the display and diff engines.
func globsToPropertyPaths(globs []property.Glob) []resource.PropertyPath {
	if len(globs) == 0 {
		return nil
	}
	out := make([]resource.PropertyPath, len(globs))
	for i, g := range globs {
		out[i] = resource.ToResourcePropertyPath(g)
	}
	return out
}
...
func makeStepEventStateMetadata(state *resource.State, debug bool, showSecrets bool) *StepEventStateMetadata {
	...
	return &StepEventStateMetadata{
		...
		HideDiffs:      globsToPropertyPaths(state.HideDiff),
	}
}
