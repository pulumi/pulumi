...
func TestStateMigrationRenameChild(t *testing.T) {
	...
	// The lifecycle harness generates an update plan for every preview. A changing migration cannot be represented
	// in that plan, and the rejected preview must not persist anything.
	_, err = lt.TestOp(Update).Run(
...
// TestStateMigrationFold exercises a many-to-one migration by folding an old component child into the renamed managed
...
// TestStateMigrationRejectsPredecessorURNReuse verifies that references captured before a committed migration cannot
// become ambiguous with a later resource that reuses the removed predecessor's primary URN. Predecessor aliases remain
// supported by TestStateMigrationAliasClaimsSuccessor above.
...
// TestStateMigrationUpdatePlan verifies that an update plan cannot describe or authorize a checkpoint rewrite,
// either while it is generated or while it is applied. No-op migrations remain safe so callbacks can stay attached
// after every stack has migrated.
...
// TestStateMigrationAcrossTypes tests that an explicit custom predecessor authorizes a type change while preserving
// the managed resource's physical identity.
