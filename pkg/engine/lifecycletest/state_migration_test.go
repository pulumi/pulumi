...
// TestStateMigrationFold exercises an N-to-M migration by folding two old children into one canonical successor.
// References from a resource outside the component subtree are rewritten instead of being dropped.
func TestStateMigrationFold(t *testing.T) {
	...
	programF := deploytest.NewLanguageRuntimeF(func(_ plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		if migrate {
			...
			childB, err := monitor.RegisterResource("pkgA:m:typA", "childB", true, deploytest.ResourceOptions{
				Parent: comp.URN,
				Inputs: resource.PropertyMap{"foo": resource.NewProperty("bar")},
			})
			if err != nil {
