...
func TestResourceHookDryRun(t *testing.T) {
	...
	programF := deploytest.NewLanguageRuntimeF(func(info plugin.RunInfo, monitor *deploytest.ResourceMonitor) error {
		...
		funTrue := func(ctx context.Context, urn resource.URN, id resource.ID, outputs resource.PropertyMap) error {
		...
		funFalse := func(ctx context.Context, urn resource.URN, id resource.ID, outputs resource.PropertyMap) error {
			assert.Equal(t, urn, resource.URN("urn:pulumi:test::test::pkgA:m:typA::resA"))
			if info.DryRun {
