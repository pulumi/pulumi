...
func ExcludeRefreshWithTargetedProviderParentChangeDestroyV2(
	snap *SnapshotSpec,
	prog *ProgramSpec,
	_ *ProviderSpec,
	plan *PlanSpec,
	...
	targetURNs := make(map[resource.URN]bool)
	for _, res := range plan.TargetURNs {
		targetURNs[res] = true
	}
	...
	for _, res := range prog.ResourceRegistrations {
		...
		snapParent, inSnap := snapParents[res.URN()]
		if inSnap && snapParent == res.Parent {
			continue
		}
...
func ExcludeComponentWithProviderRefreshProgram(
	...
	refreshProgramUpdate := plan.Operation == PlanOperationUpdate && plan.Refresh && plan.RefreshProgram
	if plan.Operation != PlanOperationRefreshV2 && !refreshProgramUpdate {
		return false
	}
