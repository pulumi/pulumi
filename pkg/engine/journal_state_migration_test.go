...
func TestJournalStateMigrationPersistsTypedPatches(t *testing.T) {
	...
	transaction := &deploy.StateMigrationTransaction{
		RootURN:                  old.URN,
		PriorSubtree:             []*pkgresource.State{old},
		ResultSubtree:            []*pkgresource.State{successor},
		SuccessorURNs:            map[resource.URN]resource.URN{old.URN: successor.URN},
		PreparedPriorResources:   []*pkgresource.State{successor, rewrittenRetained},
		RetainedResourceRewrites: map[*pkgresource.State]*pkgresource.State{retained: rewrittenRetained},
	}
	require.NoError(t, sm.StateMigration(transaction))
