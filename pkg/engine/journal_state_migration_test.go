...
func TestJournalStateMigrationVersionGate(t *testing.T) {
	...
	newState := func(name string) *resource.State {
	...
	base := &deploy.Snapshot{Resources: []*resource.State{a, b, c}}
	...
	t.Run("rejected on journal version 1", func(t *testing.T) {
		...
		err = sm.StateMigration([]*resource.State{a}, []*resource.State{newState("d")})
	...
	t.Run("journaled on journal version 2", func(t *testing.T) {
		...
		d := newState("d")
		require.NoError(t, sm.StateMigration([]*resource.State{a, c}, []*resource.State{d}))
