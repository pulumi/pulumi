...
type TestJournal struct {
	// SkipVerify makes Snap skip integrity verification of the reconstructed snapshot. The journals of
	// workflow node deployments describe open subgraphs of the stack's snapshot (their roots are
	// parented to the node resource, which lives outside them); they are verified as part of the
	// merged stack snapshot when it is written instead.
	SkipVerify bool
