...
func TestDescribe(t *testing.T) { //nolint:paralleltest,lll // non-thread-safe shared state
	...
	expected := map[esc.Pos]string{
		...
		{Line: 9, Column: 5}:   "Concatenates the elements of its second argument to create a single string. The first argument is placed between each element in the result.", //nolint:lll
		...
		{Line: 13, Column: 7}:  "Options for JWT login. JWT login uses an OIDC token issued by the Pulumi Cloud to generate an ephemeral token.", //nolint:lll
