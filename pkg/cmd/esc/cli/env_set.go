...
func newEnvSetCmd(env *envCommand) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if ref.version != "" {
				return errors.New("the set command does not accept versions")
			}
			...
			if looksLikeSecret(path, yamlValue) && !secret && !plaintext {
				return errors.New(
					"value looks like a secret; rerun with --secret to mark it as such, or --plaintext if you meant to leave it as plaintext", //nolint:lll
				)
			}
			if secret {
	...
	cmd.Flags().
		StringVarP(&file, "file", "f", "", "If set, the value is read from the specified file. Pass `-` to read from standard input.") //nolint:lll
	cmd.Flags().StringVar(
		&draft, "draft", "",
