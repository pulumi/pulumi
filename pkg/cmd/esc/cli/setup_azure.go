...
// createAzureEnvironments adds the azure-login provider into each environment
...
func newSetupAzureCmd(setup *setupCommand) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Derive the tenant from the selected subscriptions
			tenant := tenantID
			if tenant == "" {
	...
	cmd.Flags().StringVar(&appName, "app-name", "",
		"the app registration display name to create or reuse. defaults to 'pulumi-esc-oidc-app'")
