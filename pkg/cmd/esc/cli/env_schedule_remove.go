	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !yes {
				...
				if !ui.ConfirmPrompt(prompt, "remove", display.Options{
			...
			if err := env.esc.client.DeleteEnvironmentSchedule(ctx, ref.orgName, ref.projectName, ref.envName, scheduleID); err != nil { //nolint:lll
				return err
			}
