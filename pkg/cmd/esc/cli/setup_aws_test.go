...
func TestClassifySSOTokenError(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			retry, cause := classifySSOTokenError(tt.err)
			assert.Equal(t, tt.wantRetry, retry)
			if tt.wantRetry {
				// The original error is handed back so the poll loop can still classify it.
				require.ErrorIs(t, cause, tt.err)
				return
			}
			require.Error(t, cause)
			assert.Contains(t, cause.Error(), tt.wantMsg)
		})
	}
}
...
func TestClassifySSOTokenErrorUnwrapsOperationError(t *testing.T) {
	...
	retry, cause := classifySSOTokenError(wrapped)
	assert.False(t, retry)
	require.Error(t, cause)
	assert.Contains(t, cause.Error(), "denied")

	wrapped = fmt.Errorf("operation error SSO OIDC: CreateToken, %w", &ssooidctypes.AuthorizationPendingException{})
	retry, cause = classifySSOTokenError(wrapped)
	assert.True(t, retry)
	require.ErrorIs(t, cause, wrapped)
}
...
func TestOIDCIssuer(t *testing.T) {
	...
	tests := []struct {
		...
		{backendURL: "http://example.com:8080/myorg", want: "http://example.com:8080/oidc"},
		{backendURL: "https://api.example.com/acme", want: "https://api.example.com/oidc"},
