...
const gcpServiceAccountEnvHashLen = 8

func gcpOIDCServiceAccountName(orgID, escEnvironmentName string) string {
	return fmt.Sprintf("pulumi-oidc-%s-%s",
		orgIDPrefix(orgID), envHash(escEnvironmentName, gcpServiceAccountEnvHashLen))
}
...
// selectGCPProjects resolves which projects to configure
...
func createGCPEnvironment(
	ctx context.Context, setup *setupCommand, org string, r accountSetupResult,
) error {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, p := range selected {
				ref := setup.env.parseRef(org + "/" + setup.escEnvName(p))
			...
			for _, project := range selected {
				...
				escEnvName := setup.escEnvName(project)
			...
			for _, r := range results {
				...
				if err := createGCPEnvironment(ctx, setup, org, r); err != nil {
