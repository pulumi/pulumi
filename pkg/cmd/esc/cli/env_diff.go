...
func newEnvDiffCmd(env *envCommand) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "diff [<org-name>/][<project-name>/]<environment-name>[@<version>] [[[org-name/][<project-name>/]<environment-name>]@<version>]", //nolint:lll
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			compareRef := environmentRef{
				...
				baseRef.envName,
				"latest",
				baseRef.isUsingLegacyID,
				baseRef.hasAmbiguousPath,
			}
			if len(args) != 0 {
