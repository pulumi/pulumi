...
//
// NOTE: this is not a public API. It is internal to pulumi/pulumi, and we reserve the right to make breaking
// changes — adding or removing parameters, removing methods, changing types, etc. — at any time.
type Client interface {
	...
	CreateEnvironment(ctx context.Context, orgName, projectName, envName string) error
...
// CreateEnvironment creates an environment named envName in org orgName and project projectName.
func (pc *client) CreateEnvironment(ctx context.Context, orgName, projectName, envName string) error {
	req := struct {
...
func (pc *client) CheckYAMLEnvironment(
	...
	// NOTE: the trailing options are a variadic for backwards-compatible extension; this is an internal API and
	// may change freely.
...
func (pc *client) GetOpenEnvironment(
	ctx context.Context,
	orgName, projectName, envName, openSessionID string,
) (*esc.Environment, error) {
	var resp esc.Environment
...
func (pc *client) GetAnonymousOpenEnvironment(
	ctx context.Context,
	orgName, openSessionID string,
) (*esc.Environment, error) {
	var resp esc.Environment
...
func (pc *client) GetOpenProperty(
	ctx context.Context,
	orgName, projectName, envName, openSessionID, property string,
) (*esc.Value, error) {
	queryObj := struct {
...
func (pc *client) GetAnonymousOpenProperty(
	ctx context.Context,
	orgName, openSessionID, property string,
) (*esc.Value, error) {
	queryObj := struct {
		Property string `url:"property"`
	}{
...
func envHooksPath(orgName, projectName, envName string, parts ...string) string {
	...
	var baseSb1258 strings.Builder
	for _, p := range parts {
		baseSb1258.WriteString("/" + p)
	}
	base += baseSb1258.String()
	return base
}
...
func (pc *client) GetEnvironmentWebhook(
	...
	var resp EnvironmentWebhook
	if err := pc.restCall(ctx, http.MethodGet, envHooksPath(orgName, projectName, envName, webhookName), nil, nil, &resp); err != nil { //nolint:lll
		return nil, err
	}
	return &resp, nil
}
...
func (pc *client) CreateEnvironmentWebhook(
	...
	var resp EnvironmentWebhook
	if err := pc.restCall(ctx, http.MethodPost, envHooksPath(orgName, projectName, envName), nil, &req, &resp); err != nil { //nolint:lll
		return nil, err
	}
	return &resp, nil
}
...
func (pc *client) ListEnvironmentScheduleHistory(
	...
	path := fmt.Sprintf(
		"/api/esc/environments/%v/%v/%v/schedules/%v/history",
		orgName,
		projectName,
		envName,
		scheduleID,
...
func (pc *client) httpCall(
	...
	if pc.apiToken != "" {
		req.Header.Set("Authorization", "token "+pc.apiToken)
	}
