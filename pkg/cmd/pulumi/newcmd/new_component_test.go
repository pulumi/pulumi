...
func TestScaffoldPythonComponent(t *testing.T) {
	...
	// Verify component file contains correct class name
	...
	// Verify __main__.py contains provider host
	...
	// Verify PulumiPlugin.yaml has correct structure
...
func TestScaffoldTypeScriptComponent(t *testing.T) {
	...
	// Verify index.ts contains correct class name
...
func TestScaffoldGoComponent(t *testing.T) {
	...
	// Verify component.go contains correct struct name
	...
	// Verify main.go contains provider builder
...
func TestRunNewComponentWithoutLanguage(t *testing.T) {
	t.Parallel()
	...
	err := runNewComponent(args)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "unsupported language")
}

func TestRunNewComponentWithUnsupportedLanguage(t *testing.T) {
	t.Parallel()
...
func TestRunNewComponentWithShortLanguageName(t *testing.T) {
	t.Parallel()
...
func TestRunNewComponentUsesDirectoryNameWhenNoNameProvided(t *testing.T) {
	t.Parallel()
	...
	// Since temp dir names have random characters, just check that a .py file exists
	files, err := filepath.Glob(filepath.Join(tempDir, "*.py"))
	require.NoError(t, err)
	assert.NotEmpty(t, files, "Expected at least one .py file to be created")
}

func TestRunNewComponentCreatesDirectoryIfNotExists(t *testing.T) {
	t.Parallel()

	tempDir := t.TempDir()
	targetDir := filepath.Join(tempDir, "new-component-dir")
...
func TestRunNewComponentWithSingleLanguageType(t *testing.T) {
	t.Parallel()
	...
	// Verify single-language Python component was created (no __main__.py or PulumiPlugin.yaml)
	componentFile := filepath.Join(tempDir, "test_component.py")
	assert.FileExists(t, componentFile)

	// Single-language components should NOT have provider host files
	mainFile := filepath.Join(tempDir, "__main__.py")
	assert.NoFileExists(t, mainFile, "Single-language component should not have __main__.py")

	pluginFile := filepath.Join(tempDir, "PulumiPlugin.yaml")
	assert.NoFileExists(t, pluginFile, "Single-language component should not have PulumiPlugin.yaml")

	// Verify component file doesn't use TypedDict pattern
	content, err := os.ReadFile(componentFile)
	require.NoError(t, err)
	assert.NotContains(t, string(content), "TypedDict")
	assert.NotContains(t, string(content), "component_provider_host")
	assert.Contains(t, string(content), "class TestComponentArgs:")
}

func TestRunNewComponentWithMultiLanguageType(t *testing.T) {
	t.Parallel()
	...
	// Verify multi-language Python component was created with provider host
	componentFile := filepath.Join(tempDir, "test_component.py")
	assert.FileExists(t, componentFile)

	mainFile := filepath.Join(tempDir, "__main__.py")
	assert.FileExists(t, mainFile, "Multi-language component should have __main__.py")

	pluginFile := filepath.Join(tempDir, "PulumiPlugin.yaml")
	assert.FileExists(t, pluginFile, "Multi-language component should have PulumiPlugin.yaml")

	// Verify component file uses TypedDict pattern
	content, err := os.ReadFile(componentFile)
	require.NoError(t, err)
	assert.Contains(t, string(content), "TypedDict")
}

func TestRunNewComponentWithInvalidType(t *testing.T) {
	t.Parallel()
...
func TestScaffoldCSharpComponent(t *testing.T) {
	...
	// Verify .cs file contains correct class
...
func TestScaffoldJavaComponent(t *testing.T) {
	...
	// Verify Java file contains correct class
...
func TestScaffoldYamlComponent(t *testing.T) {
	...
	// Verify Pulumi.yaml contains correct structure
