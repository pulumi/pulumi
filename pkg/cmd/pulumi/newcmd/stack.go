...
func PromptAndCreateStack(ctx context.Context, ws pkgWorkspace.Context, b backend.Backend, prompt promptForValueFunc,
	...
	if stack != "" {
		...
		s, err := cmdStack.InitStack(ctx, ws, b, stackName, root, setCurrent, secretsProvider, useRemoteConfig)
		if err != nil {
			return nil, err
		}
		return s, nil
	...
	for {
		...
		s, err := cmdStack.InitStack(ctx, ws, b, formattedStackName, root, setCurrent, secretsProvider, useRemoteConfig)
		if err != nil {
			if !yes {
