...
func NewPulumiCmd() (*cobra.Command, func()) {
	...
	cmd := &cobra.Command{
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			slog.Info("CLI Metadata", "metadata", metadata)
			...
			if err != nil {
				slog.Info("error parsing current version", "err", err)
			} else {
				slog.Info("Pulumi " + ver.String())
			}

			if profiling != "" {
				if err := cmdutil.InitProfiling(profiling, memProfileRate); err != nil {
					slog.Warn("could not initialize profiling", "err", err)
				}
			}

			if env.SkipUpdateCheck.Value() {
				slog.Info("skipping update check")
			} else {
...
func checkForUpdate(ctx context.Context, cloudURL string, metadata map[string]string) *updateCheckResult {
	...
	if err != nil {
		slog.Info("error fetching latest version information",
			"skipEnvVar", env.SkipUpdateCheck.Var().Name(), "err", err)
	}
