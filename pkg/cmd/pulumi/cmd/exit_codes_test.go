...
func TestExitCodeFor_KnownErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			err:  &api.APIError{ExitCode: cmdutil.ExitCodeError},
			want: ExitCodeError,
		},
		{
			name: "api auth error preserves exit code",
			err:  &api.APIError{ExitCode: cmdutil.ExitAuthenticationError},
			want: ExitAuthenticationError,
		},
		{
			name: "api cancelled preserves exit code",
			err:  &api.APIError{ExitCode: cmdutil.ExitCancelled},
			want: ExitCancelled,
		},
	}
