...
func (w Workspace) RunPackage(
	...
	// An oci:// "path" names a provider IMAGE, not a binary on disk: run it as a one-shot
	// pod container and attach — the same boot `package add oci://` uses to read a schema
	// (see packages.SchemaFromSchemaSource). This is what lets `install` replay an oci
	// package pin recorded in Pulumi.yaml. Close reaps the container; the container is
	// also pod-labeled, so pod teardown is the backstop for callers that never close.
	if ref, ok := strings.CutPrefix(pluginPath, "oci://"); ok {
		p, stop, err := oci.ProviderFromImage(w.pctx, ref)
		if err != nil {
			return nil, err
		}
		var pluginResp *plugin.ParameterizeResponse
		if params != nil && !params.Empty() {
			resp, err := p.Parameterize(ctx, plugin.ParameterizeRequest{Parameters: params})
			if err != nil {
				return nil, errors.Join(err, stop())
			}
			pluginResp = &resp
		}
		return ociImageProvider{pluginProvider{p, originalSpec, pluginResp}, stop}, nil
	}
...
func (p pluginProvider) GetSchema(
	...
	// An oci:// package's ref is likewise not in the image's own schema: the pin is where
	// the package LIVES, and codegen bakes it into the generated SDK so consumption is
	// self-locating. The same injection `package add oci://` performs (schemaFromImage) —
	// without it, an SDK regenerated by `install` would resolve by convention instead of
	// by its pin, silently pointing at a different source than the one added.
	if strings.HasPrefix(p.originalSpec.Source, "oci://") {
		pkgSpec.PluginDownloadURL = p.originalSpec.Source
	}
...
// ociImageProvider ties the one-shot schema container's stop to Close, so closing the
// provider also reaps the container it runs in.
type ociImageProvider struct {
	pluginProvider
	stop func() error
}

func (p ociImageProvider) Close() error {
	return errors.Join(p.pluginProvider.Close(), p.stop())
}
