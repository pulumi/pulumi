...
// TestSkeleton_RealSpecOpsRoundTrip walks every op in the embedded spec
// that declares a request body and confirms the generator never panics
// and either returns nil (no required fields) or valid JSON.
func TestSkeleton_RealSpecOpsRoundTrip(t *testing.T) {
	t.Parallel()
	idx := loadTestIndex(t)
	for _, op := range idx.ByKey {
		if op.BodySchemaJSON == nil {
			continue
		}
		out := GenerateBodySkeleton(op.BodySchemaJSON)
		if out == nil {
			continue
		}
		var parsed any
		require.NoError(t, json.Unmarshal(out, &parsed),
			"skeleton for %s must round-trip through json.Unmarshal", op.OperationID)
	}
}
