...
//
// The parent `cloud` package exports data types, helpers, and the HTTP
...
// init registers this package's Run function with the parent cloud package
// so `runInteractive` can dispatch through it without creating an import
// cycle.
func init() {
	cloud.InteractiveRunner = Run
}
...
func Run(ctx context.Context, idx *cloud.Index, cfg cloud.TUIConfig) ([]byte, error) {
