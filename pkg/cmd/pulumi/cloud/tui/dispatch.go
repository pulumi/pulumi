...
// dispatchHeaders builds the http.Header for a dispatch request. The TUI
// doesn't support user-supplied -H overrides, so this is just Accept +
// Content-Type. Authorization is pinned by the Client itself.
func dispatchHeaders(contentType, accept string) http.Header {
	h := http.Header{}
	if accept != "" {
		h.Set("Accept", accept)
	}
...
func runDispatch(ctx context.Context, req dispatchRequest) tea.Cmd {
	return func() tea.Msg {
		...
		if !req.paginate {
			resp, err := req.client.RawCall(ctx, req.method, req.path, req.query,
				bodyReader(req.body), dispatchHeaders(req.contentType, req.accept), len(req.body) > 0)
			if err != nil {
		...
		for page < cloud.PaginationLimit {
			page++
			q := url.Values{}
			...
			resp, err := req.client.RawCall(ctx, req.method, req.path, q,
				bodyReader(req.body), dispatchHeaders(req.contentType, req.accept), len(req.body) > 0)
			if err != nil {
			...
			items, pageField, nextCursor, nextCursorParam, err := cloud.ExtractPage(body, req.op)
			if err != nil {
