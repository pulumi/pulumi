...
func newRootModel(ctx context.Context, idx *cloud.Index, cfg cloud.TUIConfig) rootModel {
	theme := newTheme()
	return rootModel{
		ctx:      ctx,
		idx:      idx,
...
func (m rootModel) buildDispatchRequest() dispatchRequest {
	op := m.request.op
	...
	concretePath := cloud.SubstituteInteractivePath(op, pathMap)
