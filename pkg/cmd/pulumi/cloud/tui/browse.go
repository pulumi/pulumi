...
type browseItem struct{ op *cloud.Operation }
...
func newBrowseModel(theme Theme, idx *cloud.Index) browseModel {
...
//     categories mode.
func buildBrowseGrouping(idx *cloud.Index) ([]list.Item, map[string][]list.Item, map[string]*categoryItem) {
	endpointsByTag := make(map[string][]list.Item)
	opsByTag := make(map[string][]*cloud.Operation)
...
func (m *browseModel) SelectedOp() *cloud.Operation {
...
func (m *browseModel) refreshDetails() {
	...
	case modeEndpoints:
		...
		m.details.SetText(op.OperationID, cloud.RenderDescribeMarkdown(op))
		m.details.GotoTop()
	case modeCategories:
