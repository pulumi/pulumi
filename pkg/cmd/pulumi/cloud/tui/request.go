...
func newRequestModel(theme Theme, cfg cloud.TUIConfig) requestModel {
...
// cloud.InteractiveDefault.
func (m *requestModel) SetOp(op *cloud.Operation) {
	m.op = op
	m.statusMsg = ""
	m.statusErr = false

	m.pathNames = cloud.PathParamsInOrder(op)
	m.pathInputs = make([]textinput.Model, len(m.pathNames))
	for i, n := range m.pathNames {
		ti := textinput.New()
		ti.Prompt = fmt.Sprintf("  {%s} ", n)
		ti.SetValue(cloud.InteractiveDefault(n, m.cfg))
		ti.CharLimit = 256
		m.pathInputs[i] = ti
	}
	...
	if m.hasBody {
		m.bodySkel = cloud.BodySkeletonSeed(op)
		ta := textarea.New()
...
func (m *requestModel) includeQueryParam(p cloud.ParamSpec) bool {
...
func (m *requestModel) editBodyCmd() tea.Cmd {
	...
	editor := cloud.PickEditor(os.Getenv)
