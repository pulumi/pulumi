...
func loadTestModel(t *testing.T) *schemaRenderer {
	t.Helper()
	doc, err := libopenapi.NewDocument(testSpecJSON)
	if err != nil {
...
// TestDiscriminatorRenderIsIdempotent pins that repeated renders of the same
// discriminator schema produce identical output, i.e. renderDiscriminator
// does not mutate the shared component schema cache. Today the discriminator
// path routes every variant through mergeAllOf's fresh allocation so the
// mutation is unreachable via the bundled spec — this test guards against a
// future spec regression that introduces a non-allOf variant.
func TestDiscriminatorRenderIsIdempotent(t *testing.T) {
	t.Parallel()
	renderer := loadTestModel(t)

	first := getComponentSchema(t, renderer, "AgentEntity")
	second := getComponentSchema(t, renderer, "AgentEntity")
	third := getComponentSchema(t, renderer, "AgentEntity")

	if first != second || second != third {
		t.Errorf("repeated discriminator renders drifted; shared schema state is being mutated.\n"+
			"first vs second equal: %v\nsecond vs third equal: %v", first == second, second == third)
	}
}
