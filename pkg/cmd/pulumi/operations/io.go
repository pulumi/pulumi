...
func configureNeoTaskOption(neoTaskEnabledFlag bool, cmd *cobra.Command, displayOpts *display.Options,
	isDIYBackend bool,
) {
	// If the flag is explicitly set, use that value. Otherwise fall back to environment variable.
	var startNeoTask bool
	if cmd.Flags().Changed("neo-task") {
		startNeoTask = neoTaskEnabledFlag
	} else {
		startNeoTask = env.NeoTaskEnabled.Value()
	}
	logging.V(7).Infof("neo-task flag=%v, PULUMI_NEO_TASK=%v, using value=%v",
		neoTaskEnabledFlag, env.NeoTaskEnabled.Value(), startNeoTask)
