	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			source := importStack("organization/test/sourceStack",
				[]*resource.State{provider, greeting("stayBehind", c.stayBehindRef), moveMe}, c.exts)
