...
func TestStackSecretsManagerLoaderDecrypterFallsBack(t *testing.T) {
	...
	// Act.
	_, needsSave, err := ssml.getDecrypter(context.Background(), s, ps)
	...
	assert.False(t, needsSave, "A mock decrypter should have no effect on the project stack")
...
func TestStackSecretsManagerLoaderDecrypterUpdatesConfig(t *testing.T) {
	...
	// Assert.
	assert.NoError(t, err)
	assert.True(t, needsSave, "A fallback passphrase decrypter should be written to the project stack")
...
func TestStackSecretsManagerLoaderDecrypterUsesDefaultSecretsManager(t *testing.T) {
	...
	// Assert.
	assert.NoError(t, err)
	assert.False(t, needsSave, "No fallback manager should mean no changes to the project stack")
...
func TestStackSecretsManagerLoaderEncrypterFallsBack(t *testing.T) {
	...
	// Act.
	_, needsSave, err := ssml.getEncrypter(context.Background(), s, ps)
	...
	// our fallback was called as a proxy.
	assert.NoError(t, err)
	assert.False(t, needsSave, "A mock encrypter should have no effect on the project stack")
...
func TestStackSecretsManagerLoaderEncrypterUpdatesConfig(t *testing.T) {
	...
	// Assert.
	assert.NoError(t, err)
	assert.True(t, needsSave, "A fallback passphrase encrypter should be written to the project stack")
	assert.Equal(t, "test-salt", ps.EncryptionSalt, "The encryption salt should be set on the project stack")
}
