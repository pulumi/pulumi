...
func (ws *replayWorkspace) HasPlugin(spec workspace.PluginSpec) bool {
...
type HasPlugin func(spec workspace.PluginSpec) bool

func (HasPlugin) isReplayStep() {}

func (ws *replayWorkspace) HasPluginGTE(spec workspace.PluginSpec) (bool, error) {
	next, idx := ws.pop("HasPluginGTE", []any{spec})
	step, ok := next.(HasPluginGTE)
	require.True(ws.t, ok, "%d: Expected step %T but found %T", idx, step, next)
	result1, result2 := step(spec)
	ws.record(idx, []any{result1, result2})
	return result1, result2
}

type HasPluginGTE func(spec workspace.PluginSpec) (bool, error)

func (HasPluginGTE) isReplayStep() {}

func (ws *replayWorkspace) GetPluginPath(ctx context.Context, spec workspace.PluginSpec) (string, error) {
...
type GetPluginPath func(ctx context.Context, spec workspace.PluginSpec) (string, error)
...
func (ws *replayWorkspace) LinkPackage(
	...
	next, idx := ws.pop("LinkPackage", []any{project, projectDir, packageName, pluginDir, params})
	step, ok := next.(LinkPackage)
	require.True(ws.t, ok, "%d: Expected step %T but found %T", idx, step, next)
	result := step(ctx, project, projectDir, packageName, pluginDir, params)
	ws.record(idx, []any{result})
	return result
}
