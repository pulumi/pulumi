...
// --- Conflict detection tests (T017) ---

// newRemoteStack returns a MockStack configured as a service-backed stack.
...
func TestLoadProjectStack_ConflictDetection_HardErrorWithConfigValues(t *testing.T) {
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "both service-backed and local configuration exist")
}
