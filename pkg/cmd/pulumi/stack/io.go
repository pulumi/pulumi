...
func LoadProjectStack(
	...
	if stack.ConfigLocation().IsRemote {
		...
		// remote config stack is ambiguous, so we fail hard to prevent silent data loss.
		...
		if statErr == nil {
			...
			if loadErr == nil && hasMeaningfulConfig(localPS) {
				return nil, fmt.Errorf(
					"both remote and local configuration exist for stack %s\n"+
						"  local file:       %s\n"+
						"  remote config:    ESC environment %s\n\n"+
						"Delete the local config file to use remote config, or run\n"+
...
func ChooseStack(ctx context.Context, sink diag.Sink, ws pkgWorkspace.Context,
	...
	if lopt.OfferNew() {
		...
		return nil, backenderr.NoStacksError{}
