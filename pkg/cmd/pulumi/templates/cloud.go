...
func (s *Source) getOrgTemplates(
	...
	if err != nil {
		...
		slog.Info("could not get a backend for org templates")
		return
	} else if b == nil {
		slog.Info("no current logged in user")
		return
	}
	...
	if _, _, _, err := b.CurrentUser(); err != nil {
		if errors.Is(err, backenderr.ErrLoginRequired) {
			slog.Info("user is not logged in")
	...
	if !b.SupportsTemplates() {
		slog.Info("backend does not support Org Templates", "backend", b.Name())
		return
	}

	slog.Info("Listing Org Templates from the cloud")
	...
	handleOrg := func(org string) {
		defer wg.Done()
		slog.Info("checking for templates", "org", org)
		...
		if apiError := new(apitype.ErrorResponse); errors.As(err, &apiError) {
			...
			if apiError.Code == 402 {
				slog.Info("org does not have access to org templates", "org", org, "code", apiError.Code)
				return
			}
		} else if err != nil {
			s.addError(fmt.Errorf("list templates: %w", err))
			slog.Warn("failed to get templates", "org", org, "err", err)
			return
		} else if orgTemplates.HasAccessError {
			slog.Warn("failed to get templates: access denied, please check that backend can access all template sources",
			...
			slog.Warn("failed to get templates: backend could not download the template", "org", org, "backend", b.Name())
			return
		}

		for source, sourceTemplates := range orgTemplates.Templates {
			slog.Info("sourcing templates", "source", source)
			for _, template := range sourceTemplates {
				...
				if templateName != "" && templateName != template.Name {
					slog.Debug("skipping template", "name", template.Name)
					continue
				}

				slog.Debug("adding template", "name", template.Name)
...
func (t orgTemplate) Download(ctx context.Context) (workspace.Template, error) {
	...
	slog.Info("downloaded template", "name", t.t.Name, "dir", templateDir)
...
func writeTar(ctx context.Context, reader *tar.Reader, dst string) error {
	for {
		...
		slog.Info("decompressing", "name", header.Name)
