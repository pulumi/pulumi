...
func (r registryTemplate) Download(ctx context.Context) (workspace.Template, error) {
	...
	if err != nil {
		return workspace.Template{}, errors.Join(
			fmt.Errorf("failed to make temporary directory: %w", err),
			templateBytes.Close())
	}
	...
	if err := writeTar(ctx, tar.NewReader(templateBytes), templateDir); err != nil {
		return workspace.Template{}, errors.Join(err, templateBytes.Close())
	}

	template, err := workspace.LoadTemplate(templateDir)
	return template, errors.Join(err, templateBytes.Close())
}
