...
func TestWhoAmICmd_verbose_orgToken(t *testing.T) {
	t.Parallel()

	ws := &pkgWorkspace.MockContext{}
	be := &backend.MockBackend{
		CurrentUserF: func() (string, []string, *workspace.TokenInformation, error) {
			return "user4", []string{"org1", "org2"}, &workspace.TokenInformation{
				Name:         "org-token",
				Organization: "org1",
			}, nil
		},
	...
	stdout := buff.String()
	assert.Contains(t, stdout, "User: user4")
	assert.Contains(t, stdout, "Organizations: org1, org2")
	assert.Contains(t, stdout, "Backend URL: https://pulumi.example.com")
	assert.Contains(t, stdout, "Token type: organization: org1")
	assert.Contains(t, stdout, "Token name: org-token")
}

func TestWhoAmICmd_default_orgToken(t *testing.T) {
	t.Parallel()

	ws := &pkgWorkspace.MockContext{}
	be := &backend.MockBackend{
		CurrentUserF: func() (string, []string, *workspace.TokenInformation, error) {
			return "user5", []string{"org1", "org2"}, &workspace.TokenInformation{
				Name:         "org-token",
				Organization: "org1",
			}, nil
		},
	...
	assert.Equal(t, "user5\n", buff.String())
}

func TestWhoAmICmd_default_teamToken(t *testing.T) {
	t.Parallel()

	ws := &pkgWorkspace.MockContext{}
	be := &backend.MockBackend{
		CurrentUserF: func() (string, []string, *workspace.TokenInformation, error) {
			return "user6", []string{"org1", "org2"}, &workspace.TokenInformation{
				Name: "team-token",
				Team: "myTeam",
			}, nil
		},
...
func TestWhoAmICmd_helpText(t *testing.T) {
	...
	expectedText := "When the current token is a Pulumi Cloud team token or an organization token, " +
		"the command will return the name of the organization with which the token is associated."

	assert.Contains(t, cmd.Long, expectedText)
}
