...
// splitSegments splits p on "/". We don't use path.Clean because it would
// collapse doubled slashes and strip trailing ones, both of which matter
// for matching the OpenAPI templates verbatim.
...
func MatchByOperationID(idx *Index, id string) (*MatchResult, error) {
	...
	for _, op := range idx.Operations {
		if strings.ToLower(op.OperationID) == lower {
