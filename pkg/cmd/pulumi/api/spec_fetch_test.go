...
func newSpecServer(t *testing.T, serveBody []byte) (*httptest.Server, *atomic.Int64) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		hits.Add(1)
		require.Equal(t, specEndpoint, r.URL.Path)
...
func TestEnsureSpec_CacheHitUnderTTL(t *testing.T) {
	...
	data, err := ensureSpec(t.Context(), false)
	require.NoError(t, err)
...
func TestEnsureSpec_RefreshForcesFetch(t *testing.T) {
	...
	data, err := ensureSpec(t.Context(), true)
	require.NoError(t, err)
	assert.JSONEq(t, `{"fresh":true}`, string(data))
	assert.Equal(t, int64(1), hits.Load())
}

// TestEnsureSpec_StaleFallbackOnFetchFailure pins that a 5xx during refresh
// returns the stale cached bytes with a stderr warning.
//
//nolint:paralleltest // mutates PULUMI_HOME / PULUMI_API / os.Stderr
func TestEnsureSpec_StaleFallbackOnFetchFailure(t *testing.T) {
	...
	assert.Contains(t, stderr, "using cached spec")
}
...
func TestFetchSpec_Unauthorized(t *testing.T) {
	...
	_, err := fetchSpec(t.Context(), srv.URL)
	require.Error(t, err)
	var apiErr *APIError
