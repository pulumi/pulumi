...
// written. Not parallel-safe (the Stdout swap is process-global).
...
func TestMarshalAccumulated_PreservesEnvelope(t *testing.T) {
	t.Parallel()
	// When the server returned an object envelope, we rewrap so --jq
	// filters like '.accounts[]' keep working across --paginate.
...
func TestFlushAccumulated_SilentSuppressesStdout(t *testing.T) {
	...
	got := captureStdout(t, func() {
		flushAccumulated(items, "accounts", &apiFlags{silent: true})
	})
	assert.Empty(t, got, "--silent must suppress the partial-failure flush")
}

// TestFlushAccumulated_JQFilters pins that --jq applies on the failure-path
// flush, matching the success path in writeAccumulated.
//
//nolint:paralleltest // mutates os.Stdout
func TestFlushAccumulated_JQFilters(t *testing.T) {
	items := []json.RawMessage{json.RawMessage(`{"id":"a"}`), json.RawMessage(`{"id":"b"}`)}
	got := captureStdout(t, func() {
		flushAccumulated(items, "accounts", &apiFlags{jq: ".accounts | length"})
	})
...
func TestFlushAccumulated_DefaultWritesEnvelope(t *testing.T) {
	items := []json.RawMessage{json.RawMessage(`{"id":"a"}`)}
	got := captureStdout(t, func() {
		flushAccumulated(items, "accounts", &apiFlags{})
	})
	assert.JSONEq(t, `{"accounts":[{"id":"a"}]}`, got)
}
...
func TestRunPaginate_TruncationEmitsPartialPaginationError(t *testing.T) {
	...
	_ = captureStdout(t, func() {
		err = runPaginate(t.Context(), apiClient, req, &apiFlags{})
	})
	require.Error(t, err)
	var apiErr *APIError
...
func TestRunPaginate_TruncationRespectsSilent(t *testing.T) {
	...
	out := captureStdout(t, func() {
		_ = runPaginate(t.Context(), apiClient, req, &apiFlags{silent: true})
	})
	assert.Empty(t, strings.TrimSpace(out), "--silent must suppress truncation flush")
}
