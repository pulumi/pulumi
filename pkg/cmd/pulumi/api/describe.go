...
// newDescribeCmd builds `pulumi cloud api describe <path>`.
func newDescribeCmd() *cobra.Command {
	var method string
	var output, jq string

	cmd := &cobra.Command{
		Use:   "describe <path-or-operation-id>",
		...
		Long: "Show the parameters, request body, and response schema for a Pulumi Cloud\n" +
			"API operation.\n\n" +
			"The argument may be either a path (with optional template variables,\n" +
			"e.g. `/api/stacks/{orgName}`) or an operation ID as shown in `ls`\n" +
			"(e.g. `ListAccounts`). Operation IDs are matched case-insensitively.\n\n" +
			"Default output is a human-readable schema render. Pass --output=json for\n" +
			"the stable agent envelope, including the inlined JSON schema. Passing\n" +
			"--jq implies --output=json.",
		Example: "  # Describe an operation by its ID.\n" +
			...
			"  # Paste-friendly: copy a METHOD + path row from `ls` verbatim.\n" +
			"  pulumi cloud api describe 'GET /api/user'\n\n" +
			"  # Extract just the request body schema.\n" +
			"  pulumi cloud api describe CreateStackTag --jq '.operation.requestBody.jsonSchema'\n\n" +
			"  # Pull the parameter list for scripting.\n" +
			"  pulumi cloud api describe GetStack --jq '.operation.parameters[] | {name, in, required}'",
	...
	cmd.Flags().StringVar(&output, "output", "",
		"Output format: default is a human-readable schema render; "+
			...
			"`json` emits the stable agent envelope (required when using --jq)")
	...
	cmd.RunE = runWithEnvelope(func(cmd *cobra.Command, args []string) error {
		return runDescribe(cmd.Context(), args, method,
			cmd.Flags().Changed("method"), output, jq, cmd.Flags().Changed("output"),
			refreshSpecFlag(cmd))
	})
	return cmd
}
...
func runDescribe(
	ctx context.Context,
	args []string, method string, methodExplicit bool, output, jq string, outputSet, refresh bool,
) error {
	...
	if mode == OutputJSON {
		return emitDescribeJSON(mr.Op, jq)
	}
	if mode == OutputMarkdown {
		return emitDescribeMarkdown(mr.Op)
	}
	return emitDescribeText(mr.Op)
}

// emitDescribeMarkdown writes a markdown description of the operation to stdout.
func emitDescribeMarkdown(op *Operation) error {
...
func emitDescribeJSON(op *Operation, jq string) error {
	payload := describedOp{
	...
	return writeJSONEnvelope(describeEnvelope{
		SchemaVersion: SchemaVersion,
		Operation:     payload,
	}, jq, "describe")
}

// emitDescribeText writes a human-readable view to stdout.
func emitDescribeText(op *Operation) error {
