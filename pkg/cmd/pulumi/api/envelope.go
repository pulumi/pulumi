...
// WriteErrorEnvelope writes err as a JSON envelope to w. When pretty is false
// it emits one line so log-line parsers can split on "\n" safely; when true
// it indents for human terminals. Callers should pass !stderrIsTTY() (i.e.
// compact when piped, indented at a TTY) to match the stdout convention.
func WriteErrorEnvelope(w io.Writer, err *APIError, pretty bool) error {
	return WriteJSON(w, err.Envelope, pretty)
}

// now returns the current time in RFC3339Nano. Indirected so tests can stub it.
...
func NewEvent(name string) *Event {
	return &Event{SchemaVersion: SchemaVersion, Event: name, Timestamp: now()}
}
...
//
// Commands that use this wrapper should set SilenceErrors=true and
// SilenceUsage=true so cobra does not also print its default formatting.
//
...
func runWithEnvelope(fn func(*cobra.Command, []string) error) func(*cobra.Command, []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		if !apiErr.Silent {
			_ = WriteErrorEnvelope(os.Stderr, apiErr, stderrIsTTY())
			apiErr.Silent = true
		}
		return apiErr
	}
