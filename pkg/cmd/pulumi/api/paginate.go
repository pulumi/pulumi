...
// streaming per-page progress events to stderr, and concatenating items into
// a single JSON array on stdout.
//
// Returns a Silent APIError on partial failure so the caller's wrapper
// doesn't double-emit — the partial_failure event on stderr is the ground
// truth for agents.
func runPaginate(
	ctx context.Context,
	apiClient *APIClient,
	...
	// produce — preserving --jq compatibility. Empty means the server returned a bare array.
	...
	for page < PaginationLimit {
		...
		if errors.Is(ctx.Err(), context.Canceled) {
			return emitCancelled(flags, page, accumulated, itemsField)
		}
		...
		resp, err := apiClient.Do(ctx, req.Method, req.Path, q, bodyReader, req.ContentType, req.Accept, req.Headers)
		if err != nil {
			if errors.Is(ctx.Err(), context.Canceled) {
				return emitCancelled(flags, page, accumulated, itemsField)
			}
			return emitPartialFailure(flags, page, accumulated, itemsField,
		...
		if resp.StatusCode >= 400 {
			return emitPartialFailure(flags, page, accumulated, itemsField, httpErrorEnvelopeBytes(resp, body))
		}
		...
		pageEvent := NewEvent("page")
		pageEvent.Page = page
		pageEvent.Count = pageCount
		if nextCursor != "" {
			pageEvent.Cursor = nextCursor
		}
		maybeWriteStderrEvent(flags.emitEvents, pageEvent)
	...
	// the dataset was truncated, not exhausted. Emit a distinct event and a
	// partial-pagination error so agents can detect it via exit code + code
	// field, and resume from the returned cursor.
	if truncated {
		flushAccumulated(accumulated, itemsField, flags)
		ev := NewEvent("truncated")
		ev.PagesFetched = page
		ev.Cursor = cursor
	...
	done := NewEvent("complete")
	done.TotalPages = page
	done.TotalItems = len(accumulated)
	maybeWriteStderrEvent(flags.emitEvents, done)

	return writeAccumulated(accumulated, itemsField, flags)
}
...
// writeAccumulated writes the combined result to stdout, optionally post-
// processed with --jq. When itemsField is non-empty (the server returned an
// object envelope with a named array), the output is wrapped back into that
// same shape ({<itemsField>: [...]}) so --jq filters work identically
// whether or not --paginate is set. When itemsField is empty (server
// returned a bare array), the output stays a bare array.
func writeAccumulated(items []json.RawMessage, itemsField string, flags *apiFlags) error {
	...
	if flags.jq != "" {
		return ApplyJQ(os.Stdout, combined, flags.jq)
	}
	pretty := stdoutIsTTY()
	if pretty {
...
// see a different type on failure. Honors --silent (no stdout) and --jq (apply
// filter), matching the behaviour of the success-path writeAccumulated.
func flushAccumulated(accumulated []json.RawMessage, itemsField string, flags *apiFlags) {
	...
	if flags != nil && flags.jq != "" {
		_ = ApplyJQ(os.Stdout, combined, flags.jq)
		return
	}
...
func emitPartialFailure(
	flags *apiFlags, failedAt int, accumulated []json.RawMessage, itemsField string, apiErr *APIError,
) error {
	flushAccumulated(accumulated, itemsField, flags)
	ev := NewEvent("partial_failure")
	ev.PagesFetched = failedAt - 1
	ev.FailedAt = failedAt
	ev.ErrorDetail = &apiErr.Envelope.Error
	maybeWriteStderrEvent(flags.emitEvents, ev)
	apiErr.Silent = true
	return apiErr
}

// emitCancelled writes accumulated pages to stdout, emits a cancelled event
// to stderr (when enabled), and returns a Silent APIError with cmdutil.ExitCancelled.
func emitCancelled(
	flags *apiFlags, inFlight int, accumulated []json.RawMessage, itemsField string,
) error {
	flushAccumulated(accumulated, itemsField, flags)
	ev := NewEvent("cancelled")
	ev.Phase = "paginate"
	if inFlight > 1 {
		completed := make([]int, 0, inFlight-1)
	...
	if inFlight >= 1 {
		ev.InProgress = []int{inFlight}
	}
	maybeWriteStderrEvent(flags.emitEvents, ev)
	return &APIError{ExitCode: cmdutil.ExitCancelled, Silent: true}
}

// maybeWriteStderrEvent writes a JSONL event to stderr only when the caller
// passed --emit-events. Keeping events opt-in means `--paginate 2>&1 | jq`
// doesn't confuse progress markers for response data.
func maybeWriteStderrEvent(emit bool, ev *Event) {
	if !emit {
		return
	}
	_ = WriteEvent(os.Stderr, ev)
}

// httpErrorEnvelopeBytes builds an APIError from an error response body.
// 4xx and 5xx both map to cmdutil.ExitCodeError (1); only 401/403 map to
// cmdutil.ExitAuthenticationError (3). The `code` field on the envelope (ErrHTTP4xx /
// ErrHTTP5xx) still distinguishes them for JSON consumers, but the exit code
// is reserved for auth so agent wrappers keying on exit 3 don't misfire on
// transient 5xx from Pulumi Cloud. Paginate's mid-loop path embeds the
// returned APIError in the partial_failure event.
