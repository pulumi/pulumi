...
// to force a re-fetch even when the cache has a copy.
func LoadIndex(ctx context.Context, refresh bool) (*Index, error) {
	data, err := ensureSpec(ctx, refresh)
	if err != nil {
		return nil, err
	}
...
func parseOperation(path, method string, op *v3high.Operation, renderer *schemaRenderer) *Operation {
	// Prefer the second tag when the spec provides one: Pulumi's OpenAPI uses
	// tags[0] as a broad bucket (e.g. "Stacks") and tags[1] as the fine-grained
	// sub-grouping users actually want to navigate by.
	tag := "Miscellaneous"
	switch {
	case len(op.Tags) >= 2:
		tag = op.Tags[1]
