...
func fetchSpec(ctx context.Context, cloudURL string) ([]byte, error) {
	...
	c := client.NewClient(cloudURL, token, false, cmdutil.Diag())
	resp, err := c.RawCall(ctx, http.MethodGet, specEndpoint, nil, nil,
		buildAPIHeaders("", "application/json", nil), false)
	if err != nil {
