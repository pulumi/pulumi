...
// User-supplied headers win over the encoder defaults (Content-Type, Accept) so a
// caller's -H 'Content-Type: application/x-yaml' overrides a JSON default, with the
// sole exception of Authorization — we never let a typo leak a token.
func (c *APIClient) Do(ctx context.Context, method, path string, query url.Values, body io.Reader,
	contentType, accept string, headers []ParsedHeader,
) (*http.Response, error) {
	u := c.BaseURL + path
	if len(query) > 0 {
		u += "?" + query.Encode()
	...
	// Apply user-supplied headers last so they override encoder defaults
	// (Accept, Content-Type). Authorization stays pinned to the resolved token.
	...
	if ce := resp.Header.Get("Content-Encoding"); ce == "gzip" || ce == "x-gzip" {
		...
		// Content-Length applied to the compressed bytes, not the decompressed
		// body now exposed by resp.Body.
