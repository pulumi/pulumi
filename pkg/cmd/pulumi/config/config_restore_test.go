...
func TestConfigRestore_CreatesNewRevision(t *testing.T) {
	...
	cmd := &configRestoreCmd{
		stdout: &stdout,
		getEnvironment: func(_ context.Context, _ backend.EnvironmentsBackend, _, _, _, version string, _ bool) ([]byte, string, int, error) {
			if version == "3" {
				return oldContent, "etag-old", 3, nil
			}
			return []byte("values:\n  pulumiConfig:\n    key: currentvalue\n"), "etag-current", 5, nil
		},
		updateEnvironmentWithProject: func(_ context.Context, _ backend.EnvironmentsBackend, _, _, _ string, yaml []byte, etag string) error {
			writtenYAML = yaml
			writtenEtag = etag
			return nil
		},
...
func TestConfigRestore_EtagConflict(t *testing.T) {
	...
	cmd := &configRestoreCmd{
		...
		updateEnvironmentWithProject: func(_ context.Context, _ backend.EnvironmentsBackend, _, _, _ string, _ []byte, _ string) error {
