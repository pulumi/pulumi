...
func newConfigPinCmd(ws pkgWorkspace.Context, stackRef *string) *cobra.Command {
	impl := &configPinCmd{
		ws:       ws,
		stackRef: stackRef,
		stdout:   os.Stdout,
...
This command is only meaningful for service-backed stacks. For local stacks
...
type configPinCmd struct {
	...
	loadRemoteConfig func(ctx context.Context, stack backend.Stack, project *workspace.Project) (*workspace.ProjectStack, error)
	// saveRemoteConfig is overridable for testing.
	saveRemoteConfig func(ctx context.Context, stack backend.Stack, ps *workspace.ProjectStack) error
	// getEnvironment is overridable for testing.
	getEnvironment func(ctx context.Context, envBackend backend.EnvironmentsBackend, org, project, name, version string) error
}
...
func (cmd *configPinCmd) run(ctx context.Context, versionArg string) error {
	...
	if !loc.IsRemote || loc.EscEnv == nil {
		fmt.Fprintln(cmd.stdout, "Config pinning is only applicable to service-backed stacks; this stack uses local config.")
		return nil
	}
...
func rejectIfPinned(stack backend.Stack) error {
	...
	return fmt.Errorf(
		"config is pinned to version %s; run `pulumi config pin latest` to unpin before making changes",
		version)
}
