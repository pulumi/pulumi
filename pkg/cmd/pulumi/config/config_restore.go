...
type configRestoreCmd struct {
	...
	getEnvironment              func(ctx context.Context, eb backend.EnvironmentsBackend, org, project, name, version string, decrypt bool) ([]byte, string, int, error)
	updateEnvironmentWithProject func(ctx context.Context, eb backend.EnvironmentsBackend, org, project, name string, yaml []byte, etag string) error
}
...
func (cmd *configRestoreCmd) run(ctx context.Context, revisionArg string) error {
	...
	if !loc.IsRemote || loc.EscEnv == nil {
		return fmt.Errorf("config restore is only supported for service-backed stacks; this stack uses local config")
	}
...
func (cmd *configRestoreCmd) restoreRevision(
	...
	if getFn == nil {
		getFn = func(ctx context.Context, eb backend.EnvironmentsBackend, org, proj, name, version string, decrypt bool) ([]byte, string, int, error) {
	...
	if updateFn == nil {
		updateFn = func(ctx context.Context, eb backend.EnvironmentsBackend, org, proj, name string, yaml []byte, etag string) error {
	...
	fmt.Fprintf(cmd.stdout, "Restored config to revision %s (created as new revision).\n", revisionArg)
	return nil
}
