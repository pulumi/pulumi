...
// consoleURLProvider is the subset of the Pulumi Cloud backend interface needed
...
func newConfigEnvConsoleCmd(ws pkgWorkspace.Context, stackRef *string) *cobra.Command {
	impl := &configWebCmd{
		ws:       ws,
		stackRef: stackRef,
		stdout:   os.Stdout,
	...
	cmd := &cobra.Command{
		Use:   "console",
		Short: "Open the ESC environment for a remote config stack in the Pulumi Cloud console",
		Long: `Opens the ESC environment for a remote config stack in the Pulumi Cloud console.

This command is only supported for remote config stacks.`,
...
func (cmd *configWebCmd) run(ctx context.Context) error {
	...
	cloudBe, ok := stack.Backend().(consoleURLProvider)
	if !ok {
	...
	consoleURL := cloudBe.CloudConsoleURL(orgName, "esc", envProject, envName)
	if consoleURL == "" {
