...
For remote config stacks, the ESC environment definition YAML is downloaded,
...
For local stacks, the Pulumi.<stack>.yaml file is opened directly.`,
	...
	cmd.Flags().BoolVar(&impl.showSecrets, "show-secrets", false,
		"Decrypt secrets in plaintext when downloading the ESC environment (remote config stacks only)")
