...
func NewConfigEditor(
	...
	return &escConfigEditor{
		envBackend: state.envBackend,
		orgName:    state.orgName,
		envProject: state.envProject,
...
// is an integer (array index), which is not supported for remote config stacks.
func setNestedYAMLValue(parent map[string]any, rootKey string, remainingPath resource.PropertyPath, value any) error {
	...
	if _, isInt := remainingPath[0].(int); isInt {
		return errors.New(
			"array index paths are not supported for remote config; " +
...
// supported for remote config stacks.
...
// parseEscEnvRef splits an ESC environment reference in "<project>/<name>" or
// "<project>/<name>@<version>" format. Any @version suffix is stripped so the
// returned project and name can be used directly with ESC API calls.
func parseEscEnvRef(ref string) (project, name string, err error) {
	ref = stripEscEnvVersion(ref)
...
// escEnvState holds a loaded ESC environment definition with the metadata
// needed to read, modify, and save it back. Used by both the ConfigEditor
// and the env ls/add/rm commands.
type escEnvState struct {
	envBackend backend.EnvironmentsBackend
	orgName    string
	envProject string
	envName    string
...
// loadESCEnvState loads the ESC environment linked to a remote config stack.
func loadESCEnvState(ctx context.Context, stack backend.Stack) (*escEnvState, error) {
	loc := stack.ConfigLocation()
	if !loc.IsRemote || loc.EscEnv == nil {
		return nil, fmt.Errorf("stack %q does not use remote config", stack.Ref().Name())
	}

	envBackend, ok := stack.Backend().(backend.EnvironmentsBackend)
	if !ok {
		return nil, fmt.Errorf(
			"backend %q does not support ESC environments", stack.Backend().Name())
	}
	...
	yamlBytes, etag, _, err := envBackend.GetEnvironment(
		ctx, orgName, envProject, envName, "", false)
	if err != nil {
...
// imports returns the current imports list from the environment definition.
func (s *escEnvState) imports() []string {
	raw, _ := s.envDef["imports"].([]any)
...
// setImports replaces the imports list in the environment definition.
func (s *escEnvState) setImports(imports []string) {
	if len(imports) == 0 {
		delete(s.envDef, "imports")
		return
	}
	raw := make([]any, len(imports))
	for i, v := range imports {
		raw[i] = v
	}
	s.envDef["imports"] = raw
}

// save serializes the environment definition and persists it via the ESC API.
func (s *escEnvState) save(ctx context.Context) error {
	var buf bytes.Buffer
	enc := yaml.NewEncoder(&buf)
	enc.SetIndent(2)
	if err := enc.Encode(s.envDef); err != nil {
		return fmt.Errorf("serialising ESC environment definition: %w", err)
	}
	if err := enc.Close(); err != nil {
		return fmt.Errorf("serialising ESC environment definition: %w", err)
	...
	diags, err := s.envBackend.UpdateEnvironmentWithProject(
		ctx, s.orgName, s.envProject, s.envName, buf.Bytes(), s.etag)
	if err != nil {
		if isHTTPConflict(err) {
			return fmt.Errorf(
				"the ESC environment %s/%s was modified concurrently; please retry: %w",
				s.envProject, s.envName, err)
		}
		return fmt.Errorf("saving ESC environment %s/%s: %w", s.envProject, s.envName, err)
	}
	if len(diags) > 0 {
		return fmt.Errorf("ESC environment %s/%s validation failed:\n%s",
			s.envProject, s.envName, formatEnvDiags(diags))
	}
	return nil
}
