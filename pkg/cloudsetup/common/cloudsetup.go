...
// DefaultProject is the ESC project an environment reference names when it omits one.
const DefaultProject = "default"

// OIDCClaims returns the audience and the `currentEnvironment.name` value used in the subject.
// For environments in the legacy default project, use an audience without prefix and environment name without project.
func OIDCClaims(audiencePrefix, orgName, escEnvironmentName string) (audience, subjectEnvName string) {
	if project, name, ok := strings.Cut(escEnvironmentName, "/"); ok && project == DefaultProject {
		return orgName, name
	}
	return audiencePrefix + ":" + orgName, escEnvironmentName
}
