...
// LocalClient is a Client that can also resolve the tenant of a set of subscriptions.
type LocalClient interface {
	Client
	// Tenant returns the Azure tenant the given subscriptions belong to, erroring if they span more than one.
...
// the app registration to find-or-create; empty uses the default name.
...
func (c *client) Tenant(ctx context.Context, subscriptionIDs []string) (string, error) {
	...
	// Map every visible subscription to its tenant, then resolve the requested ones.
