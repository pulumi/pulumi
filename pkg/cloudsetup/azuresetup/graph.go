...
// graphScope is the OAuth scope requested for Microsoft Graph calls.
var graphScope = []string{"https://graph.microsoft.com/.default"}

// graphClient talks to Microsoft Graph directly over REST.
type graphClient struct {
	pipeline runtime.Pipeline
}

// NewGraphClient builds a GraphClient backed by direct Microsoft Graph REST calls.
...
// Request/response bodies for the Graph endpoints used below.
type (
	graphApp struct {
...
// do issues a Graph request and verifies its response status (200 OK unless okCodes is given).
