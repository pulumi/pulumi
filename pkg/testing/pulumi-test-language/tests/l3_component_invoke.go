...
func init() {
	// An invoke written inside a component must be generated as a child of that component, just like a
	// resource is. Provider resolution is how the program observes that: an invoke resolves its provider
	// through its parent, so an invoke in a component instantiated with `providers = [prov]` must be
	// served by `prov`, and `getConfig` echoes back the `name` its provider was configured with.
	LanguageTests["l3-component-invoke"] = LanguageTest{
