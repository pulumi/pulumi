...
func init() {
	LanguageTests["l3-range-bool-ref"] = LanguageTest{
		...
		Runs: []TestRun{
			{
				Config: config.Map{
					config.MustMakeKey("l3-range-bool-ref", "createBool"): config.NewValue("true"),
				},
				Assert: func(l *L, res AssertArgs) {
					RequireStackResource(l, res.Err, res.Changes)

					// stack + nestedobject provider + boolResource + boolTarget
