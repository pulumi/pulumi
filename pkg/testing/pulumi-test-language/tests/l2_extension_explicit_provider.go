...
func init() {
	...
	LanguageTests["l2-extension-explicit-provider"] = LanguageTest{
		...
		Runs: []TestRun{
			{
				Assert: func(l *L, res AssertArgs) {
					...
					// The explicit provider must serve both resources. A default provider
					// would also be an extbase provider, so assert on the declared name.
					explicit := RequireSingleResource(l, snap.Resources, "pulumi:providers:extbase")
					require.Equal(l, "prov", explicit.URN.Name(),
						"the program declares a single provider named prov")
					...
					require.Contains(l, greeting.Provider, "pulumi:providers:extbase::prov::",
						"the extension resource must use the explicit provider, not a default one")
					...
					require.Contains(l, base.Provider, "pulumi:providers:extbase::prov::",
						"the base resource must use the explicit provider, not a default one")
				},
			},
		},
	}
