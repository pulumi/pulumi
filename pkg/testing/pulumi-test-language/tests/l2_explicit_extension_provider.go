...
func init() {
	...
	LanguageTests["l2-explicit-extension-provider"] = LanguageTest{
		...
		Runs: []TestRun{
			{
				Assert: func(l *L, res AssertArgs) {
					...
					explicitRef, err := sdkproviders.NewReference(explicit.URN, explicit.ID)
					...
					require.Equal(l, explicitRef.String(), greeting.Provider,
						"the extension resource must use the explicit provider")
					...
					require.Equal(l, explicitRef.String(), base.Provider,
						"the base resource must use the explicit provider")
				},
			},
		},
	}
