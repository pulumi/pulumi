...
func init() {
	LanguageTests["l2-resource-option-depends-on"] = LanguageTest{
		...
		Runs: []TestRun{
			{
				Assert: func(l *L, res AssertArgs) {
					...
					withDeps := RequireSingleNamedResource(l, res.Snap.Resources, "withDependsOn")
					assert.Equal(l, []resource.URN{noDeps.URN}, withDeps.Dependencies)
				},
			},
		},
	}
