...
func init() {
	validate := func(l *L, events []engine.Event, value string) {
		...
		expectedViolations := []engine.PolicyViolationEventPayload{
			{
				ResourceURN:       "urn:pulumi:test::policy-stack-config::simple:index:Resource::res",
				Message:           "<{%reset%}>Verifies property is " + value + "\nProperty was " + opp + "<{%reset%}>\n",
				Color:             "raw",
				PolicyName:        "validate-" + value,
				PolicyPackName:    "stack-config",
	...
	LanguageTests["policy-stack-config"] = LanguageTest{
