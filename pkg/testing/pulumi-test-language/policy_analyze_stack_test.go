...
func (h *PolicyAnalyzeStackLanguageHost) GenerateProject(
	...
	if req.TargetDirectory != filepath.Join(h.tempDir, "projects", "policy-analyze-stack") {
...
func (h *PolicyAnalyzeStackLanguageHost) GetRequiredPlugins(
	...
	if req.Info.ProgramDirectory != filepath.Join(h.tempDir, "projects", "policy-analyze-stack") {
...
func (h *PolicyAnalyzeStackLanguageHost) InstallDependencies(
	...
	if req.IsPlugin {
		...
		if req.Info.RootDirectory != filepath.Join(h.tempDir, "projects", "policy-analyze-stack") {
			return fmt.Errorf("unexpected root directory to install dependencies %s", req.Info.RootDirectory)
		}
	}
...
func (h *PolicyAnalyzeStackLanguageHost) Run(
	...
	// Only register a second resource when the config toggle is set: this is what should trip
	// the stack-size policy on the second run.
	if req.Config["policy-analyze-stack:extraCount"] == "1" {
		_, err = monitor.RegisterResource(ctx, &pulumirpc.RegisterResourceRequest{
			Type:   "simple:index:Resource",
			Custom: true,
			Name:   "res2",
