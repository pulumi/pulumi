...
type PolicyAnalyzeStackLanguageHost struct {
	pulumirpc.UnimplementedLanguageRuntimeServer

	tempDir string
}

func (h *PolicyAnalyzeStackLanguageHost) Pack(
...
func (h *PolicyAnalyzeStackLanguageHost) GenerateProject(
	...
	if req.TargetDirectory != filepath.Join(h.tempDir, "projects", "policy-analyze-stack", "0") &&
		req.TargetDirectory != filepath.Join(h.tempDir, "projects", "policy-analyze-stack", "1") {
	...
	if project.Name != "policy-analyze-stack" {
...
func (h *PolicyAnalyzeStackLanguageHost) GeneratePackage(
...
func (h *PolicyAnalyzeStackLanguageHost) GetRequiredPlugins(
	ctx context.Context, req *pulumirpc.GetRequiredPluginsRequest,
) (*pulumirpc.GetRequiredPluginsResponse, error) {
	if req.Info.ProgramDirectory != filepath.Join(h.tempDir, "projects", "policy-analyze-stack", "0") &&
		req.Info.ProgramDirectory != filepath.Join(h.tempDir, "projects", "policy-analyze-stack", "1") {
...
func (h *PolicyAnalyzeStackLanguageHost) GetProgramDependencies(
...
func (h *PolicyAnalyzeStackLanguageHost) InstallDependencies(
	req *pulumirpc.InstallDependenciesRequest, server pulumirpc.LanguageRuntime_InstallDependenciesServer,
) error {
	if req.IsPlugin {
		if !strings.HasSuffix(req.Info.RootDirectory, filepath.Join("policy_packs", "analyze-stack")) {
			return fmt.Errorf("unexpected root directory to install dependencies %s", req.Info.RootDirectory)
		}
	} else {
		if req.Info.RootDirectory != filepath.Join(h.tempDir, "projects", "policy-analyze-stack", "0") &&
			req.Info.RootDirectory != filepath.Join(h.tempDir, "projects", "policy-analyze-stack", "1") {
			return fmt.Errorf("unexpected root directory to install dependencies %s", req.Info.RootDirectory)
		}
	}
...
func (h *PolicyAnalyzeStackLanguageHost) Run(
	ctx context.Context, req *pulumirpc.RunRequest,
) (*pulumirpc.RunResponse, error) {
	run0 := filepath.Join(h.tempDir, "projects", "policy-analyze-stack", "0")
	run1 := filepath.Join(h.tempDir, "projects", "policy-analyze-stack", "1")
	if req.Info.RootDirectory != run0 && req.Info.RootDirectory != run1 {
	...
	// Only run 1 registers a second resource, which is what should trip the stack-size policy.
	if req.Info.RootDirectory == run1 {
		_, err = monitor.RegisterResource(ctx, &pulumirpc.RegisterResourceRequest{
			Type:   "simple:index:Resource",
			Custom: true,
			Name:   "res2",
...
func (h *PolicyAnalyzeStackLanguageHost) Link(
...
type stackAnalyzerPlugin struct {
	pulumirpc.UnimplementedAnalyzerServer
}

func (a *stackAnalyzerPlugin) GetAnalyzerInfo(context.Context, *emptypb.Empty) (*pulumirpc.AnalyzerInfo, error) {
...
// Analyze does nothing: this pack only has a stack-level policy.
func (a *stackAnalyzerPlugin) Analyze(
	ctx context.Context, req *pulumirpc.AnalyzeRequest,
) (*pulumirpc.AnalyzeResponse, error) {
	return &pulumirpc.AnalyzeResponse{}, nil
}

func (a *stackAnalyzerPlugin) AnalyzeStack(
	ctx context.Context, req *pulumirpc.AnalyzeStackRequest,
) (*pulumirpc.AnalyzeResponse, error) {
	var diags []*pulumirpc.AnalyzeDiagnostic
	count := 0
	for _, r := range req.Resources {
		...
		count++
		if count > 1 {
			diags = append(diags, &pulumirpc.AnalyzeDiagnostic{
				PolicyName:        "stack-size",
				PolicyPackName:    "analyze-stack",
				PolicyPackVersion: "1.0.0",
				EnforcementLevel:  pulumirpc.EnforcementLevel_MANDATORY,
				Message:           "Stack must contain at most one simple:index:Resource\nFound an extra simple:index:Resource",
			})
			// Only report one stack-level violation.
			break
		}
	}
	return &pulumirpc.AnalyzeResponse{Diagnostics: diags}, nil
}

func (a *stackAnalyzerPlugin) Cancel(ctx context.Context, req *emptypb.Empty) (*emptypb.Empty, error) {
	return &emptypb.Empty{}, nil
}

func (h *PolicyAnalyzeStackLanguageHost) RunPlugin(
...
// Runs a stack-level policy conformance test with a mocked runtime.
func TestPolicyAnalyzeStack(t *testing.T) {
	t.Parallel()

	ctx := t.Context()
	tempDir := t.TempDir()
	engine := runner.NewLanguageTestServer(tests.LanguageTestdata, tests.LanguageTests)
	runtime := &PolicyAnalyzeStackLanguageHost{tempDir: tempDir}
	handle, err := rpcutil.ServeWithOptions(rpcutil.ServeOptions{
		Init: func(srv *grpc.Server) error {
