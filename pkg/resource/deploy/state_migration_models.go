		...
		case value.IsResourceReference():
			...
			if fixed != ref.URN {
				if identity, ok := successorIdentities[fixed]; ok {
					if identity.custom {
						return resource.MakeCustomResourceReference(fixed, identity.id, "")
					}
					return resource.MakeComponentResourceReference(fixed, "")
				}

				// Without the successor's physical identity, retain the embedded ID while updating its logical identity.
				ref.URN = fixed
				ref.Name = fixed.Name()
