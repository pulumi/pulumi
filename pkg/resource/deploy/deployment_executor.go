	...
	canceled, err := func() (bool, error) {
		...
		for {
			...
			if seenNil && ex.asyncEventsExpected == 0 {
				...
				// nodes deleted or created this run are settled by sweep time.
				if p := ex.deployment.opts.WorkflowProgressor; p != nil &&
					ex.stepGen.mode == updateMode && !ex.deployment.opts.DryRun {
					persistState := func(urn resource.URN, outputs resource.PropertyMap) error {
						return ex.stepExec.ExecuteRegisterResourceOutputs(&workflowOutputsEvent{urn, outputs})
					}
					if perr := p.Progress(ctx, ex.deployment, persistState); perr != nil {
