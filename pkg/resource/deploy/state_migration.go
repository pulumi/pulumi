...
func (sg *stepGenerator) applyStateMigrations(
	...
	if opts.StateMigrationSerializer == nil {
	...
	//
	// Resources that are pending deletion are not handed to migrations because callbacks cannot account for state they
	// do not receive. We do not reject the migration here: a permanently attached migration may be a no-op, in which
	// case the update can proceed and clean them up normally. Keep them separately so a migration that does change
	// state can be rejected below before anything is committed.
