...
func TestCommitStateMigrationPreservesLivePointers(t *testing.T) {
	...
	require.NoError(t, prepareAndCommitStateMigration(sg,
		oldURN,
...
func TestCommitStateMigrationWithoutSuccessorsDoesNotPublishRewrite(t *testing.T) {
	t.Parallel()

	const rootURN = resource.URN("urn:pulumi:test::test::pkg:m:Component::component")
	prior := &pkgresource.State{
		URN: rootURN, Type: rootURN.Type(),
		Inputs: resource.PropertyMap{"version": resource.NewProperty("old")},
	}
	result := prior.Copy()
	result.Inputs = resource.PropertyMap{"version": resource.NewProperty("new")}
	d := &Deployment{
		prev:   &Snapshot{Resources: []*pkgresource.State{prior}},
		events: &mockEvents{},
	}

	require.NoError(t, prepareAndCommitStateMigration(
		&stepGenerator{deployment: d}, rootURN,
		[]*pkgresource.State{prior}, []*pkgresource.State{result}, nil))
	require.Equal(t, []*pkgresource.State{result}, d.prev.Resources)
	assert.Empty(t, d.stateMigrationRewrites)
}

func TestCommitStateMigrationPersistenceFailureLeavesStateUntouched(t *testing.T) {
	t.Parallel()
	...
	consumer := &pkgresource.State{
		URN: consumerURN, Type: consumerURN.Type(), Custom: true, ID: "consumer-id",
		Dependencies: []resource.URN{oldURN},
	}
	successor := &pkgresource.State{
		URN: successorURN, Type: successorURN.Type(), Custom: true, ID: old.ID,
	}
	persistErr := errors.New("could not persist migration")
	d := &Deployment{
		prev: &Snapshot{Resources: []*pkgresource.State{old, consumer}},
		events: &mockEvents{OnStateMigrationF: func(*StateMigrationTransaction) error {
			return persistErr
		}},
	}
	sg := &stepGenerator{deployment: d}
	transaction, err := sg.prepareStateMigrationTransaction(
		successorURN, old, []*pkgresource.State{old}, []*pkgresource.State{successor},
		map[resource.URN]resource.URN{oldURN: successorURN})
	require.NoError(t, err)

	err = sg.commitStateMigration(transaction)
	require.ErrorIs(t, err, persistErr)
	require.Len(t, d.prev.Resources, 2)
	assert.Same(t, old, d.prev.Resources[0])
	assert.Same(t, consumer, d.prev.Resources[1])
	assert.Equal(t, []resource.URN{oldURN}, consumer.Dependencies)
	assert.Empty(t, d.stateMigrationRewrites)
}
...
func TestPrepareStateMigrationTransactionRejectsRetainedProviderReferenceRewrite(t *testing.T) {
	t.Parallel()
	...
	_, err := sg.prepareStateMigrationTransaction(
		successorURN, old, []*pkgresource.State{old}, []*pkgresource.State{successor},
		map[resource.URN]resource.URN{oldURN: successorURN})
...
func TestPrepareStateMigrationTransactionRejectsCurrentProviderReferenceRewrite(t *testing.T) {
	t.Parallel()
