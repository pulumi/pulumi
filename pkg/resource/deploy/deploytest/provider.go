...
type Provider struct {
	...
	ParameterizeF func(key tokens.Package, args []string, version *semver.Version, value *pbstruct.Value) error
}
...
func (prov *Provider) GetSchema(version int, key string) ([]byte, error) {
	if prov.GetSchemaF == nil {
		return []byte("{}"), nil
	}
	return prov.GetSchemaF(version, key)
}
...
func (prov *Provider) Parameterize(key tokens.Package, args []string, version *semver.Version, value *pbstruct.Value) error {
