...
func (l *ResourceHooks) RegisterResourceHook(name string, hook ResourceHook) error {
	l.resourceHooksLock.Lock()
	defer l.resourceHooksLock.Unlock()

	if _, has := l.resourceHooks[name]; has {
		return fmt.Errorf("resource hook already registered for name %q", name)
	}

	if l.resourceHooks == nil {
		l.resourceHooks = make(map[string]ResourceHook)
	}
	l.resourceHooks[name] = hook
