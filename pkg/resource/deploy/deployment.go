...
type Deployment struct {
	...
	// true if PlanError should return the errors, else it logs them as warning
	enforcePlan bool
}

// PlanError either logs the error as a warning diagnostic and returns nil, or returns it as a failure result
func (d *Deployment) PlanError(urn resource.URN, err error) result.Result {
	if d.enforcePlan {
		return result.FromError(err)
	}
	msg := diag.RawMessage(urn, err.Error())
	d.ctx.Diag.Warningf(msg)
	return nil
}
...
func NewDeployment(ctx *plugin.Context, target *Target, prev *Snapshot, plan *Plan, enforcePlan bool, source Source,
