...
func (s *CreateStep) Apply() (resource.Status, StepCompleteFunc, error) {
	if err := s.Deployment().RunHooks("beforeCreate", s.new, true); err != nil {
	...
	if err := s.Deployment().RunHooks("afterCreate", s.new, false); err != nil {
...
func (s *DeleteStep) Apply() (resource.Status, StepCompleteFunc, error) {
	if err := s.Deployment().RunHooks("beforeDelete", s.old, true); err != nil {
		return resource.StatusOK, nil, err
	}
	...
	if err := s.Deployment().RunHooks("afterDelete", s.old, false); err != nil {
		return resource.StatusOK, nil, err
	}
...
func (s *UpdateStep) Apply() (resource.Status, StepCompleteFunc, error) {
	if err := s.Deployment().RunHooks("beforeUpdate", s.new, true); err != nil {
		return resource.StatusOK, nil, err
	}
	...
	if err := s.Deployment().RunHooks("afterUpdate", s.new, false); err != nil {
