...
func (s *ImportStep) Apply() (_ resource.Status, _ StepCompleteFunc, err error) {
	...
	suppliedInputs := resource.ToResourcePropertyMap(s.new.Inputs)
	suppliedOutputs := resource.ToResourcePropertyMap(s.new.Outputs)
	...
	s.new.Inputs = resource.FromResourcePropertyMap(inputs)
	s.new.Outputs = resource.FromResourcePropertyMap(outputs)
