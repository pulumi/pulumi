...
func newQueryResourceMonitor(
	...
	go func() {
		for e := range providerRegChan {
			...
			inputs, _, err := reg.Check(context.TODO(), urn, resource.PropertyMap{}, e.goal.Properties, false, nil)
			if err != nil {
				providerRegErrChan <- err
				return
			}
			id, _, _, err := reg.Create(context.TODO(), urn, inputs, 9999, false)
			if err != nil {
				providerRegErrChan <- err
				return
			}
...
func (rm *queryResmon) Invoke(
	...
	prov, err := getProviderFromSource(nil, rm.reg, rm.defaultProviders, providerReq, req.GetProvider(), tok)
	if err != nil {
		return nil, err
	}
