...
func (sg *stepGenerator) generateSteps(event RegisterResourceEvent) ([]Step, error) {
	...
	if prov != nil {
		...
		if !isTargeted {
			...
			checkInputs = func(urn resource.URN, name, typ string, olds, news resource.PropertyMap,
		...
		if recreating || wasExternal || sg.isTargetedReplace(urn) || !hasOld {
			inputs, failures, err = checkInputs(
				urn, string(urn.Name()), string(urn.Type()),
				nil, goal.Properties, allowUnknowns, randomSeed)
		} else {
			inputs, failures, err = checkInputs(
				urn, string(urn.Name()), string(urn.Type()),
				oldInputs, inputs, allowUnknowns, randomSeed)
		}
...
func (sg *stepGenerator) providerChanged(urn resource.URN, old, new *resource.State) (bool, error) {
	...
	purn := newRef.URN()
	diff, err := newProv.DiffConfig(
		purn, string(purn.Name()), string(purn.Type()),
		oldRes.Inputs, oldRes.Outputs, newRes.Inputs, true, nil)
	if err != nil {
		return false, err
	}
...
func diffResource(urn resource.URN, id resource.ID, oldInputs, oldOutputs,
	...
	diff, err := prov.Diff(
		urn, string(urn.Name()), string(urn.Type()),
		id, oldInputs, oldOutputs, newInputs, allowUnknowns, ignoreChanges)
	if err != nil {
		return diff, err
	}
...
func (sg *stepGenerator) calculateDependentReplacements(root *resource.State) ([]dependentReplace, error) {
	...
	requiresReplacement := func(r *resource.State) (bool, []resource.PropertyKey, error) {
		...
		diff, err := prov.Diff(
			r.URN, string(r.URN.Name()), string(r.URN.Type()),
			r.ID, r.Inputs, r.Outputs, inputsForDiff, true, nil)
		if err != nil {
			return false, nil, err
		}
