...
func (sg *stepGenerator) generateStepsFromDiff(
	...
	// Unknowns in replacement triggers are fine during preview, but they should raise an error during the actual operation.
	// This check only applies when we have an old resource (i.e., during updates/replaces), since a replacement
	// trigger only makes sense when there's something to replace.
