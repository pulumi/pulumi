	...
	if err == nil {
		...
		if step.Logical() && step.New() != nil {
			...
			if prior, has := se.pendingNews.Load(step.URN()); has {
				...
				// Excepting OpSame for the pattern of
				// ReadStep signalling failed
				// conditional read.
				if priorStep.Op() != OpSame {
					return fmt.Errorf("resource '%s' registered twice (%s and %s)",
						step.URN(), priorStep.Op(), step.Op())
				}
			}
