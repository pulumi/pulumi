...
type Plan interface {
	...
	Replaces() map[resource.URN][]resource.PropertyKey            // resources being replaced and their properties.
	Unchanged() map[resource.Resource]resource.Resource           // the resources untouched by this plan (map from old to new).
	Apply(prog Progress) (Snapshot, Step, resource.Status, error) // performs the operations specified in this plan.
}
...
type Step interface {
	...
	Old() resource.Resource          // the old resource state, if any, before performing this step.
	New() resource.Resource          // the new resource state, if any, after performing this step.
	NewProps() resource.PropertyMap  // the projected new resource state, factoring in dependency updates.
	Next() Step                      // the next step to perform, or nil if none.
	Apply() (resource.Status, error) // performs the operation specified by this step.
}
...
func NewPlan(ctx *plugin.Context, old Snapshot, new Snapshot, analyzers []tokens.QName) (Plan, error) {
	return newPlan(ctx, old, new, analyzers)
}

type plan struct {
	ctx       *plugin.Context                         // this plan's plugin context.
	...
	replaces  map[resource.URN][]resource.PropertyKey // resources being replaced and their properties.
	unchanged map[resource.Resource]resource.Resource // the resources that are remaining the same without modification.
}

var _ Plan = (*plan)(nil)

func (p *plan) Replaces() map[resource.URN][]resource.PropertyKey  { return p.replaces }
func (p *plan) Unchanged() map[resource.Resource]resource.Resource { return p.unchanged }
...
func (p *plan) checkpoint(resources []resource.Resource) Snapshot {
...
func newPlan(ctx *plugin.Context, old Snapshot, new Snapshot, analyzers []tokens.QName) (*plan, error) {
	...
	for _, new := range pb.NewRes {
		...
		if old, hasold := pb.Olds[m]; hasold {
			...
			computed := new.Inputs().ReplaceResources(func(r resource.URN) resource.URN {
				if pb.Replace(r) {
					// If the resource is being replaced, simply mangle the resource.URN so that it's different; this value
					...
					r = r.Replace()
					glog.V(7).Infof("Patched resource '%v's resource.URN property: %v", m, r)
				}
				return r
			})
			...
			if !old.Inputs().DeepEquals(computed) {
				...
				if len(replacements) > 0 {
					...
					replkeys := make([]resource.PropertyKey, len(replacements))
					for i, repl := range replacements {
						replkeys[i] = resource.PropertyKey(repl)
					}
...
type planBuilder struct {
	...
	Olds      map[resource.URN]resource.Resource      // a map of resource.URN to old resource.
	OldRes    []resource.Resource                     // a flat list of old resources (in topological order).
	News      map[resource.URN]resource.Resource      // a map of resource.URN to new resource.
	NewRes    []resource.Resource                     // a flat list of new resources (in topological order).
	Depends   map[resource.URN][]resource.URN         // a map of resource.URN to all existing (old) dependencies.
	Creates   map[resource.URN]*planVertex            // a map of pending creates to their associated vertex.
	Updates   map[resource.URN]*planVertex            // a map of pending updates to their associated vertex.
	Deletes   map[resource.URN]*planVertex            // a map of pending deletes to their associated vertex.
	Replaces  map[resource.URN][]resource.PropertyKey // a map of resource.URNs scheduled for replacement to properties being replaced.
	Unchanged map[resource.Resource]resource.Resource // a map of unchanged resources to their ID-stamped state.
}

// newPlanBuilder initializes a fresh plan state instance, ready to use for planning.
func newPlanBuilder(ctx *plugin.Context, old Snapshot, new Snapshot) *planBuilder {
	...
	return &planBuilder{
		P:         p,
		Olds:      make(map[resource.URN]resource.Resource),
		OldRes:    oldres,
		News:      make(map[resource.URN]resource.Resource),
		NewRes:    newres,
		Depends:   make(map[resource.URN][]resource.URN),
		Creates:   make(map[resource.URN]*planVertex),
		Updates:   make(map[resource.URN]*planVertex),
		Deletes:   make(map[resource.URN]*planVertex),
		Replaces:  make(map[resource.URN][]resource.PropertyKey),
		Unchanged: make(map[resource.Resource]resource.Resource),
	}
}

func (pb *planBuilder) Replace(m resource.URN) bool {
	return len(pb.Replaces[m]) > 0
}

func (pb *planBuilder) ConnectCreate(m resource.URN, v *planVertex) {
	pb.connectCreateUpdate(m, v, false)
}

func (pb *planBuilder) ConnectUpdate(m resource.URN, v *planVertex) {
	pb.connectCreateUpdate(m, v, true)
}

func (pb *planBuilder) connectCreateUpdate(m resource.URN, v *planVertex, update bool) {
	var label string
	if update {
		label = "Updating"
	} else {
	...
	new := v.Step().New()
	for dep := range new.Dependencies() {
...
func (pb *planBuilder) ConnectDelete(m resource.URN, v *planVertex) {
	// Add edges to:
...
func (pb *planBuilder) Plan() (*plan, error) {
	...
	var roots []*planEdge
	for _, vs := range []map[resource.URN]*planVertex{pb.Creates, pb.Updates, pb.Deletes} {
		for _, v := range vs {
			if len(v.Ins()) == 0 {
...
type step struct {
	...
	old      resource.Resource    // the state of the resource before this step.
	new      resource.Resource    // the state of the resource after this step.
	newprops resource.PropertyMap // the resource's properties, factoring in dependency updates.
...
func (s *step) NewProps() resource.PropertyMap { return s.newprops }
...
func newCreateStep(p *plan, new resource.Resource) *step {
	return &step{p: p, op: OpCreate, new: new, newprops: new.Inputs()}
}

func newDeleteStep(p *plan, old resource.Resource) *step {
	return &step{p: p, op: OpDelete, old: old, newprops: nil}
}

func newUpdateStep(p *plan, old resource.Resource, new resource.Resource, newprops resource.PropertyMap) *step {
	return &step{p: p, op: OpUpdate, old: old, new: new, newprops: newprops}
}

func newReplaceStep(p *plan, old resource.Resource, new resource.Resource, newprops resource.PropertyMap) *step {
	return &step{p: p, op: OpReplace, old: old, new: new, newprops: newprops}
}

func newReplaceCreateStep(p *plan, new resource.Resource) *step {
	return &step{p: p, op: OpReplaceCreate, new: new, newprops: new.Inputs()}
}

func newReplaceDeleteStep(p *plan, old resource.Resource) *step {
