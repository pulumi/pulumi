	...
	originalSnap := &deploy.Snapshot{
		Resources: []*resource.State{
			...
			},
			{
				...
				Custom: true,
				Inputs: resource.PropertyMap{
					"stringProp": resource.NewStringProperty("inputValue"),
					"numberProp": resource.NewNumberProperty(42),
					"boolProp":   resource.NewBoolProperty(true),
					"nullProp":   resource.NewNullProperty(),
					"infProp":    resource.NewNumberProperty(math.Inf(1)),
					"negInfProp": resource.NewNumberProperty(math.Inf(-1)),
				},
				Outputs: resource.PropertyMap{
					"outputProp": resource.NewStringProperty("outputValue"),
				},
		...
		Resources: []*resource.State{
			{
				...
				Type: resource.RootStackType,
				Inputs: resource.PropertyMap{
					"nan": resource.NewNumberProperty(math.NaN()),
				},
				Outputs: resource.PropertyMap{},
			},
		},
