...
func TestDeserializeResourceReferencePropertyValueID(t *testing.T) {
	...
	deserialized, err := DeserializePropertyValue(serialized, config.NewPanicCrypter(), config.NewPanicCrypter(), SecretsCache{})
	assert.NoError(t, err)
...
func TestRoundTripPropertyValue(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		deserialized, err := DeserializePropertyValue(wireObject, config.NopDecrypter, config.NopEncrypter, SecretsCache{})
		require.NoError(t, err)
