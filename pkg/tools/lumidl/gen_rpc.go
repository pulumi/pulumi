...
func (g *RPCGenerator) EmitResource(w *bufio.Writer, module tokens.Module, pkg *Package, res *Resource) {
	...
	writefmtln(w, "    Query(ctx context.Context, type string) ([]*%v, error)", name)
	writefmtln(w, "}")
	writefmtln(w, "")
	...
	writefmtln(w, "}")
	writefmtln(w, "")
	writefmtln(w, "func (p *%vProvider) Query(", name)
	writefmtln(w, "		ctx context.Context, req *lumirpc.QueryRequest) (*lumirpc.QueryResponse, error) {")
	writefmtln(w, " 	contract.Assert(req.GetType() == string(%vToken))", name)
	writefmtln(w, " 	objs, err := p.ops.Query(ctx,type)")
	writefmtln(w, " 	if err != nil {")
	writefmtln(w, " 		return nil, err")
	writefmtln(w, " 	}") // Create an array for each obj in objs, marshal properties.
	writefmtln(w, "		var ret []*%v", name)
	writefmtln(w, " 	for _, obj := range objs {")
	writefmtln(w, "			ret = append(ret, plugin.MarshalProperties(")
	writefmtln(w, "			nil, resource.NewPropertyMap(obj), plugin.MarshalOptions{})")
	writefmtln(w, "		return &lumirpc.QueryResponse{ret}")
	writefmtln(w, "		}")
	writefmtln(w, "}")
	writefmtln(w, "")
