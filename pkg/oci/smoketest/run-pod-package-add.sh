...
# A synthesized image cached from an earlier run does not encode whether the shim was
# embedded (refs don't carry the mode), so a stale one would silently satisfy the pull
# with the wrong topology. Clear it up front, as the wrapper does on mode switches.
docker image rm -f "$IMAGE_REF" >/dev/null 2>&1 || true
...
###############################################################################
# Phase 3: install replays the recorded pin (the fresh-checkout story). The project now
# carries packages: {random: oci://…}. Wipe everything add generated — as a fresh clone
# would be — and `pulumi install` must rebuild it: the oci:// entry resolves as a PIN
# (not registry-identifier parsing), no plugin binary download is attempted, the SDK is
# regenerated by running the image, and the regenerated SDK bakes the pin so a consumer
# still resolves from the pinned source rather than by convention.
###############################################################################
echo "==> PHASE 3: pulumi install (replay the recorded oci:// pin, as a fresh checkout would)"
rm -rf "$WORK/project/sdks" "$WORK/project/LINK_RECEIPT"
engine_run "pulumi install" 2>&1 | tee "$WORK/install.log"

echo "==> asserting install resolved the pin and regenerated the SDK from the image"
if grep -q "invalid identifier" "$WORK/install.log"; then
  echo "!! install still cannot resolve the recorded oci:// pin (registry-identifier parsing)"
  exit 1
fi
if ! find "$WORK/project/sdks" -name '*.go' 2>/dev/null | grep -q .; then
  echo "!! install did not regenerate the SDK from the pinned image"
  find "$WORK/project" -maxdepth 2 -type f 2>/dev/null | sed 's/^/    /' | head -20
  exit 1
fi
if ! grep -rq "$OCI_SOURCE" "$WORK/project/sdks"; then
  echo "!! the regenerated SDK does not bake the oci:// pin — a consumer would resolve by"
  echo "   convention instead of from the pinned source"
  exit 1
fi
if [ ! -f "$RECEIPT" ]; then
  echo "!! install did not re-link the regenerated SDK (no LINK_RECEIPT)"
  exit 1
fi
echo "    install regenerated + re-linked the SDK from the image; the pin is baked into the SDK"

echo "==> package-add-from-image smoke test PASS — a provider was consumed as an IMAGE:"
echo "    schema read from the running container, SDK generated via delegation, ref (not path)"
echo "    recorded, and 'pulumi install' replays the recorded pin from a clean tree"
