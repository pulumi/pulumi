module oci-smoke-transform-program
...
// Build against the in-repo SDK. The program binary is compiled on the host and
// COPY'd into the image, so this relative replace never enters the Docker build.
