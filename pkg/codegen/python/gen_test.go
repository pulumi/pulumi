...
// TestGenerateDeprecatedInputWarnOnNonNilInInit verifies that deprecated optional input
// properties emit a warnings.warn call in __init__ only when the caller passes a non-None
// value. This is the user-construction warning path. The separate deserialization path
// (translate_output_properties → input_type_from_dict) bypasses __init__ entirely, so
// provider-returned values never trigger these warnings.
func TestGenerateDeprecatedInputWarnOnNonNilInInit(t *testing.T) {
	t.Parallel()
	...
	// Input types may be generated inline in a resource file (e.g. thing.py) or in
	// a dedicated _inputs.py. Search for the file containing the ThingArgs input type.
	var inputs string
	for _, file := range files {
		if strings.Contains(string(file), "class ThingArgs") {
			inputs = string(file)
			break
		}
	}

	require.NotEmpty(t, inputs, "no generated file with ThingArgs found")

	// __init__ must guard the deprecated warning behind a non-None check.
	assert.Contains(t, inputs, "if cidr_block is not None:\n            warnings.warn",
		"__init__ must warn only for non-None deprecated values")

	// Setter must NOT carry a @_utilities.deprecated() decorator — the warning is in __init__,
	// not in the setter, so that deserialization via pulumi.set() does not trigger warnings.
	assert.NotContains(t, inputs,
		"@cidr_block.setter\n    @_utilities.deprecated",
		"setter must not have @_utilities.deprecated() decorator")

	// Non-deprecated fields must use pulumi.set directly (no warning wrapping).
	assert.Contains(t, inputs, `pulumi.set(__self__, "name", name)`,
		"non-deprecated fields must use pulumi.set")
}
