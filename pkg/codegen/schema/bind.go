...
//     invalid type is accepted in any position, and carries diagnostics that explain the semantic error during
//     binding. This allows binding to continue and produce as much information as possible for the end user.
...
//     diagnostic. Until we have line/column information, we use JSON pointers to the offending entities. These
//     pointers are passed around using `path` parameters. The `errorf` function is provided as a utility to easily
//     create a diagnostic error that is appropriately tagged with a JSON pointer.
