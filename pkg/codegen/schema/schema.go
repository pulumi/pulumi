...
type Resource struct {
	...
	IsComponent bool
	// Methods is the list of methods for the resource. Methods are never flattened across the extends chain: this list
	// holds only the resource's own methods, including overrides. Use AllMethods to observe the transitive set.
	Methods []*Method
	// BaseResource is the immediate base component that this component extends, or nil if it has no base. The bound
	// InputProperties and Properties are always the full flattened member set of the chain; OwnProperties and
	// InheritedProperties partition that set relative to BaseResource.
	BaseResource *Resource
	// Abstract indicates that the component is abstract and cannot be instantiated directly, only extended.
	Abstract bool
	// baseDescriptor is the package descriptor used to resolve a cross-package base, if any. It is nil for same-package
	// bases; BaseDescriptor synthesizes a descriptor from BaseResource in that case.
	baseDescriptor *PackageDescriptor
	// IsOverlay indicates whether the type is an overlay provided by the package. Overlay code is generated by the
	// package rather than using the core Pulumi codegen libraries.
	IsOverlay bool
	// OverlaySupportedLanguages indicates what languages the overlay supports. This only has an effect if
...
// baseMemberNames returns the set of member names in the immediate base's flattened property list. It is the key used
// to partition this resource's flattened members into own vs. inherited.
func (r *Resource) baseMemberNames(inputs bool) map[string]struct{} {
	if r.BaseResource == nil {
		return nil
	}
	base := r.BaseResource.Properties
	if inputs {
		base = r.BaseResource.InputProperties
	}
...
// OwnInputProperties returns the input properties declared by this resource itself, i.e. the flattened input properties
// minus those inherited from the base. For a root component it is the full input property list.
func (r *Resource) OwnInputProperties() []*Property {
	return partitionProperties(r.InputProperties, r.baseMemberNames(true), true)
}

// OwnProperties returns the output properties declared by this resource itself, i.e. the flattened properties minus
// those inherited from the base. For a root component it is the full property list.
func (r *Resource) OwnProperties() []*Property {
	return partitionProperties(r.Properties, r.baseMemberNames(false), true)
}

// InheritedInputProperties returns the input properties this resource inherits from its base (the flattened input
// properties whose names appear in the base). It is empty for a root component.
func (r *Resource) InheritedInputProperties() []*Property {
	return partitionProperties(r.InputProperties, r.baseMemberNames(true), false)
}

// InheritedProperties returns the output properties this resource inherits from its base (the flattened properties
// whose names appear in the base). It is empty for a root component.
func (r *Resource) InheritedProperties() []*Property {
	return partitionProperties(r.Properties, r.baseMemberNames(false), false)
}

// partitionProperties selects the members of props whose membership in names matches want. When names is nil (no base)
// every property counts as own (want == true) and none as inherited (want == false).
func partitionProperties(props []*Property, names map[string]struct{}, own bool) []*Property {
	result := slice.Prealloc[*Property](len(props))
	for _, p := range props {
		_, inherited := names[p.Name]
		if inherited != own {
...
// AllMethods returns this resource's own methods plus the methods inherited from its base chain, with
// nearest-level-wins shadowing by name (an override on a derived component hides the base's method of the same name).
// The result is sorted by method name.
func (r *Resource) AllMethods() []*Method {
	seen := map[string]struct{}{}
	var result []*Method
	for res := r; res != nil; res = res.BaseResource {
...
// BaseDescriptor returns the package descriptor of the immediate base component: its name, version, download URL, and
// parameterization. Generated base-construct stubs need exactly this — and only the immediate base — to register the
// base package and issue the base-construct RPC; the chain is walked one level at a time. It returns nil for a root
// component.
func (r *Resource) BaseDescriptor() *PackageDescriptor {
	if r.BaseResource == nil {
		return nil
	}
	if r.baseDescriptor != nil {
		return r.baseDescriptor
	}
	ref := r.BaseResource.PackageReference
	return &PackageDescriptor{
		Name:        ref.Name(),
		Version:     ref.Version(),
		DownloadURL: ref.PluginDownloadURL(),
	}
}
...
func (pkg *Package) marshalResource(r *Resource) (ResourceSpec, error) {
	...
	// The bound model is always flattened, so we emit the extends ref alongside the full member set. The reference is
	// reconstructed from the base's package identity and token; requiredFeatures is never emitted because a flattened
	// package no longer relies on inheritance materialization.
	var extends *TypeSpec
	if r.BaseResource != nil {
		extends = &TypeSpec{
			Ref: pkg.marshalTypeRef(r.BaseResource.PackageReference, "resources", r.BaseResource.Token),
		}
	}
...
type ResourceSpec struct {
	...
	// Extends, if present, references the base component that this component extends. Only the $ref field may be set.
	// Extending is only valid for components (IsComponent must be true), and the referenced resource must itself be a
	// component.
	Extends *TypeSpec `json:"extends,omitempty" yaml:"extends,omitempty"`
	// Abstract indicates that the component is abstract and cannot be instantiated directly, only extended. Only valid
	// for components (IsComponent must be true).
...
type PackageSpec struct {
	...
	// RequiredFeatures lists schema features a consumer must support in order to correctly interpret this package. A
	// consumer that does not recognize a listed feature must reject the package rather than silently misinterpret it.
	// The binder materializes flattened resource definitions, so a successfully bound and re-marshaled package never
	// carries the "inheritance" feature.
