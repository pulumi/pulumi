...
type Resource struct {
	...
	// package rather than using the core Pulumi codegen libraries.
	IsOverlay bool
	// ReplaceOnChangesOverride contains an optional list of property path patterns that will be automatically propagated
	// to the resource registration calls in each SDK. The values specified here are merged with property-level
	// ReplaceOnChanges annotation.
	// See https://www.pulumi.com/docs/intro/concepts/resources/options/replaceonchanges/ for supported path patterns.
	//
	// For instance, this can be used to mark an entire resource immutable by specifying ReplaceOnChangesOverride: ["*"]
	// or adding resource-specific overrides for replacement behavior in nested properties.
	//
	// TODO: Check if this is a valid path. Error out if not (stringly typed).
	ReplaceOnChangesOverride []string
}

// ReplaceOnChanges is the set of resource paths where ReplaceOnChanges is true.
//
...
func (r *Resource) ReplaceOnChanges() (changes [][]*Property, overrides []string, err []error) {
	...
	// TODO validate r.ReplaceOnChangesOverride
	return changes, r.ReplaceOnChangesOverride, err
}
...
func FormatReplaceOnChanges(propertyList [][]*Property, replaceOnChanges []string, nameFormatter func(string) string) []string {
	...
	out := make([]string, len(propertyList)+len(replaceOnChanges))
	for i, p := range propertyList {
		names := make([]string, len(p))
		for j, n := range p {
			if _, ok := nonOptional(n.Type).(*ArrayType); ok {
				names[j] = nameFormatter(n.Name) + "[*]"
	...
	for i, r := range replaceOnChanges {
		out[len(propertyList)+i] = formatReplaceOnChangesPattern(r, nameFormatter)
	}
	return out
}

func formatReplaceOnChangesPattern(replaceOnChangePattern string, nameFormatter func(string) string) string {
	s := scanner.Scanner{}
	s.Init(strings.NewReader(replaceOnChangePattern))

	formatted := strings.Builder{}
	for tok := s.Scan(); tok != scanner.EOF; tok = s.Scan() {
		switch tok {
		case '[', ']', '.', '*':
			formatted.WriteString(s.TokenText())
		default:
			formatted.WriteString(nameFormatter(s.TokenText()))
		}
	}
...
type ResourceSpec struct {
	...
	//
	// TODO: Check if this is a valid path. Error out if not (stringly typed).
	ReplaceOnChangesOverride []string `json:"replaceOnChangesOverride,omitempty" yaml:"replaceOnChangesOverride,omitempty"`
}
