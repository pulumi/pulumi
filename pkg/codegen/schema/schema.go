...
func GetMember(container HasMembers, pointer string) (interface{}, bool) {
	refs := strings.Split(pointer, "/")

	normalized := refs[:0]
	for _, r := range refs {
		if r != "" {
			unescaped, err := url.PathUnescape(r)
			if err != nil {
				return nil, false
			}
			normalized = append(normalized, unescaped)
		}
	}
	refs = normalized

	object := interface{}(container)
	for len(refs) > 0 {
		container, ok := object.(HasMembers)
		if !ok {
			return nil, false
		}
		object, refs, ok = container.getOwnMember(refs)
		if !ok {
			return nil, false
		}
	}
...
func (t *EnumType) GetMember(pointer string) (interface{}, bool) {
...
func (t *EnumType) getOwnMember(path []string) (interface{}, []string, bool) {
	...
	idx, err := strconv.ParseInt(path[1], 10, 0)
	if err != nil || idx < 0 || int(idx) >= len(t.Elements) {
		return nil, nil, false
	}

	return t.Elements[idx], path[2:], true
}
...
func (t *ObjectType) getOwnMember(path []string) (interface{}, []string, bool) {
	...
	property, ok := t.Property(path[0])
	return property, path[1:], ok
}
...
func (r *Resource) getOwnMember(path []string) (interface{}, []string, bool) {
	...
	switch path[0] {
	case "properties":
		return &ObjectType{Properties: r.Properties}, path[1:], true
	case "inputProperties":
		return &ObjectType{Properties: r.InputProperties}, path[1:], true
	case "stateInputs":
		return r.StateInputs, path[1:], true
	case "methods":
		if len(path) < 2 {
			return nil, nil, false
		}
...
func (f *Function) getOwnMember(path []string) (interface{}, []string, bool) {
	...
	case "inputs":
		...
		return f.Inputs, path[1:], true
	case "outputs":
		if f.Outputs != nil {
			return nil, nil, false
		}
		return f.Outputs, path[1:], true
	default:
		return nil, nil, false
	}
}
...
func (pkg *Package) getOwnMember(refs []string) (interface{}, []string, bool) {
	...
	switch refs[0] {
	case "types":
		m, ok := pkg.GetType(refs[1])
		return m, refs[2:], ok
	case "resources":
		m, ok := pkg.GetResource(refs[1])
		return m, refs[2:], ok
	case "functions":
		m, ok := pkg.GetFunction(refs[1])
		return m, refs[2:], ok
	default:
		return nil, nil, false
	}
...
func (f *Function) NeedsOutputVersion() bool {
