...
func (pkg *pkgContext) genInputImplementation(w io.Writer, name, receiverType, elementType string, ptrMethods bool) {
...
func (pkg *pkgContext) genInputImplementationWithArgs(w io.Writer, genArgs genInputImplementationArgs) {
	...
	if genArgs.ptrMethods && !pkg.isGeneric {
...
func (pkg *pkgContext) genOutputType(w io.Writer, baseName, elementType string, ptrMethods bool) {
...
func (pkg *pkgContext) genArrayOutput(w io.Writer, baseName, elementType string) {
	...
	pkg.genOutputType(w, baseName+"Array", "[]"+elementType, false)
...
func (pkg *pkgContext) genMapOutput(w io.Writer, baseName, elementType string) {
	...
	pkg.genOutputType(w, baseName+"Map", "map[string]"+elementType, false)
...
func (pkg *pkgContext) genPtrOutput(w io.Writer, baseName, elementType string) {
	...
	pkg.genOutputType(w, baseName+"Ptr", "*"+elementType, false)
...
func (pkg *pkgContext) genEnum(w io.Writer, enumType *schema.EnumType) error {
	...
	if details.arrayInput {
		...
		pkg.genInputImplementation(w, name+"Array", name+"Array", "[]"+name, false)
	}
	...
	if details.mapInput {
		...
		pkg.genInputImplementation(w, name+"Map", name+"Map", "map[string]"+name, false)
	}
...
func (pkg *pkgContext) genEnumOutputTypes(w io.Writer, name, elementArgsType, elementGoType, asFuncName string) {
	pkg.genOutputType(w, name, name, true)
...
func (pkg *pkgContext) genInputTypes(w io.Writer, t *schema.ObjectType, details *typeDetails) error {
	...
	if details.input {
		...
		pkg.genInputImplementation(w, name, inputName, name, details.ptrInput)
	}

	if pkg.isGeneric {
		// If we are in a generic context, we don't need to generate any type except the
		// Args type.
		return nil
	}
	...
	if details.ptrInput {
		...
		pkg.genInputImplementation(w, name+"Ptr", "*"+ptrTypeName, "*"+name, false)
	}
...
func (pkg *pkgContext) genOutputTypes(w io.Writer, genArgs genOutputTypesArgs) {
	...
	if details.ptrOutput && !pkg.isGeneric {
		pkg.genPtrOutput(w, name, name)
...
func (pkg *pkgContext) genResource(w io.Writer, r *schema.Resource, generateResourceContainerTypes bool) error {
	...
	pkg.genInputImplementation(w, name, "*"+name, "*"+name, false)
	...
	pkg.genOutputType(w, name, "*"+name, false)
	...
	for _, elementTypeName := range sortedElems {
		...
		for _, name := range collectionTypes {
			...
			case strings.HasSuffix(name, "ArrayInput"):
				...
				pkg.genInputImplementation(w, name, name, "[]"+info.resolvedElementType, false)
			...
			case strings.HasSuffix(name, "MapInput"):
				...
				pkg.genInputImplementation(w, name, name, "map[string]"+info.resolvedElementType, false)
...
type GenericToggle int

const (
	GenericToggleYes = iota
	GenericToggleNo
	GenericToggleBoth
...
func GeneratePackage(tool string, pkg *schema.Package) (map[string][]byte, error) {
	...
	var setFile func(string, string, GenericToggle)
	setFile = func(relPath, contents string, generic GenericToggle) {
		switch generic {
		case GenericToggleYes:
			relPath = path.Join(GenericPrefix, relPath)
		case GenericToggleBoth:
			// Write the generic version
			setFile(relPath, contents, GenericToggleYes)
			// Write the normal version
		case GenericToggleNo:
			// do nothing
		default:
			panic(fmt.Sprintf("Invalid option: %v", generic))
		}
	...
	for _, mod := range pkgMods {
		...
		case "":
			...
			setFile(path.Join(mod, "doc.go"), buffer.String(), GenericToggleBoth)
		...
		case "config":
			if len(pkg.pkg.Config) > 0 {
				...
				setFile(path.Join(mod, "config.go"), buffer.String(), GenericToggleBoth)
			}
		}
		...
		for _, r := range pkg.resources {
			...
			err := pkg.genGeneric(func(gt GenericToggle) error {
				importsAndAliases := map[string]string{}
				pkg.getImports(r, importsAndAliases)
				importsAndAliases["github.com/pulumi/pulumi/sdk/v3/go/pulumi"] = ""

				buffer := &bytes.Buffer{}
				goImports := []string{"reflect", "context"}
				...
				setFile(path.Join(mod, camel(rawResourceName(r))+".go"), buffer.String(), gt)
				return nil
			})
			if err != nil {
				return nil, err
		...
		if len(pkg.enums) > 0 {
			...
			setFile(path.Join(mod, "pulumiEnums.go"), buffer.String(), GenericToggleBoth)
		}
		...
		err := pkg.genGeneric(func(gt GenericToggle) error {
			for types, i := pkg.types, 0; len(types) > 0; i++ {
				...
				buffer := &bytes.Buffer{}
				err := pkg.generateTypes(buffer, chunk, sortedKnownTypes)
				if err != nil {
					return err
				}
				...
				setFile(path.Join(mod, typePath+".go"), buffer.String(), gt)
			}
			return nil
		})
		if err != nil {
		...
		if pkg.needsUtils || len(mod) == 0 {
			...
			setFile(path.Join(mod, "pulumiUtilities.go"), buffer.String(), GenericToggleBoth)
		}
		...
		if len(pkg.resources) != 0 && !allResourcesAreOverlays(pkg.resources) {
			...
			setFile(path.Join(mod, "init.go"), buffer.String(), GenericToggleBoth)
		}
	}
...
func (pkg *pkgContext) generateTypes(w io.Writer, types []*schema.ObjectType, knownTypes []schema.Type) error {
