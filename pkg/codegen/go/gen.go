...
func (pkg *pkgContext) genResourceModule(w io.Writer) {
	...
	if !topLevelModule {
		imports.Add(pkg.importBasePath)
	}
	...
	if topLevelModule {
		...
		basePkg := pkg.importBasePath[strings.LastIndex(pkg.importBasePath, "/")+1:]
...
func GeneratePackage(tool string, pkg *schema.Package) (map[string][]byte, error) {
	...
	for _, mod := range pkgMods {
		...
		if pkg.needsUtils || len(mod) == 0 {
			...
			pkg.genHeader(buffer, []string{"fmt", "os", "reflect", "regexp", "strconv", "strings"}, imports)
