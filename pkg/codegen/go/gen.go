...
func (pkg *pkgContext) genEnumInputTypes(w io.Writer, name string, enumType *schema.EnumType, elementGoType string) {
	...
	fmt.Fprintf(w, "type %sPtr %s\n", codegen.Camel(name), elementGoType)
	fmt.Fprintln(w)
...
func (pkg *pkgContext) assignProperty(w io.Writer, p *schema.Property, object, value string, indirectAssign bool) {
	...
	if codegen.IsNOptionalInput(p.Type) {
		...
		tmpName := codegen.Camel(p.Name) + "_"
		fmt.Fprintf(w, "%s := %s\n", tmpName, value)
...
func (pkg *pkgContext) genInputTypes(w io.Writer, t *schema.ObjectType, details *typeDetails) error {
	...
	if details.ptrInput {
		...
		ptrTypeName := codegen.Camel(name) + "PtrType"
...
func (pkg *pkgContext) genResource(w io.Writer, r *schema.Resource, generateResourceContainerTypes bool) error {
	...
	for _, method := range r.Methods {
		...
		if f.Outputs != nil {
			if shouldLiftReturn {
				outputsType = fmt.Sprintf("%s%sResultOutput", codegen.Camel(name), methodName)
			} else {
		...
		if f.Outputs == nil {
			...
			fmt.Fprintf(w, "\treturn %s.(%s).%s(), nil\n", resultVar, codegen.Camel(outputsType), Title(f.Outputs.Properties[0].Name))
		} else {
			// Check the error before proceeding.
			fmt.Fprintf(w, "\tif err != nil {\n")
			fmt.Fprintf(w, "\t\treturn %s{}, err\n", outputsType)
		...
		if f.Outputs != nil {
			...
			if shouldLiftReturn {
				outputStructName = codegen.Camel(name)
			}
...
func (pkg *pkgContext) genConfig(w io.Writer, variables []*schema.Property) error {
	...
	for _, p := range variables {
		...
		printCommentWithDeprecationMessage(w, p.Comment, p.DeprecationMessage, false)
		configKey := fmt.Sprintf("\"%s:%s\"", pkg.pkg.Name, codegen.Camel(p.Name))
...
func generatePackageContextMap(tool string, pkg *schema.Package, goInfo GoPackageInfo, externalPkgs *Cache) map[string]*pkgContext {
	...
	scanResource := func(r *schema.Resource) {
		...
		getNames := func(suffix string) []string {
			...
			names = append(names, codegen.Camel(rawResourceName(r))+suffix+"Args")
			names = append(names, "New"+rawResourceName(r)+suffix)
			if !r.IsProvider && !r.IsComponent {
				names = append(names, rawResourceName(r)+suffix+"State")
				names = append(names, codegen.Camel(rawResourceName(r))+suffix+"State")
...
func GeneratePackage(tool string, pkg *schema.Package) (map[string][]byte, error) {
	...
	for _, mod := range pkgMods {
		...
		for _, r := range pkg.resources {
			...
			setFile(path.Join(mod, codegen.Camel(rawResourceName(r))+".go"), buffer.String())
		}
		...
		for _, f := range pkg.functions {
			...
			fileName := path.Join(mod, codegen.Camel(tokenToName(f.Token))+".go")
...
func Title(s string) string {
	...
	runes := []rune(s)
	return codegen.ConvertHyphens(string(append([]rune{unicode.ToUpper(runes[0])}, runes[1:]...)), "_")
}
