...
// An import, either itemized or of a whole module.
//
// It is illegal to have both named items and a module name to import.
type Import struct {
	// The list of specific items to import
	//
	// Example:
	// import {Foo, Bar} from "..";
	NamedItems codegen.StringSet
...
func (imp *Import) AddItem(item string) {
	contract.Assert(imp.ModuleName == "")
	if imp.NamedItems == nil {
		imp.NamedItems = codegen.NewStringSet()
	}
	imp.NamedItems.Add(item)
}

func (imp *Import) IsItimized() bool {
	contract.Assert(len(imp.NamedItems) > 0 || imp.ModuleName != "")
	return len(imp.NamedItems) > 0
}

func (imp *Import) SortedValues() []string {
	if imp == nil {
		return []string{}
	}
	contract.Assert(imp.ModuleName == "")
	return imp.NamedItems.SortedValues()
}
...
func (mod *modContext) getTypeImports(t schema.Type, recurse bool, externalImports codegen.StringSet, imports Imports, seen codegen.Set) bool {
	return mod.getTypeImportsForResource(t, recurse, externalImports, imports, seen, nil)
}

func (mod *modContext) getTypeImportsForResource(t schema.Type, recurse bool, externalImports codegen.StringSet, imports Imports, seen codegen.Set, res *schema.Resource) bool {
	if seen.Has(t) {
		return false
	}
	seen.Add(t)
	...
	resourceOrTokenImport := func(tok string) bool {
		...
		// This is always a local path. Missing the "." indicates that we are
		// not looking for the above directory (".."). Because codegen
		// references above types directly, we handle these imports differently.
		if !strings.HasPrefix(modPath, ".") {
			firstSegment := strings.Index(modPath, "/")
			if firstSegment != -1 {
				modPath = modPath[:firstSegment]
			}
			name := modPath
			modPath = "./" + modPath
			if imports[modPath] == nil {
				imports[modPath] = &Import{}
			}
			imports[modPath].ModuleName = name
		} else {
			if imports[modPath] == nil {
				imports[modPath] = &Import{}
			}
			imports[modPath].AddItem(name)
		}
		return false
	}
...
func (mod *modContext) getImports(member interface{}, externalImports codegen.StringSet, imports Imports) bool {
	return mod.getImportsForResource(member, externalImports, imports, nil)
}

func (mod *modContext) getImportsForResource(member interface{}, externalImports codegen.StringSet, imports Imports, res *schema.Resource) bool {
	seen := codegen.Set{}
...
func (mod *modContext) genHeader(w io.Writer, imports []string, externalImports codegen.StringSet, importedTypes Imports) {
	...
	if len(importedTypes) > 0 {
		...
		for _, modPath := range modules {
			imp := importedTypes[modPath]
			if imp.IsItimized() {
				fmt.Fprintf(w, "import {")
				for i, name := range imp.SortedValues() {
					if i > 0 {
						fmt.Fprint(w, ", ")
					}
					fmt.Fprint(w, name)
...
func (mod *modContext) genConfig(w io.Writer, variables []*schema.Property) error {
	externalImports, imports := codegen.NewStringSet(), Imports{}
