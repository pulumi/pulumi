...
func (mod *modContext) objectType(pkg schema.PackageReference, details *typeDetails, tok string, input, args, enum bool) string {
	root := "outputs."
	if (details != nil && details.inputType) || input {
		root = "inputs."
	}
	...
	return pkgName + root + title(name)
}
...
func (mod *modContext) sdkImports(nested, utilities bool) []string {
	...
	if nested {
		...
		if mod.hasEnums() {
			imports = append(imports, `import * as enums from "./enums";`)
		}
	}
...
func (mod *modContext) genTypes() (string, string, error) {
	...
	for _, t := range mod.types {
		...
		if mod.details(t).inputType {
			if err := mod.genType(inputs, t, true, 0); err != nil {
				return "", "", err
			}
		} else if mod.details(t).outputType {
			if err := mod.genType(outputs, t, false, 0); err != nil {
				return "", "", err
			}
		}
	}
...
func (mod *modContext) gen(fs codegen.Fs) error {
	...
	if mod.hasEnums() {
		...
		for _, e := range mod.enums {
			if err := mod.genEnum(buffer, e); err != nil {
				return err
			}
		}

		fs.Add(path.Join(modDir, "enums.ts"), buffer.Bytes())
	}
...
func (mod *modContext) genIndex(exports []fileInfo) string {
	...
	if len(mod.enums) > 0 {
		...
		fmt.Fprintf(w, "export * from \"./enums\";\n")
	}
...
func generateModuleContextMap(tool string, pkg *schema.Package, extraFiles map[string][]byte,
	...
	if len(pkg.Config) > 0 &&
		...
		config := getMod("config")

		visitObjectTypes(pkg.Config, func(t *schema.ObjectType) {
			config.details(t).outputType = true
		})
	}
...
// genBackwardsCompatibleTypesFiles generates a `types` module hierarchy for
// backwards compatibility with older SDKs and SDK consumers.
//
// Historically, there was a single `types` module that contained all types
// for a given provider -- inputs, outputs, and enumerations. These have since
// been split up and such types are now located as closely as possible to their
// related resources and functions. As well as locating types where most users
// expect, this also results in improved type checking performance, particularly
// for large providers. We can't break older consumers without a major version
// change, however, so this function reconstructs the original `types` modules
// by re-exporting all the fine-grained modules we now generate. This function
// should be deleted when the next major version of the SDK is released.
func genBackwardsCompatibleTypesFiles(fs codegen.Fs, modules map[string]*modContext) error {
	var indexModule *modContext

	ioModules := make([]*modContext, 0)
	enumModules := make([]*modContext, 0)

	// `genEnums` generates a module that re-exports a set of enumeration types. The
	// `reexports` parameter is designed for the top-level `index` module, which
	// re-exports all its children in a qualified manner.
	genEnums := func(mod *modContext, reexports []*modContext) error {
		modDir := strings.ToLower(mod.mod)

		fileDir := path.Join("types", "enums", modDir)
		filePath := path.Join(fileDir, "index.ts")

		importPath, err := filepath.Rel(fileDir, path.Join(modDir, "enums"))
		if err != nil {
			return err
		}

		b := new(bytes.Buffer)
		mod.genHeader(b, nil, nil, nil)
		b.WriteString(fmt.Sprintf("export * from \"%s\";\n", filepath.ToSlash(importPath)))

		for _, reexport := range reexports {
			modDir := strings.ToLower(reexport.mod)
			...
			b.WriteString(fmt.Sprintf("export * as %s from \"%s\";\n", modDir, filepath.ToSlash(importPath)))
		}

		fs.Add(filePath, b.Bytes())
		return nil
	}

	// `genIO` generates a module that re-exports all input or output types for a
	// given module. The `reexports` parameter is designed for the top-level
	// `index` module, which re-exports all its children in a qualified manner.
	genIO := func(mod *modContext, io string, reexports []*modContext) error {
		b := new(bytes.Buffer)
		mod.genHeader(b, nil, nil, nil)
		b.WriteString(fmt.Sprintf("export * from \"../%s\";\n", io))
		...
		for _, reexport := range reexports {
			...
			importPath, err := filepath.Rel("types", path.Join(modDir, io))
			if err != nil {
				return err
			}
		...
		filePath := path.Join("types", io+".ts")
		fs.Add(filePath, b.Bytes())

		return nil
	}

	// The very top-level `types/index` module re-exports all the types
	// accumulated by `genEnums` and `genIO`.
	genTopLevel := func(mod *modContext) {
		b := new(bytes.Buffer)
		mod.genHeader(b, nil, nil, nil)
		b.WriteString("export * as enums from \"./enums\";\n")
		b.WriteString("export * as input from \"./input\";\n")
		b.WriteString("export * as output from \"./output\";\n")

		filePath := path.Join("types", "index.ts")
		fs.Add(filePath, b.Bytes())
	}
	...
	for _, mod := range modules {
		...
		if len(mod.types) > 0 {
			ioModules = append(ioModules, mod)
		}

		if mod.hasEnums() {
			enumModules = append(enumModules, mod)
			if err := genEnums(mod, nil); err != nil {
				return err
			}
		}
	}

	sort.SliceStable(ioModules, func(i, j int) bool {
		return ioModules[i].mod < ioModules[j].mod
	})

	sort.SliceStable(enumModules, func(i, j int) bool {
		return enumModules[i].mod < enumModules[j].mod
	})

	if indexModule != nil {
		if err := genEnums(indexModule, enumModules); err != nil {
			return err
		}

		if err := genIO(indexModule, "input", ioModules); err != nil {
			return err
		}

		if err := genIO(indexModule, "output", ioModules); err != nil {
			return err
		}

		genTopLevel(indexModule)
	}
