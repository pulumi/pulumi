...
func (mod *modContext) genType(w io.Writer, obj *schema.ObjectType, input bool, level int) error {
	...
	if hasInfo {
		...
		if requiredProperties != nil {
			required := mapset.NewSet[string](requiredProperties...)
...
func (mod *modContext) getImports(member interface{}, externalImports mapset.Set[string], imports map[string]mapset.Set[string]) bool {
	return mod.getImportsForResource(member, externalImports, imports, nil)
}

func (mod *modContext) getImportsForResource(member interface{}, externalImports mapset.Set[string], imports map[string]mapset.Set[string], res *schema.Resource) bool {
...
func (mod *modContext) genHeader(w io.Writer, imports []string, externalImports mapset.Set[string], importedTypes map[string]mapset.Set[string]) {
	...
	if externalImports.Cardinality() > 0 {
		for _, i := range codegen.SortedValues(externalImports) {
			fmt.Fprintf(w, "%s\n", i)
		}
		fmt.Fprintf(w, "\n")
	}
	...
	if len(importedTypes) > 0 {
		...
		for _, module := range modules {
			...
			for i, name := range codegen.SortedValues(importedTypes[module]) {
				if i > 0 {
					fmt.Fprint(w, ", ")
				}
				fmt.Fprint(w, name)
...
func (mod *modContext) genIndex(exports []fileInfo) string {
	...
	if children.Cardinality() > 0 {
		...
		for _, child := range codegen.SortedValues(children) {
			directChildren.Add(path.Base(child))
		}
		sorted := codegen.SortedValues(directChildren)
...
func (mod *modContext) genResourceModule(w io.Writer) {
	...
	if providerOnly := len(mod.resources) == 1 && mod.resources[0].IsProvider; providerOnly {
		...
		fmt.Fprintf(w, "    },\n")
		fmt.Fprintf(w, "};\n")
		for _, name := range codegen.SortedValues(registrations) {
