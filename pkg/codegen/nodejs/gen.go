...
func (mod *modContext) splitTypesEnabled() bool {
	var info, ok = mod.pkg.Language["nodejs"].(NodePackageInfo)
	// Use the default if no language-specific configuration was provided.
	if !ok {
		return SplitTypesDefault
	}
	// Use the default if no config was explicitly provided.
	if info.SplitTypesFiles == nil {
		return SplitTypesDefault
	}
	return *info.SplitTypesFiles
}

func (mod *modContext) genTypes() ([]*ioFile, error) {
	if mod.splitTypesEnabled() {
		return mod.genTypesWithSplitting()
	}
	return mod.genTypesNoSplitting()
}

func (mod *modContext) genTypesWithSplitting() ([]*ioFile, error) {
	var (
...
func (mod *modContext) genTypesNoSplitting() ([]*ioFile, error) {
	var modDir = strings.ToLower(mod.mod)
	var externalImports, imports = codegen.NewStringSet(), map[string]codegen.StringSet{}
	...
	var inputName = path.Join(modDir, "input.ts")
	var outputName = path.Join(modDir, "output.ts")
	var inputs, outputs = newIOFile(inputName), newIOFile(outputName)
	var allFiles = []*ioFile{inputs, outputs}
	mod.genHeader(inputs.writer(), mod.sdkImports(true, false), externalImports, imports)
	mod.genHeader(outputs.writer(), mod.sdkImports(true, false), externalImports, imports)

	// Build a namespace tree out of the types, then emit them.
	// We emit these namespaces as individual files if SplitTypes is enabled.
	// Otherwise, they are written to /types/{input, output}.ts, and defined
	// as nested TypeScript namespaces.
	namespaces := mod.getNamespaces()

	if err := mod.genNamespace(inputs.writer(), namespaces[""], true, 0); err != nil {
		return allFiles, err
	}
	if err := mod.genNamespace(outputs.writer(), namespaces[""], false, 0); err != nil {
		return allFiles, err
	}
