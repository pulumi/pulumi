...
type Foo struct {
	...
	Key    gcpiamv1.KeyOutput `pulumi:"key"`
	Policy dns.PolicyOutput   `pulumi:"policy"`
}
...
func NewFoo(ctx *pulumi.Context,
	...
	if args.AuditConfig == nil {
		return nil, errors.New("invalid value for required argument 'AuditConfig'")
	}
	if args.DnsKeySpec == nil {
		return nil, errors.New("invalid value for required argument 'DnsKeySpec'")
	}
	if args.Key == nil {
	...
	var resource Foo
	err := ctx.RegisterResource("example::Foo", name, args, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
...
func GetFoo(ctx *pulumi.Context,
	...
	var resource Foo
	err := ctx.ReadResource("example::Foo", name, id, state, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
...
type fooArgs struct {
	AuditConfig gcpiamv1.AuditConfig `pulumi:"auditConfig"`
	DnsKeySpec  dns.DnsKeySpec       `pulumi:"dnsKeySpec"`
	Key         *gcpiamv1.Key        `pulumi:"key"`
	Policy      *dns.Policy          `pulumi:"policy"`
}

// The set of arguments for constructing a Foo resource.
type FooArgs struct {
	AuditConfig gcpiamv1.AuditConfigInput
	DnsKeySpec  dns.DnsKeySpecInput
	Key         gcpiamv1.KeyInput
	Policy      dns.PolicyInput
}
