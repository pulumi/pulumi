...
func (pm pluginManager) EnsureSpec(ctx context.Context, spec workspace.PluginSpec) (string, error) {
	...
	if err := ensureSpec(ctx, dag, &spec, root); err != nil {
		return "", err
	}
...
func ensureSpec(ctx context.Context, dag *pdag.DAG[step], spec *workspace.PluginSpec, root pdag.Node) error {
	if spec.Version == nil {
		var err error
...
func (step downloadAndUnpackSpecStep) run(ctx context.Context, _ pluginManager) error {
	...
	if errors.Is(err, workspace.ErrPluginNotFound) ||
		...
		binary := filepath.Join(dir, "pulumi-resource-"+step.spec.Name)
...
func ensureProjectDependencies(
	...
	for name, spec := range project.GetPackageSpecs() {
		...
		if plugin.IsLocalPluginPath(ctx, spec.Source) {
			...
			specIsInstalled, ready = dag.NewNode(noOpStep{})
			defer ready()
			if err := ensureSpec(ctx, dag, &pluginSpec, specIsInstalled); err != nil {
				return err
			}
			ready()
		}
...
func (step genAndLinkInstalledPackageSpecStep) run(ctx context.Context, pm pluginManager) error {
	...
	schema, spec, err := load.SchemaFromSchemaSource(ctx, step.spec.Source,
	...
	*step.spec = spec
	...
	return link.LinkInto(ctx, step.project, step.projectDir, schema, sink, pm.host)
}
...
func downloadPluginFromSpecToTarball(ctx context.Context, spec *workspace.PluginSpec) (PluginContent, error) {
	...
	return TarPlugin(downloadedFile), nil
}
