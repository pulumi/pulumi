...
func SchemaFromSchemaSource(
	...
	if ext := filepath.Ext(packageSource); slices.Contains(encoding.Exts, ext) {
		if !parameters.Empty() {
			return nil, workspace.PackageSpec{}, fmt.Errorf("parameterization arguments are not supported for %s files", ext)
		}
		...
		return &pkgSpec, workspace.PackageSpec{Source: packageSource}, encoder.Unmarshal(f, &pkgSpec)
	}
	...
	if !parameters.Empty() {
		if p.AlreadyParameterized {
			return nil, workspace.PackageSpec{}, fmt.Errorf("cannot specify parameters since %s is already parameterized", packageSource)
		}
		...
		// TODO: SchemaFromSchemaSource should convey the pasteurization when
		// parameters is ParameterizeValue.
		if args, ok := parameters.(*plugin.ParameterizeArgs); ok {
			spec.Parameters = args.Args
		}
	}
...
func ProviderFromSource(
	...
	setupProvider := func(
		...
		if descriptor.Parameterization != nil {
			_, err := p.Parameterize(ctx, plugin.ParameterizeRequest{
			...
			if err != nil {
				return Provider{}, workspace.PackageSpec{}, fmt.Errorf("failed to parameterize %s: %w", p.Provider.Pkg().Name(), err)
			}
		}
	...
	case packageresolution.ExternalSourceResult, packageresolution.InstalledInWorkspaceResult:
		return setupProvider(descriptor, workspace.PackageSpec{
