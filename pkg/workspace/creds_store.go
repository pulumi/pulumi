...
const PulumiCredentialsPathEnvVar = sdkWorkspace.PulumiCredentialsPathEnvVar

const BookkeepingDir = sdkWorkspace.BookkeepingDir

// Account holds the information associated with a Pulumi account. Aliased to the sdk type so
// that credential files are described by a single Go type on both sides of the sdk/pkg boundary.
type Account = sdkWorkspace.Account

// Credentials hold the information necessary for authenticating Pulumi Cloud API requests.
// Aliased to the sdk type — see Account.
type Credentials = sdkWorkspace.Credentials

// TokenInformation describes the token used to authenticate the current user.
type TokenInformation = sdkWorkspace.TokenInformation

func GetPulumiHomeDir() (string, error) {
	return sdkWorkspace.GetPulumiHomeDir()
}
...
// getAccountAt loads an account from the given credentials file.
