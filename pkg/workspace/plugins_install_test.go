...
// prepareTestDir creates an installed plugin in PULUMI_HOME/plugins.
//
...
func testDeletePlugin(t *testing.T, plugin workspace.PluginInfo) {
	paths := []string{
		plugin.Path,
		plugin.Path + ".partial",
		plugin.Path + ".lock",
...
func TestInstallNoDeps(t *testing.T) {
	...
	err := InstallPluginContent(context.Background(), plugin, workspace.TarPlugin(tarball), false)
	require.NoError(t, err)
...
func TestReinstall(t *testing.T) {
	...
	err = InstallPluginContent(context.Background(), plugin, workspace.TarPlugin(tarball), true)
	require.NoError(t, err)
...
func TestConcurrentInstalls(t *testing.T) {
	...
	assertSuccess := func() workspace.PluginInfo {
		pluginInfo := assertPluginInstalled(t, dir, plugin)
