...
func InstallPluginContent(ctx context.Context, spec workspace.PluginSpec, content PluginContent, reinstall bool) error {
	...
	lock, err := store.LockSpec(ctx, spec)
	if err != nil {
		return err
	}
	defer func() { contract.IgnoreError(lock.Unlock()) }()
	...
	if _, err := store.Dir(ctx, spec); err == nil {
		...
		// The directory already exists, it is correctly installed, and we are not
		// re-installing, so the plugin is already installed.
		if !isPartial && !reinstall {
			return nil
		}
	...
	if err := store.SetPartial(ctx, spec); err != nil {
		return err
	}

	if err := store.Write(ctx, spec, content); err != nil {
		return err
	}

	err = installDependenciesForPluginSpec(ctx, store, spec, os.Stderr /* redirect stdout to stderr */, os.Stderr)
	if err != nil {
		return err
	}

	// Installation is complete. Remove the partial file.
	return store.RemovePartial(ctx, spec)
}

func installDependenciesForPluginSpec(
	ctx context.Context, store pluginstorage.Store, spec workspace.PluginSpec,
	stdout, stderr io.Writer,
) error {
	dir, err := store.Dir(ctx, spec)
	if err != nil {
		return err
	}
