...
func (c *compiler) discoverClusterArch(w workspace.W, stack *ast.Stack) (*ast.Cluster, backends.Arch) {
	...
	if c.opts.Cluster != "" {
		...
		if cl, exists := stack.Clusters[c.opts.Cluster]; exists {
			...
			if cl, exists := w.Settings().Clusters[c.opts.Cluster]; exists {
				...
				c.Diag().Errorf(errors.ErrorClusterNotFound.At(stack), c.opts.Cluster)
				return nil, arch
			}
		}
	}
	...
	if cluster == nil {
		...
		if arch.Cloud == clouds.None {
			c.Diag().Errorf(errors.ErrorMissingTarget.At(stack))
			return nil, arch
		}
...
func (c *compiler) getClusterArch(stack *ast.Stack, cluster *ast.Cluster,
	...
	if cluster.Cloud != "" {
		...
		if !ok {
			c.Diag().Errorf(errors.ErrorUnrecognizedCloudArch.At(stack), cluster.Cloud)
	...
	if cluster.Scheduler != "" {
		...
		if !ok {
			c.Diag().Errorf(errors.ErrorUnrecognizedSchedulerArch.At(stack), cluster.Scheduler)
			return existing, false
		}
		targetScheduler = ts
	}
	...
	if targetCloud != existing.Cloud && existing.Cloud != clouds.None {
		c.Diag().Errorf(
			errors.ErrorConflictingClusterArchSelection.At(stack), existing, cluster.Name, tarch)
		return tarch, false
	}
