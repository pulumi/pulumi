...
func TestTodo(t *testing.T) {
	...
	require.Equal(t, 2, table.State.Inputs.Len())
	assert.Equal(t, "id", table.State.Inputs.Get("primaryKey").AsString())
	...
	require.Equal(t, 5, endpoint.State.Inputs.Len())
	assert.Equal(t,
		"https://eupwl7wu4i.execute-api.us-east-2.amazonaws.com/", endpoint.State.Inputs.Get("url").AsString())
...
func TestCrawler(t *testing.T) {
	...
	require.Equal(t, 1, heartbeat.State.Inputs.Len())
	assert.Equal(t, "rate(5 minutes)", heartbeat.State.Inputs.Get("scheduleExpression").AsString())
	...
	require.Equal(t, 1, function.State.Inputs.Len())
