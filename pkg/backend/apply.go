...
type Applier func(ctx context.Context, kind apitype.UpdateKind, stack Stack, op UpdateOperation,
	opts ApplierOptions, events chan<- engine.Event1) (*deploy.Plan, sdkDisplay.ResourceChanges, result.Result)
...
func PreviewThenPrompt(ctx context.Context, kind apitype.UpdateKind, stack Stack,
	...
	eventsChannel := make(chan engine.Event1)
...
func confirmBeforeUpdating(kind apitype.UpdateKind, stack Stack,
	events []engine.Event1, opts UpdateOptions) result.Result {
	for {
		var response string
...
func createDiff(updateKind apitype.UpdateKind, events []engine.Event1, displayOpts display.Options) string {
	buff := &bytes.Buffer{}
