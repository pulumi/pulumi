...
func TestStateMigrationPatchesRewriteBeforeEncryptionAndReplayVerbatim(t *testing.T) {
	t.Parallel()

	// This test covers secret handling during migration and replay. References inside secrets must be rewritten while
	// plaintext is available, then encrypted in both the base and new-state patches. Replay must then apply those
	// prepared ciphertexts. After decryption references in both replayed patch states must point to the successor.
	//
	// The simulated update replaces `predecessor` with `successor`. An existing resource from the base snapshot and a
	// resource created earlier in the current update both have a secret output that references `predecessor`, so the
	// migration must patch one base state and one new state.
