...
// policyPackVersionTagPattern is the regex pattern for allowable verison tags
// of a Policy Pack.
...
func (pc *Client) PublishPolicyPack(ctx context.Context, orgName string,
	...
	if analyzerInfo.VersionTag != "" && !policyPackVersionTagRE.MatchString(analyzerInfo.VersionTag) {
		msg := fmt.Sprintf("invalid version %q - version may only contain alphanumeric, hyphens, underscores, periods, or colons. "+
			"It must also be between 1 and 150 characters long.", analyzerInfo.VersionTag)
