...
func (pc *Client) ListOrgTemplates(ctx context.Context, org string) (apitype.ListTemplatesResult, error) {
	var resp apitype.ListTemplatesResult
	if err := pc.restCall(ctx, "GET", "/api/orgs/"+url.PathEscape(org)+"/templates", nil, nil, &resp); err != nil {
		return apitype.ListTemplatesResult{}, err
	}
...
func (t *TarReaderCloser) Close() error { return t.data.Close() }
...
func (pc *Client) DownloadOrgTemplate(ctx context.Context, org, sourceURL string) (*TarReaderCloser, error) {
	...
	header.Add("Accept-Encoding", "application/x-tar")
	...
	return &TarReaderCloser{Reader: *tar.NewReader(resp), data: resp}, nil
}
