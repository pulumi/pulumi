...
func (u *cloudUpdate) recordEngineEvents(startingSeqNumber int, events []sdkDisplay.Event) error {
...
func (u *cloudUpdate) RecordAndDisplayEvents(
	...
	events <-chan sdkDisplay.Event, done chan<- bool, opts display.Options, isPreview bool) {
	...
	displayEvents := make(chan sdkDisplay.Event) // Note: unbuffered, but we assume it won't matter in practice.
	displayEventsDone := make(chan bool)

	persistEvents := make(chan sdkDisplay.Event, 100)
...
func persistEngineEvents(
	...
	events <-chan sdkDisplay.Event, done chan<- bool) {
	...
	var eventBatch []sdkDisplay.Event
