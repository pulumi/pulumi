...
func (b *cloudBackend) createAndStartUpdate(
	...
	if caps := b.Capabilities(ctx); op.CoherenceWindow != "" && (!caps.CoherenceWindows || !caps.StackOutputs) {
...
func (b *cloudBackend) runEngineAction(
	...
	if status == apitype.UpdateStatusSucceeded {
		...
		if outputs, outputsErr = outputsRecorder.response(ctx, op.SecretsManager); outputsErr != nil {
			status = apitype.UpdateStatusFailed
		}
	}
	...
	if outputsErr != nil {
		updateErr = result.MergeBails(updateErr, fmt.Errorf("recording stack outputs: %w", outputsErr))
	}
