...
func (b *cloudBackend) BeginApply(
	...
	op *backend.UpdateOperation,
	opts backend.ApplierOptions,
) (backend.Application, chan<- engine.Event, <-chan bool, error) {
	...
	u, tokenSource, err := b.newUpdate(ctx, stack.Ref(), *op, update, updateMeta.leaseToken)
	if err != nil {
		return nil, nil, nil, err
	}
	...
	return a, appEvents, appEventsDone, err
}
...
func (a *cloudApplication) UpdateInfo() engine.UpdateInfo {
	return a.info
}
...
func (a *cloudApplication) CreateParentSpanContext() (opentracing.SpanContext, error) {
	if parentSpan := opentracing.SpanFromContext(a.ctx); parentSpan != nil {
		return parentSpan.Context(), nil
	}
