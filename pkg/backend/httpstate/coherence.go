...
func (r *stackOutputsRecorder) record(e engine.Event) {
	if r == nil || e.Type != engine.ResourceOutputsEvent {
		return
	}
...
func (r *stackOutputsRecorder) response(
	...
	r.mu.Lock()
	outputs, seen := r.outputs, r.seen
	r.mu.Unlock()
	if !seen {
		return nil, nil
	}

	serialized, err := stack.SerializeProperties(ctx, outputs, sm.Encrypter(), false /* showSecrets */)
	if err != nil {
