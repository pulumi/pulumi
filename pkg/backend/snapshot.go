...
type SnapshotManager struct {
	...
	// The set of URNs this plan has produced new states for. Normally each URN is stepped at most
	// once per plan and completion order is a valid topological order; workflow node reconciles can
	// step the same URN several times in one plan (e.g. a node re-entered around a cycle), which
	// re-appends dependencies (like a slice's default provider) after resources recorded earlier
	// that refer to them. Seeing a repeat sets needsToposort so Snap restores a valid order.
...
func (sm *SnapshotManager) Snap() *deploy.Snapshot {
	...
	if sm.needsToposort {
		...
		// dependency order: restore one. A sort failure (e.g. a genuinely dangling reference) is
		// left for integrity verification to report on the unsorted snapshot.
