...
// StateMigration persists the complete post-migration checkpoint through the ordinary full-snapshot path. Unlike the
// journal manager, this manager shares resource pointers with the deployment and does not need a durable migration
// record: once saved, the checkpoint simply contains the migration's resulting state.
//
// PreparedPriorResources supplies the rewritten prior-state resource list. The manager combines it with its own live
// operation inventory, which may contain authoritative state that is not present in Deployment.news or
// Deployment.reads, and temporarily exposes those prepared values to saveSnapshot. This reuses the normal encryption,
// integrity checking, repair, and persister path without mutating shared resource pointers before the save succeeds.
// Afterward, it copies the same prepared rewrites into its live operation objects so their pointer identities remain
// valid for existing bookkeeping.
