...
func (r *JournalReplayer) Add(entry apitype.JournalEntry) {
	...
	case apitype.JournalEntryKindSuccess:
		...
		if entry.Delete != nil {
			r.markAsDeletion[*entry.Delete] = struct{}{}
		}
		if entry.RemoveNew != nil {
			r.toRemove[*entry.RemoveNew] = struct{}{}
		}
		if entry.PendingReplacement != nil {
			r.markAsPendingReplacement[*entry.PendingReplacement] = struct{}{}
		}
...
func rebuildDependencies(resources []apitype.ResourceV3) {
	...
	for i := range resources {
		...
		if !referenceable[resources[i].DeletedWith] {
			referenceable[resources[i].DeletedWith] = true
		}
		resources[i].Dependencies = newDeps
		resources[i].PropertyDependencies = newPropDeps
		referenceable[resources[i].URN] = true
...
func (r *JournalReplayer) GenerateDeployment() *apitype.DeploymentV3 {
...
func (sj *snapshotJournaler) snap() (*deploy.Snapshot, error) {
	...
	// TODO: we need to deal with features and version here I think..
	deploymentV3 := replayer.GenerateDeployment()
...
func NewSnapshotJournaler(
	...
	// TODO: deal with metadata and error
	deployment, _, _, _ := stack.SerializeDeploymentWithMetadata(
