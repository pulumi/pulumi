...
func ShowJSONEvents(events <-chan event.Event, done chan<- bool, opts Options) {
...
func ShowPreviewDigest(events <-chan event.Event, done chan<- bool, opts Options) {
	// Ensure we close the done channel before exiting.
	defer func() { close(done) }()
	...
	for e := range events {
		...
		case event.PreludeEvent:
			...
			digest.Config = e.Payload().(event.PreludeEventPayload).Config
		...
		case event.StdoutColorEvent:
			...
			p := e.Payload().(event.StdoutEventPayload)
			digest.Diagnostics = append(digest.Diagnostics, display.PreviewDiagnostic{
		...
		case event.ResourcePreEvent:
			...
			if m := e.Payload().(event.ResourcePreEventPayload).Metadata; shouldShow(m, opts) || isRootStack(m) {
		...
		case event.ResourceOutputsEvent, event.ResourceOperationFailed:
		...
		case event.SummaryEvent:
			...
			p := e.Payload().(event.SummaryEventPayload)
