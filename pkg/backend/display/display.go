...
func logJSONEvent(encoder *json.Encoder, event sdkDisplay.Event, opts Options, seq int) error {
...
func startEventLogger(events <-chan sdkDisplay.Event, done chan<- bool, opts Options) (<-chan sdkDisplay.Event, chan<- bool) {
	...
	outEvents, outDone := make(chan sdkDisplay.Event), make(chan bool)
	go func() {
		defer close(done)
		defer func() {
...
func isRootStack(step sdkDisplay.StepEventMetadata) bool {
...
func shouldShow(step sdkDisplay.StepEventMetadata, opts Options) bool {
