...
func testProgressEvents(t *testing.T, path string, accept, interactive bool, width, height int) {
	...
	go ShowProgressEvents("test", "update", "stack", "project", eventChannel, doneChannel, Options{
		...
		Stdout:               &stdout,
		Stderr:               &stderr,
		term:                 terminal.NewMockTerminal(&stdout, width, height),
...
func TestProgressEvents(t *testing.T) {
	...
	for _, entry := range entries {
		...
		t.Run(entry.Name()+"interactive", func(t *testing.T) {
			...
			for _, dim := range dimensions {
				...
				t.Run(fmt.Sprintf("%vx%v", width, height), func(t *testing.T) {
					...
					testProgressEvents(t, path, accept, true, width, height)
				})
			}
		})
		...
		t.Run(entry.Name()+"non-interactive", func(t *testing.T) {
			...
			testProgressEvents(t, path, accept, false, 80, 24)
		})
	}
}
