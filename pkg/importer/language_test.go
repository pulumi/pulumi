...
func TestGenerateLanguageDefinitionsGeneratesRemoteComponents(t *testing.T) {
	t.Parallel()
	...
	componentURN := resource.NewURN("dev", "project", "", "component:index:Callable", "callable")
	childURN := resource.NewURN(
		"dev",
		"project",
	...
	snapshot := []*pkgresource.State{
		{
			ID:     "123",
			Custom: true,
			Type:   "pulumi:providers:component",
			URN:    "urn:pulumi:stack::project::pulumi:providers:component::default_123",
		},
		{
			ID:     "456",
			Custom: true,
			Type:   "pulumi:providers:random",
			URN:    "urn:pulumi:stack::project::pulumi:providers:random::default_456",
		},
	}
	...
	resources := []apitype.ResourceV3{
		...
		},
		{
			...
			Type:     "random:index/randomPet:RandomPet",
			Parent:   componentURN,
			Provider: fmt.Sprintf("%s::%s", snapshot[1].URN, snapshot[1].ID),
		},
	}
	...
	err := GenerateLanguageDefinitions(io.Discard, loader, generator, states, snapshot, nil)
	require.NoError(t, err)
	expectedCode := `package component {
    baseProviderName = "component"

}

resource callable "component:index:Callable" {
    value = "hello"
