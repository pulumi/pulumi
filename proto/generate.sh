...
# `status.proto` and `error_details.proto` are in our source tree so that we can implement initialization failure in the
...
$DOCKER_RUN /bin/bash -c 'set -x && GO_PULUMIRPC=/go && \
    PROTO_FILES=$(find . -name "*.proto" -not -name "status.proto" -not -name "error_details.proto") && \
