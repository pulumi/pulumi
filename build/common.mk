...
GO_TEST_FAST = PATH="$(PULUMI_BIN):$(PATH)" $(PULUMI_ROOT)/scripts/go-test.sh -short -count=1 -cover -tags=all -timeout 1h -parallel ${TESTPARALLELISM}
GO_TEST = PATH="$(PULUMI_BIN):$(PATH)" $(PULUMI_ROOT)/scripts/go-test.sh -count=1 -cover -timeout 1h -tags=all -parallel ${TESTPARALLELISM}
