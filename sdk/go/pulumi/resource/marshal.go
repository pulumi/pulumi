...
func MarshalProperties(props PropertyMap, opts MarshalOptions) (*structpb.Struct, error) {
...
func MarshalPropertyValue(v PropertyValue, opts MarshalOptions) (*structpb.Value, error) {
	if v.IsNull() {
		...
		secret := NewObjectProperty(PropertyMap{
...
func marshalUnknownProperty(elem PropertyValue, opts MarshalOptions) *structpb.Value {
...
func UnmarshalProperties(props *structpb.Struct, opts MarshalOptions) (PropertyMap, error) {
...
func UnmarshalPropertyValue(v *structpb.Value, opts MarshalOptions) (*PropertyValue, error) {
	...
	case *structpb.Value_BoolValue:
		m := NewBoolProperty(v.GetBoolValue())
		return &m, nil
	case *structpb.Value_NumberValue:
		m := NewNumberProperty(v.GetNumberValue())
		return &m, nil
	...
	case *structpb.Value_StructValue:
		...
		objmap := obj.Mappable()
		sig, hasSig := objmap[string(SigKey)]
		if !hasSig {
			// This is a weakly-typed object map.
			m := NewObjectProperty(obj)
			return &m, nil
		}
		...
		case AssetSig:
			...
			asset, isasset, err := DeserializeAsset(objmap)
			if err != nil {
				return nil, err
			}
			...
			m := NewAssetProperty(asset)
			return &m, nil
		case ArchiveSig:
			if opts.RejectAssets {
				return nil, errors.New("unexpected Asset Archive property value")
			}
			archive, isarchive, err := DeserializeArchive(objmap)
			if err != nil {
				return nil, err
			}
			// This can only be false with a non-nil error if there is a signature match. We've already verified the
		...
		case SecretSig:
			...
			return &s, nil
		default:
			return nil, fmt.Errorf("unrecognized signature '%v' in property map", sig)
		}
...
func unmarshalUnknownPropertyValue(s string, opts MarshalOptions) (PropertyValue, bool) {
	var elem PropertyValue
	var unknown bool
	switch s {
	case UnknownBoolValue:
		elem, unknown = NewBoolProperty(false), true
	case UnknownNumberValue:
		elem, unknown = NewNumberProperty(0), true
	case UnknownStringValue:
		elem, unknown = NewStringProperty(""), true
	case UnknownArrayValue:
		elem, unknown = NewArrayProperty([]PropertyValue{}), true
	case UnknownAssetValue:
		elem, unknown = NewAssetProperty(&Asset{}), true
	case UnknownArchiveValue:
		elem, unknown = NewArchiveProperty(&Archive{}), true
	case UnknownObjectValue:
		elem, unknown = NewObjectProperty(make(PropertyMap)), true
	}
	if unknown {
		comp := Computed{Element: elem}
		return NewComputedProperty(comp), true
	}
...
func MarshalAsset(v *Asset, opts MarshalOptions) (*structpb.Value, error) {
	...
	sera := v.Serialize()
	serap := NewPropertyMapFromMap(sera)
	return MarshalPropertyValue(NewObjectProperty(serap), opts)
}

// MarshalArchive marshals an archive into its wire form for resource provider plugins.
func MarshalArchive(v *Archive, opts MarshalOptions) (*structpb.Value, error) {
	// If we are not providing access to an asset's contents, we simply need to record the fact that this asset existed.
	// Serialize the asset with only its hash (if present).
	if opts.ElideAssetContents {
		v = &Archive{Hash: v.Hash}
	} else {
		// Ensure a hash is present if needed.
		if v.Hash == "" && opts.ComputeAssetHashes {
			if err := v.EnsureHash(); err != nil {
