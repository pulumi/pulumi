...
func (ctx *Context) prepareResourceInputs(props Input, t string,
	...
	parent, optDeps, protect, provider, deleteBeforeReplace,
		importID, ignoreChanges, additionalSecretOutputs, patch, err := ctx.getOpts(t, providers, opts)
	if err != nil {
...
func (ctx *Context) getOpts(t string, providers map[string]ProviderResource, opts *resourceOptions) (
	URN, []URN, bool, string, bool, ID, []string, []string, bool, error) {

	importIDInput := opts.Patch
	if importIDInput == nil {
		importIDInput = opts.Import
	}

	var importID ID
	if importIDInput != nil {
		id, _, _, err := importIDInput.ToIDOutput().awaitID(context.TODO())
		if err != nil {
			return "", nil, false, "", false, "", nil, nil, false, err
		}
		importID = id
	}
	...
	return parentURN, depURNs, opts.Protect, providerRef, opts.DeleteBeforeReplace,
		importID, opts.IgnoreChanges, opts.AdditionalSecretOutputs, opts.Patch != nil, nil
}
