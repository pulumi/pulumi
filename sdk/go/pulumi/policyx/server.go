...
func Main(policyPack func(engine.Host) (PolicyPack, error)) error {
...
type analyzerServer struct {
	...
	policyPackFactory func(engine.Host) (PolicyPack, error)
...
func (srv *analyzerServer) Handshake(
	...
	host, err := engine.NewHost(req.GetEngineAddress())
	if err != nil {
...
func (a *analyzerServer) GetPluginInfo(context.Context, *pbempty.Empty) (*pulumirpc.PluginInfo, error) {
	return &pulumirpc.PluginInfo{
		Version: a.policyPack.Version().String(),
	}, nil
}
...
func (a *analyzerServer) GetAnalyzerInfo(context.Context, *pbempty.Empty) (*pulumirpc.AnalyzerInfo, error) {
	...
	for _, p := range a.policyPack.Policies() {
		...
		case ResourceValidationPolicy:
			...
			policyManager.reportViolation = func(message string, urn string) {
				...
				ds = append(ds, &pulumirpc.AnalyzeDiagnostic{
					PolicyName:        p.Name(),
					PolicyPackName:    a.policyPack.Name(),
					PolicyPackVersion: a.policyPack.Version().String(),
					Description:       p.Description(),
			...
			pm, err := plugin.UnmarshalProperties(req.GetProperties(), plugin.MarshalOptions{
				Label:            fmt.Sprintf("%s.%s.analyze", a.policyPack.Name(), p.Name()),
				KeepUnknowns:     true,
				KeepSecrets:      true,
				KeepResources:    true,
...
func (a *analyzerServer) AnalyzeStack(ctx context.Context, req *pulumirpc.AnalyzeStackRequest) (*pulumirpc.
	AnalyzeResponse,
	error,
) {
