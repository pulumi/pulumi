...
func assertAppliedPromise[T any](t *testing.T, out Promise[T]) {
	_, known, _, _, err := out.getPromiseState().await(context.Background())
	assert.True(t, known)
	assert.Nil(t, err)
}
...
func TestBasicPromises(t *testing.T) {
	...
	// Just test basic resolve and reject functionality.
	{
		...
		v, known, secret, deps, err := out.getPromiseState().await(context.Background())
		assert.Nil(t, err)
		assert.True(t, known)
...
func TestSecretApplyPromise(t *testing.T) {
	...
	s1, resolve1, _ := newPromise[string]()
	resolve1("foo")
	s1.Secret()
	// assert that secret is immediately secret
	assert.True(t, s1.IsSecret())
	s2, resolve2, _ := newPromise[string]()
	resolve2("bar")
...
func TestApplyPromise(t *testing.T) {
	...
	{
		out3 := Join(Apply(out1, func(v string) Promise[int] {
			return out2
		}))
...
func TestApplyPromiseConvertToOutput(t *testing.T) {
	...
	}()
	{
		...
		out3 := out1.ToOutput().ApplyT(func(v string) (AnyOutput, error) {
			return out2.ToOutput(), nil
		})
