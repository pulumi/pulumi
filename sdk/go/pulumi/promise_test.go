...
func assertAppliedPromise[T any](t *testing.T, out Promise[T]) {
	_, known, _, _, err := out.await(context.Background())
	assert.True(t, known)
	assert.Nil(t, err)
}
...
func TestBasicPromises(t *testing.T) {
	t.Parallel()

	// Just test basic resolve and reject functionality.
	{
		out, resolve, _ := newPromise[int]()
		go func() {
			resolve(42)
		}()
		v, known, secret, deps, err := out.await(context.Background())
		assert.Nil(t, err)
		assert.True(t, known)
...
func TestArrayPromises(t *testing.T) {
	...
	out, resolve, _ := newPromise[[]any]()
	go func() {
		resolve([]any{nil, 0, "x"})
	}()
	{
		assertAppliedPromise(t, Apply(out, func(arr []any) struct{} {
			assert.NotNil(t, arr)
...
func TestBoolPromises(t *testing.T) {
	t.Parallel()

	out, resolve, _ := newPromise[bool]()
	go func() {
		resolve(true)
	}()
	{
		assertAppliedPromise(t, ApplyErr(out, func(v bool) (interface{}, error) {
			assert.True(t, v)
			return nil, nil
		}))
	}
...
func TestMapPromises(t *testing.T) {
	t.Parallel()

	out, resolve, _ := newPromise[map[string]any]()
	go func() {
	...
	{
		assertAppliedPromise(t, ApplyErr(out, func(v map[string]any) (interface{}, error) {
			assert.NotNil(t, v)
...
func TestNumberPromises(t *testing.T) {
	t.Parallel()

	out, resolve, _ := newPromise[float64]()
	go func() {
		resolve(42.345)
	}()
	{
		assertAppliedPromise(t, ApplyErr(out, func(v float64) (interface{}, error) {
...
// 		}()
// 		v, _, _, _, err := await(out)
// 		assert.NotNil(t, err, "Expected err to be non-nil, got nil. Value is: %#v", out.getState().inner)
// 		assert.Nil(t, v)
// 	}
// }
...
// 	assert.True(t, ok)
// 	assert.Equal(t, uint32(outputResolved), io.inner.state)
// 	assert.Equal(t, 42, io.inner.value)
...
// 	assert.True(t, ok)
// 	assert.Equal(t, uint32(outputResolved), bo.getState().inner.state)
// 	assert.Equal(t, true, bo.inner.value)
// }
...
// 	assert.True(t, ok)
// 	assert.Equal(t, uint32(outputResolved), so.inner.state)
// 	assert.Equal(t, "a stringy output", so.inner.value)
// 	assert.ElementsMatch(t, []Resource{stringDep1, stringDep2}, so.inner.deps)

// 	io, ok := argsV.I.(IntOutput)
// 	assert.True(t, ok)
// 	assert.Equal(t, uint32(outputResolved), io.inner.state)
// 	assert.Equal(t, 42, io.inner.value)
// 	assert.ElementsMatch(t, []Resource{intDep1, intDep2}, io.inner.deps)
...
// 	assert.Equal(t, uint32(outputResolved), bo.getState().inner.state)
// 	assert.Equal(t, true, bo.inner.value)
// 	assert.ElementsMatch(t, []Resource{boolDep1, boolDep2}, bo.inner.deps)
...
func TestSecretApplyPromise(t *testing.T) {
	t.Parallel()

	s1 := ToPromise("foo").Secret()
	// assert that secret is immediately secret
	assert.True(t, s1.IsSecret())
	s2 := ToPromise("bar").Secret()

	errChan := make(chan error)
	resultChan := make(chan string)
	secretChan := make(chan bool)

	s := Apply(PromiseAll(s1, s2), func(v []string) string {
		return strings.Join(v, "")
	})
	Apply(s, func(v string) string {
		// assert secretness after the output resolves
		...
		if v == "foobar" {
			...
			errChan <- fmt.Errorf("Invalid result: %v", v)
		}
		return v
	})
...
// func TestPromiseDeps(t *testing.T) {
// 	t.Parallel()

// 	stringDep1, stringDep2 := &ResourceState{}, &ResourceState{}
// 	stringOut, stringResolve, _ := newPromise[string](stringDep1)
// 	assert.ElementsMatch(t, []Resource{stringDep1}, stringOut.getPromiseState().deps)
// 	go func() {
// 		stringResolve("hello")
// 	}()

// 	boolDep1, boolDep2 := &ResourceState{}, &ResourceState{}
// 	boolOut, boolResolve, _ := newPromise[bool](boolDep1)
// 	assert.ElementsMatch(t, []Resource{boolDep1}, boolOut.getPromiseState().deps)
// 	go func() {
// 		boolResolve(true)
// 	}()
...
// 	o1.resolve(0, true, true, nil)
// 	o2.resolve(0, true, true, nil)
...
func TestApplyPromise(t *testing.T) {
	t.Parallel()
	...
	out1, _, _ := newPromise[string](r1)
	out2, _, _ := newPromise[int](r2)
	go func() {
		out1.fulfill("r1 output", true, false, []Resource{r3}, nil)
		out2.fulfill(42, true, false, []Resource{r4}, nil)
	}()
	{
		appliedOut := Apply(out1, func(v string) Promise[int] {
			return out2
		})
		assert.IsType(t, Promise[Promise[int]]{}, appliedOut)
		out3 := Join(appliedOut)
		assert.IsType(t, Promise[int]{}, out3)
...
// func TestApplyPromiseConvertToOutput(t *testing.T) {
// 	t.Parallel()
...
// 	out2, _, _ := newPromise[int](r2)
// 	go func() {
// 		out1.getPromiseState().fulfillValue("r1 output", true, false, []Resource{r3}, nil)
// 		out2.getPromiseState().fulfillValue(42, true, false, []Resource{r4}, nil)
// 	}()
// 	{
// 		// Use statically typed promises with dynamically typed applyT:
// 		out3 := out1.ApplyT(func(v string) (Output, error) {
// 			return out2, nil
// 		})
...
// func TestApplyOutputConvertToPromise(t *testing.T) {
// 	t.Parallel()
...
// 	}()
// 	{
// 		// Use dynamically typed outputs with statically typed apply:
// 		out3 := Join(Apply(out1.ToPromise(), func(v any) Promise[any] {
// 			return out2.ToPromise()
// 		}))
// 		v, _, _, deps, err := out3.getPromiseState().await(context.Background())
// 		assert.NoError(t, err)
// 		assert.Equal(t, 42, v)
// 		assert.Equal(t, fmt.Sprintf("%v", reflect.TypeOf(v)), "int")
// 		assert.Len(t, deps, 4)
...
// 	var depUrns []URN
// 	for _, v := range deps {
// 		depUrns = append(depUrns, v.URN().inner.value.(URN))
// 	}
// 	var expectedUrns []URN
// 	for _, v := range expectedDeps {
// 		expectedUrns = append(expectedUrns, v.URN().inner.value.(URN))
// 	}
