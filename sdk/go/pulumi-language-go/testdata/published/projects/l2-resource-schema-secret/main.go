...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		nested, err := output.NewComplexResource(ctx, "nested", &output.ComplexResourceArgs{
			Value: pulumi.Float64(1),
		}, pulumi.Provider(prov))
		if err != nil {
			return err
		...
		ctx.Export("nested", nested.OutputObject.ApplyT(func(outputObject output.Data) (string, error) {
			return outputObject.SecretOutput, nil
		}).(pulumi.StringOutput))
		return nil
	})
}
