...
func NewDefaultClient(apiToken string) Client {
	userAgent := fmt.Sprintf("esc-sdk/1 (%s; %s)", Version, runtime.GOOS)
...
// serviceUserInfo mirrors the Pulumi Cloud API's user-info payload.
...
// serviceUser mirrors the Pulumi Cloud API's user payload.
...
// serviceTokenInfo mirrors the Pulumi Cloud API's token-info payload.
...
func envHooksPath(orgName, projectName, envName string, parts ...string) string {
	...
	var baseSb1256 strings.Builder
	for _, p := range parts {
		baseSb1256.WriteString("/" + p)
	}
	base += baseSb1256.String()
	return base
}
