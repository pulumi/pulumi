...
import (
	...
	"github.com/pulumi/pulumi/sdk/v3/go/esc/schema"
...
func (echoRotator) Open(ctx context.Context, inputs, state map[string]esc.Value, context esc.EnvExecContext) (esc.Value, error) {
	if !inputs["next"].Unknown && inputs["next"].Value != state["current"].Value {
		return esc.Value{}, errors.New("oh no! inputs were evaluated outside of rotation")
	}
...
func (echoRotator) Rotate(ctx context.Context, inputs, state map[string]esc.Value, context esc.EnvExecContext) (esc.Value, error) {
	...
	next := inputs["next"]
	if next.Unknown {
		return esc.Value{}, errors.New("oh no! rotateOnly inputs were not evaluated during rotation")
	}
	...
	for _, e := range entries {
		...
		t.Run(e.Name(), func(t *testing.T) {
			...
			if accept() {
				...
				if doRotate {
					rotated, rotationResult, rotateDiags = RotateEnvironment(t.Context(), environmentName, env, rot128{}, testProviders{},
			...
			var rotated *esc.Environment
			if doRotate {
				rotated_, rotationResult, diags := RotateEnvironment(t.Context(), environmentName, env, rot128{}, testProviders{},
					&testEnvironments{basePath}, execContext, rotatePaths)
				var patches []*Patch
...
func benchmarkEval(b *testing.B, openDelay, loadDelay time.Duration) {
	...
	for i := 0; i < b.N; i++ {
		...
		_, evalDiags := EvalEnvironment(b.Context(), environmentName, env, rot128{}, testProviders{benchDelay: openDelay},
			envs, execContext)
