...
func TestCrypt(t *testing.T) {
	...
	for _, e := range entries {
		...
		t.Run(e.Name(), func(t *testing.T) {
			...
			if accept() {
				encrypted, err := EncryptSecrets(t.Context(), baseName, plaintextBytes, rot128{})
				require.NoError(t, err)

				err = os.WriteFile(ciphertextPath, encrypted, 0o600)
				require.NoError(t, err)

				decrypted, err := DecryptSecrets(t.Context(), baseName, encrypted, rot128{})
				require.NoError(t, err)
...
`
	...
	_, err := DecryptSecrets(t.Context(), "doc", []byte(doc), rot128{})
	assert.Error(t, err)
}
...
`
	...
	_, err := EncryptSecrets(t.Context(), "doc", []byte(doc), broken{})
	assert.Error(t, err)
}
...
`
	...
	_, err := DecryptSecrets(t.Context(), "doc", []byte(doc), broken{})
	assert.Error(t, err)
}
...
func TestEncryptDecryptSecret(t *testing.T) {
	...
	decrypted, err := DecryptSecret(ctx, crypter, encoded)
	require.NoError(t, err)
