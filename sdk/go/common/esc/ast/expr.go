			...
			if k := node.Index(i).Key.Value(); strings.HasPrefix(k, "fn::") {
				diags = append(
					...
					syntax.NodeError(
						node,
						fmt.Sprintf(
							"illegal call to builtin function: %q must be the only key within its containing object",
							k,
						),
					),
				)
...
func parseOpen(node *syntax.ObjectNode, name *StringExpr, args Expr) (Expr, syntax.Diagnostics) {
	...
	if !ok {
		diags := syntax.Diagnostics{
			ExprError(args, "the argument to fn::open must be an object containing 'provider' and 'inputs'"),
		}
...
func parseRotate(node *syntax.ObjectNode, name *StringExpr, args Expr) (Expr, syntax.Diagnostics) {
	...
	if !ok {
		diags := syntax.Diagnostics{
			ExprError(args, "the argument to fn::rotate must be an object containing 'provider', 'inputs' and 'state'"),
		}
...
func parseShortRotate(node *syntax.ObjectNode, name *StringExpr, args Expr) (Expr, syntax.Diagnostics) {
	kvp := node.Index(0)
	provider := StringSyntaxValue(
		name.Syntax().(*syntax.StringNode),
		strings.TrimPrefix(kvp.Key.Value(), "fn::rotate::"),
	)

	obj, ok := args.(*ObjectExpr)
	if !ok {
		diags := syntax.Diagnostics{
			ExprError(args, "the argument to fn::rotate must be an object containing 'inputs' and 'state'"),
		}
		return RotateSyntax(node, name, args, nil, nil, nil), diags
	}
...
func parseValidate(node *syntax.ObjectNode, name *StringExpr, args Expr) (Expr, syntax.Diagnostics) {
	...
	if !ok {
		diags := syntax.Diagnostics{
			ExprError(args, "the argument to fn::validate must be an object containing 'schema' and 'value'"),
		}
