...
func DecodeValue(v any) (syntax.Node, syntax.Diagnostics) {
...
func decodeValue(v reflect.Value) (syntax.Node, syntax.Diagnostics) {
	...
	for {
		...
		case reflect.Map:
			if v.Type().Key().Kind() != reflect.String {
				return nil, syntax.Diagnostics{
					syntax.Error(
						nil,
						fmt.Sprintf("cannot decode value of type %v (map keys must be strings)", v.Type()),
						"",
					),
				}
			}
		...
		default:
			return nil, syntax.Diagnostics{
				syntax.Error(nil, fmt.Sprintf("cannot decode value of type %v", v.Type()), ""),
			}
		}
	}
}
...
func EncodeValue(n syntax.Node, v any) syntax.Diagnostics {
...
func encodeValue(n syntax.Node, v reflect.Value) syntax.Diagnostics {
		...
		if v.CanSet() {
			...
			if !nv.Type().AssignableTo(v.Type()) {
				...
				return syntax.Diagnostics{
					syntax.Error(
						rng,
						fmt.Sprintf("cannot encode %v into location of type %v", nv.Type(), v.Type()),
						"",
					),
				}
			}
	...
	case *syntax.BooleanNode:
		...
		if v.Kind() != reflect.Bool {
			rng := n.Syntax().Range()
			return syntax.Diagnostics{syntax.Error(rng, fmt.Sprintf("cannot encode boolean into location of type %v", v.Type()), "")} //nolint:lll
		}
		v.SetBool(n.Value())
		return nil
	case *syntax.NumberNode:
		...
		default:
			...
			rng := n.Syntax().Range()
			return syntax.Diagnostics{syntax.Error(rng, fmt.Sprintf("cannot encode number into location of type %v", v.Type()), "")} //nolint:lll
		}
	case *syntax.StringNode:
		...
		if v.Kind() != reflect.String {
			rng := n.Syntax().Range()
			return syntax.Diagnostics{syntax.Error(rng, fmt.Sprintf("cannot encode string into location of type %v", v.Type()), "")} //nolint:lll
		}
		v.SetString(n.Value())
		return nil
	...
	case *syntax.ObjectNode:
		...
		default:
			...
			rng := syn.Range()
			path := syn.Path()
			return syntax.Diagnostics{syntax.Error(rng, fmt.Sprintf("cannot encode object into location of type %v", v.Type()), path)} //nolint:lll
		}
	default:
		panic("unreachable")
	}
