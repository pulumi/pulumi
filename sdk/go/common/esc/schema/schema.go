...
func (s *Schema) Item(index int) *Schema {
	oneOf := make([]*Schema, 0, len(s.AnyOf)+len(s.OneOf)+1)
...
func setRotateOnly(s *Schema) *Schema {
	...
	copy.Properties = maps.Collect(
		fxm.Map(s.Properties, func(k string, s *Schema) (string, *Schema) { return k, setRotateOnly(s) }),
	)
