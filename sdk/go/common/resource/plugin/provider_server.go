...
func (p *providerServer) CheckConfig(ctx context.Context,
	...
	newInputs, failures, err := p.provider.CheckConfig(urn, req.Name, req.Type, state, inputs, true)
	if err != nil {
...
func (p *providerServer) DiffConfig(ctx context.Context, req *pulumirpc.DiffRequest) (*pulumirpc.DiffResponse, error) {
	...
	diff, err := p.provider.DiffConfig(urn, req.Name, req.Type,
		oldInputs, oldOutputs, newInputs, true, req.GetIgnoreChanges())
	if err != nil {
...
func (p *providerServer) Check(ctx context.Context, req *pulumirpc.CheckRequest) (*pulumirpc.CheckResponse, error) {
	...
	newInputs, failures, err := p.provider.Check(urn, req.Name, req.Type, state, inputs, true, req.RandomSeed)
	if err != nil {
		return nil, err
	}
...
func (p *providerServer) Diff(ctx context.Context, req *pulumirpc.DiffRequest) (*pulumirpc.DiffResponse, error) {
	...
	diff, err := p.provider.Diff(urn, req.Name, req.Type,
		id, oldInputs, oldOutputs, newInputs, true, req.GetIgnoreChanges())
	if err != nil {
		return nil, err
	}
	return p.marshalDiff(diff)
...
func (p *providerServer) Create(ctx context.Context, req *pulumirpc.CreateRequest) (*pulumirpc.CreateResponse, error) {
	...
	id, state, _, err := p.provider.Create(urn, req.Name, req.Type, inputs, req.GetTimeout(), req.GetPreview())
	if err != nil {
		return nil, err
	}
...
func (p *providerServer) Read(ctx context.Context, req *pulumirpc.ReadRequest) (*pulumirpc.ReadResponse, error) {
	...
	result, _, err := p.provider.Read(urn, req.Name, req.Type, requestID, inputs, state)
	if err != nil {
		return nil, err
	}
...
func (p *providerServer) Update(ctx context.Context, req *pulumirpc.UpdateRequest) (*pulumirpc.UpdateResponse, error) {
	...
	newState, _, err := p.provider.Update(
		urn, req.Name, req.Type, id, oldOutputs, oldInputs, newInputs,
...
func (p *providerServer) Delete(ctx context.Context, req *pulumirpc.DeleteRequest) (*pbempty.Empty, error) {
	...
	if _, err = p.provider.Delete(urn, req.Name, req.Type, id, state, req.GetTimeout()); err != nil {
		return nil, err
	}
