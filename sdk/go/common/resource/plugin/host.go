...
type Logger interface {
	...
	Log(ctx context.Context, sev diag.Severity, urn resource.URN, msg string, streamID int32)
	...
	LogStatus(ctx context.Context, sev diag.Severity, urn resource.URN, msg string, streamID int32)
}
...
type Host interface {
	...
	// error is returned.
	Analyzer(ctx context.Context, nm tokens.QName) (Analyzer, error)
	...
	// well-known place.
	PolicyAnalyzer(ctx context.Context, name tokens.QName, path string, opts *PolicyAnalyzerOptions) (Analyzer, error)

	// LoadedAnalyzers returns a list of all analyzer plugins loaded by the host.
	LoadedAnalyzers() []Analyzer
	...
	Provider(ctx context.Context, pkg tokens.Package, version *semver.Version) (Provider, error)

	// CloseProvider closes the given provider plugin and deregisters it from this host.
	CloseProvider(ctx context.Context, provider Provider) error
	...
	LanguageRuntime(ctx context.Context, runtime string) (LanguageRuntime, error)

	// LoadedPlugins lists all plugins that have been loaded, with version information.
	LoadedPlugins() []workspace.PluginInfo
	...
	EnsurePlugins(ctx context.Context, plugins []workspace.PluginInfo, kinds Flags) error
...
func NewDefaultHost(ctx *Context, config ConfigSource, runtimeOptions map[string]interface{},
	...
	// Start serving an engine over gRPC.
	port, _, err := rpcutil.Serve(0, host.cancel, []func(*grpc.Server) error{
		func(srv *grpc.Server) error {
			pulumirpc.RegisterEngineServer(srv, NewEngineServer(NewDefaultEngine(host)))
			return nil
		},
	}, ctx.tracingSpan)
	if err != nil {
	...
	host.addr = fmt.Sprintf("127.0.0.1:%d", port)
...
type defaultHost struct {
	...
	cancel                  chan bool                        // the channel used to cancel the host's gRPC server.
	addr                    string                           // the address of the host's gRPC server.
}
...
func (host *defaultHost) Log(ctx context.Context, sev diag.Severity, urn resource.URN, msg string, streamID int32) {
	host.ctx.Diag.Logf(sev, diag.StreamMessage(urn, msg, streamID))
}

func (host *defaultHost) LogStatus(ctx context.Context,
...
func (host *defaultHost) loadPlugin(ctx context.Context, load func() (interface{}, error)) (interface{}, error) {
	...
	select {
	case host.loadRequests <- request:
...
func (host *defaultHost) Analyzer(ctx context.Context, name tokens.QName) (Analyzer, error) {
	plugin, err := host.loadPlugin(ctx, func() (interface{}, error) {
		...
		if err == nil && plug != nil {
			info, infoerr := plug.GetPluginInfo(ctx)
			if infoerr != nil {
				return nil, infoerr
			}
...
func (host *defaultHost) PolicyAnalyzer(ctx context.Context,
	name tokens.QName, path string, opts *PolicyAnalyzerOptions) (Analyzer, error) {
...
func (host *defaultHost) LoadedAnalyzers() []Analyzer {
	analyzers := []Analyzer{}
...
func (host *defaultHost) LanguageRuntime(ctx context.Context, runtime string) (LanguageRuntime, error) {
	plugin, err := host.loadPlugin(ctx, func() (interface{}, error) {
		// First see if we already loaded this plugin.
...
func (host *defaultHost) LoadedPlugins() []workspace.PluginInfo {
	return host.plugins
}

// EnsurePlugins ensures all plugins in the given array are loaded and ready to use.  If any plugins are missing,
// and/or there are errors loading one or more plugins, a non-nil error is returned.
func (host *defaultHost) EnsurePlugins(ctx context.Context, plugins []workspace.PluginInfo, kinds Flags) error {
	...
	for _, plugin := range plugins {
		...
		case workspace.AnalyzerPlugin:
			if kinds&AnalyzerPlugins != 0 {
				if _, err := host.Analyzer(ctx, tokens.QName(plugin.Name)); err != nil {
		...
		case workspace.LanguagePlugin:
			if kinds&LanguagePlugins != 0 {
				if _, err := host.LanguageRuntime(ctx, plugin.Name); err != nil {
					result = multierror.Append(result,
		...
		case workspace.ResourcePlugin:
			if kinds&ResourcePlugins != 0 {
				if _, err := host.Provider(ctx, tokens.Package(plugin.Name), plugin.Version); err != nil {
					result = multierror.Append(result,
...
func (host *defaultHost) SignalCancellation(ctx context.Context) error {
	// NOTE: we're abusing loadPlugin in order to ensure proper synchronization.
	_, err := host.loadPlugin(ctx, func() (interface{}, error) {
		var result error
		for _, plug := range host.resourcePlugins {
			if err := plug.Plugin.SignalCancellation(ctx); err != nil {
...
func (host *defaultHost) CloseProvider(ctx context.Context, provider Provider) error {
	// NOTE: we're abusing loadPlugin in order to ensure proper synchronization.
	_, err := host.loadPlugin(ctx, func() (interface{}, error) {
...
func GetRequiredPlugins(ctx context.Context, host Host, info ProgInfo, kinds Flags) ([]workspace.PluginInfo, error) {
	var plugins []workspace.PluginInfo

	if kinds&LanguagePlugins != 0 {
		// First make sure the language plugin is present.  We need this to load the required resource plugins.
		// TODO: we need to think about how best to version this.  For now, it always picks the latest.
		lang, err := host.LanguageRuntime(ctx, info.Proj.Runtime.Name())
		if err != nil {
		...
		if kinds&ResourcePlugins != 0 {
			...
			deps, err := lang.GetRequiredPlugins(ctx, info)
			if err != nil {
