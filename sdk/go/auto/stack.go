...
func tailLogs(command string, receivers []chan<- events.EngineEvent, version semver.Version) (Watcher, error) {
	if version.GT(semver.Version{Major: 3, Minor: 204}) {
...
func (ew *eventsWatcher) Close() {
	// No-op; gRPC server will be torn down elsewhere.
}
...
func (e *eventsServer) Event(ctx context.Context, req *pulumirpc.EventRequest) (*emptypb.Empty, error) {
...
func (e *eventsServer) Done(ctx context.Context, req *emptypb.Empty) (*emptypb.Empty, error) {
	for _, r := range e.receivers {
		close(r)
	}
	return &emptypb.Empty{}, nil
