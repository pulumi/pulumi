...
namespace Pulumi
{
    ...
    {
        void IDeployment.Call(string token, InvokeArgs args, Resource? self, InvokeOptions? options)
            => Call<object>(token, args, self, options, convertResult: false);

        Output<T> IDeployment.Call<T>(string token, InvokeArgs args, Resource? self, InvokeOptions? options)
            => Call<T>(token, args, self, options, convertResult: true);

        private Output<T> Call<T>(
            string token, InvokeArgs args, Resource? self, InvokeOptions? options, bool convertResult)
        {
        ...
        private async Task CallAsync<T>(
            string token, InvokeArgs args, Resource? self, InvokeOptions? options,
            ...
            string token, InvokeArgs args, Resource? self, InvokeOptions? options)
        {
            ...
            if (result.Failures.Count > 0)
            {
                ...
                throw new InvokeException($"Call of '{token}' failed: {reasons}");
            }
