...
namespace Pulumi
{
    ...
    public sealed class DeploymentInstance : IDeployment
    {
        ...
        /// <see cref="Task{TResult}"/>s, <see cref="Output{T}"/>s etc.).
        /// </summary>
        public Output<T> Call<T>(string token, InvokeArgs args, Resource? self = null, InvokeOptions? options = null)
            => _deployment.Call<T>(token, args, self, options);

        /// <summary>
        /// Same as <see cref="Call{T}"/>, however the return value is ignored.
        /// </summary>
        public void Call(string token, InvokeArgs args, Resource? self = null, InvokeOptions? options = null)
