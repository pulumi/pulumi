...
namespace Pulumi
{
    ...
    /// otherwise share the same key.  For example, a bag whose name is `pulumi:foo`, with keys `a`,
    /// `b`, and `c`, is entirely separate from a bag whose name is `pulumi:bar` with the same
    /// simple key names.  Each key has a fully qualified names, such as `pulumi:foo:a`, ..., and
    /// `pulumi:bar:a`, respectively.
    /// </summary>
    public class Config
{
    /**
