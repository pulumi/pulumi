...
func TestLanguage(t *testing.T) {
	...
	for _, tt := range tests.Tests {
		t.Run(tt, func(t *testing.T) {
			...
			// TODO for now just working on getting l1 tests passing
			if strings.HasPrefix(tt, "l3-") {
				t.Skip("Skipping L3 tests")
			}

			// Skip grpc-config
			if strings.HasPrefix(tt, "grpc-config") {
				t.Skip("Skipping grpc-config tests")
			}

			// TODO skip parameterized tests for now
			if strings.Contains(tt, "parameterized") {
				t.Skip("Skipping parameterized tests")
			}
