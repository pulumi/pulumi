...
def register_resource(
    ...
    async def do_register() -> None:
        try:
            ...
            lifecycle_hooks = await _prepare_lifecycle_hooks(opts.lifecycle_hooks)
...
async def _prepare_lifecycle_hooks(
    ...
    for hook_type in [
        ...
        hooks_for_type: list[Union[LifecycleHook, LifecycleHookFunction]] = getattr(
            hooks, hook_type, []
        )
        for _hook in hooks_for_type or []:
            hook = _hook
            ...
            if not isinstance(hook, LifecycleHook):
                ...
                name = str(uuid.uuid4())  # TODO could be e `{type}_{idx}`
