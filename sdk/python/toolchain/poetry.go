...
func dependenciesFromRequirementsTxt(r io.Reader, basePath string) (map[string]any, error) {
	...
	for scanner.Scan() {
		...
		if version == "" {
			// If the pkg references a local path, we have to create a
			// dependency entry that points to the path instead of a versioned
			// dependency.
			if info, err := os.Stat(pkg); err == nil {
				...
				if info.IsDir() {
					// Get the package name for the local package
					pyprojectPath := filepath.Join(pkg, "pyproject.toml")
					if pyprojectData, err := os.ReadFile(pyprojectPath); err == nil {
						...
						if err := toml.Unmarshal(pyprojectData, &pyproject); err == nil &&
							...
							deps[pkg] = map[string]string{"path": relPath}
						}
					}
				} else if strings.HasSuffix(pkg, ".whl") {
					// If it's a wheel file, we get the package name from the
					// wheel filename. By convention, everything up to the first
					// `-` is the package name.
