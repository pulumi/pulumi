...
func TestBackwardCompatibility_NoTypechecker(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// If toolchain is set, typechecker prompt should be included
			if _, hasToolchain := tt.options["toolchain"]; hasToolchain {
				foundTypechecker := false
				for _, prompt := range resp.Prompts {
					if prompt.Key == "typechecker" {
						foundTypechecker = true
						assert.Equal(t, "mypy", prompt.Default.StringValue)
						break
					}
				}
				assert.True(t, foundTypechecker, "typechecker prompt should be present when toolchain is set")
			}
		})
	}
}
