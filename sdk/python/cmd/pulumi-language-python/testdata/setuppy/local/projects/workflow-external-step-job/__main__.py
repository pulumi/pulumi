...
def register_workflows(registry: workflow.WorkflowRegistry) -> None:
    ...
    def register_job_build(job: workflow.JobContext, args: job_build_args) -> Output[bool]:
        build_invert_output = pulumi_simple_step_workflow.step_invert(job, pulumi_simple_step_workflow.step_invert_args(input=args.input), workflow.StepOptions(name="invert"))
