...
func TestRuntimeOptionsPrompts_Typechecker(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedKeys: []string{"virtualenv", "typechecker"},
			expectedDefaults: map[string]string{
				"virtualenv":  "venv",
				"typechecker": "mypy",
			},
			expectedChoices: map[string][]string{
				"virtualenv":  {"venv"},
				"typechecker": {"mypy", "pyright", "none"},
			},
		},
		{
			name:             "Poetry toolchain selected",
			options:          map[string]interface{}{"toolchain": "poetry"},
			expectedKeys:     []string{"typechecker"},
			expectedDefaults: map[string]string{"typechecker": "mypy"},
			expectedChoices: map[string][]string{
				"typechecker": {"mypy", "pyright", "none"},
			},
		},
...
func TestRuntimeOptionsPrompts_TypecheckerDescription(t *testing.T) {
	t.Parallel()

	host := &pythonLanguageHost{}

	// Test with pip toolchain to get typechecker prompt
	...
	require.NotNil(t, typecheckerPrompt, "Should have a typechecker prompt")
	assert.Equal(t, "The typechecker to use for type checking Python code", typecheckerPrompt.Description)
	assert.Equal(t, pulumirpc.RuntimeOptionPrompt_STRING, typecheckerPrompt.PromptType)
}
