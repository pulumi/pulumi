...
export const entriesOutput = res.outputMap.apply(outputMap => Object.entries(outputMap).sort(([a], [b]) => a.localeCompare(b)).map(([k, v]) => ({key: k, value: v})));
...
export const entriesObjectOutput = res.outputObject.apply(outputObject => Object.entries(outputObject).sort(([a], [b]) => a.localeCompare(b)).map(([k, v]) => ({key: k, value: v})));
