...
export async function getPackage() : Promise<string | undefined> {
	if (_packageRef === undefined) {
		...
		if (_packageRef === undefined) {
			...
			req.setName("extension");
			req.setVersion("17.17.17");
			req.setDownloadUrl("");

			const extension = new resproto.Parameterization();
			extension.setName("extended1");
			extension.setVersion("51.51.51");
			extension.setValue(Uint8Array.from(atob("ZXh0ZW5kZWQxLXBhcmFtZXRlcgo="), c => c.charCodeAt(0)));
