...
unsupported commands (e.g., `config env add` directs users to
`config edit`).

**Why this priority**: `config env` (bare) is the only way to discover
the linked environment name. `config edit` is the replacement for
`config env add/rm/ls`. `config web` provides quick access to the ESC
...
- What happens when `pulumi config env add`, `pulumi config env rm`, or `pulumi config env ls` are run against a service-backed stack? The CLI MUST return a hard error pointing to `pulumi config edit` or `pulumi config web`.
...
|----------------|----------------|
| `pulumi config env add` | Use `pulumi config edit` or `pulumi config web` |
| `pulumi config env rm` | Use `pulumi config edit` or `pulumi config web` |
| `pulumi config env ls` | Use `pulumi config edit`, `pulumi config web`, or `pulumi env get` |
...
- **FR-020**: `pulumi config env add/rm/ls` MUST return hard errors for service-backed stacks, directing users to `config edit` or `config web`.
