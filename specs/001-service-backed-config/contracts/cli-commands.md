...
## Service-Backed Environment Import Commands

For service-backed stacks, `config env ls/add/rm` operate on the ESC
environment's `imports` section directly (loaded via `GetEnvironment`,
saved via `UpdateEnvironmentWithProject` with etag concurrency).

- `config env ls` lists the imports from the ESC environment definition
- `config env add <env>` appends to the imports list and saves
- `config env rm <env>` removes from the imports list and saves

The add/rm commands show a before/after preview and prompt for
