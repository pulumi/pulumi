...
## Agent Instructions
 # What this repo is
* The core Pulumi SDK and CLI.
* Go monorepo
* multiple Go modules
* (pkg/, sdk/, tests/, etc.)
* language-specific SDKs
* (sdk/nodejs/, sdk/python/, sdk/go/, sdk/pcl/).
* Builds the pulumi CLI binary and language host binaries.
...
* backends
 ...
 # sdk/ — Language SDKs  
* shared Go SDK code
* (sdk/go/, sdk/nodejs/, sdk/python/, sdk/pcl/)
 
 # tests/ — Integration and acceptance tests
 
 # proto/ — Protobuf definitions for gRPC interfaces between engine, 
 language hosts, and providers
...
## Tool setup
* mise to manage tool
* versions (Go, Node, Python, protoc, etc.).
* See ```.mise.toml``` for the full list.
* If mise is installed and activated
(via mise activate in your shell profile),
tool versions are handled automatically can run make directly.
Otherwise, prefix all make commands with mise exec -- to ensure the correct tool versions are used:
