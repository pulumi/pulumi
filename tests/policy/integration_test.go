...
func TestValidateResource(t *testing.T) {
	runPolicyPackIntegrationTest(t, "validate_resource", nil, []policyTestScenario{
		...
		{
			WantErrors: []string{
				"[mandatory]  validate-resource-test-policy v0.0.1  " +
					"dynamic-no-state-with-value-1 (pulumi-nodejs:dynamic:Resource: a)",
		...
		// Test scenario 4: violates the second policy.
		{
			WantErrors: []string{
				"[mandatory]  validate-resource-test-policy v0.0.1  " +
					"dynamic-no-state-with-value-2 (pulumi-nodejs:dynamic:Resource: b)",
		...
		// Test scenario 5: violates the first validation function of the third policy.
		{
			WantErrors: []string{
				"[mandatory]  validate-resource-test-policy v0.0.1  " +
					"dynamic-no-state-with-value-3-or-4 (pulumi-nodejs:dynamic:Resource: c)",
		...
		// Test scenario 6: violates the second validation function of the third policy.
		{
			WantErrors: []string{
				"[mandatory]  validate-resource-test-policy v0.0.1  " +
					"dynamic-no-state-with-value-3-or-4 (pulumi-nodejs:dynamic:Resource: d)",
				"Prohibits setting state to 3 or 4 on dynamic resources.",
		...
		// Test scenario 7: violates the fourth policy.
		{
			WantErrors: []string{
				"[mandatory]  validate-resource-test-policy v0.0.1  " +
					"randomuuid-no-keepers (random:index/randomUuid:RandomUuid: r1)",
		...
		// Test scenario 9: violates the fifth policy.
		{
			WantErrors: []string{
				"[mandatory]  validate-resource-test-policy v0.0.1  " +
					"dynamic-no-state-with-value-5 (pulumi-nodejs:dynamic:Resource: e)",
