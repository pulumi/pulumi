...
func TestProjectMain(t *testing.T) {
	...
	t.Run("Error_AbsolutePath", func(t *testing.T) {
		...
		_, stderr := e.RunCommandExpectError("pulumi", "up", "--non-interactive", "--skip-preview")
		assert.Contains(t, stderr, "Updating (main-abs):")
	...
	t.Run("Error_ParentFolder", func(t *testing.T) {
		...
		_, stderr := e.RunCommandExpectError("pulumi", "up", "--non-interactive", "--skip-preview")
		assert.Contains(t, stderr, "Updating (main-parent):")
