...
func TestConcurrentCredentialsWrites(t *testing.T) {
	...
	oldCreds, err := workspace.GetStoredCredentials()
	assert.NoError(t, err)
	defer func() {
		err := workspace.StoreCredentials(oldCreds)
		assert.NoError(t, err)
	}()

	// use test creds that have at least 1 AccessToken to force a
	// disk write and contention
	testCreds := workspace.Credentials{
	...
	err = workspace.StoreCredentials(testCreds)
	assert.NoError(t, err)
