...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		// values are promises. Structs are not promises - a struct is equivalent to just splatting out
		// all of its child fields as keys or supporting `pulumi:"foo.bar"` tags, where foo is the
		// struct. Only *dynamic* structure is a promise (maps, arrays).
		//
