...
The main functionality of steps is in the `Apply` function. that is called,
the step is executed, which means the provider is called as appropriate
(depending on the type of step we're executing), and a completion function is
returned, which when called registers the result of the step.
...
Next we're going to describe the steps in a little more detail.
...
**Mutating Steps** directly modify resources via provider operations:
- **CreateStep**: Creates new resources
- **UpdateStep**: Updates existing resources
- **DeleteStep**: Deletes resources

**Non-Mutating Steps** represent logical operations:
- **SameStep**: Resource unchanged or skipped
- **ReplaceStep**: Logical marker for replacements (combination of Create +
  Delete)

**Special Steps** handle specific scenarios:
- **ReadStep**: Imports resources as external references
- **RefreshStep**: Updates state from provider
- **ImportStep**: Brings resources under Pulumi management
- **RemovePendingReplaceStep**: Cleanup step for pending replacements
- **DiffStep**: Parallel diff computation (not a real step)
- **ViewStep**: Virtual step for view resources (display only)
...
- **Old()**: State before the step (may be nil)
- **New()**: State after the step (may be nil)
...
`ViewStep`s are virtual steps for view resources (auxiliary resources providers
