...
The [](pulumirpc.Callback) message represents a callback reference and contains two essential pieces of information:

* **target** (`string`): The gRPC address of the callback service (e.g., the gRPC server started by the language SDK).
* **token** (`string`): A unique identifier for the specific function to invoke within that service. Language SDKs typically use UUIDs to ensure uniqueness.

When the engine needs to invoke a callback, it calls [](pulumirpc.Callbacks.Invoke) with a [](pulumirpc.CallbackInvokeRequest):

* **token** (`string`): The token identifying which function to invoke.
* **request** (`bytes`): A serialized protobuf message containing the arguments. The specific message type depends on the callback type (e.g., [](pulumirpc.TransformRequest) for transforms).
...
* **response** (`bytes`): A serialized protobuf message containing the result (e.g., [](pulumirpc.TransformResponse) for transforms).
